...
class RunArtifact:
    ...
    def _push_single_file(
        ...
        if artifact_type == 'metrics':
            to_yaml(self.metrics, buffer)
        elif artifact_type == 'examples':
            to_yaml(self.example_results, buffer)
        else:
    ...
    def push_metrics(self, artifact_id: Optional[str] = None):
        metrics_id = self._push_single_file('metrics', artifact_id=artifact_id)
        self.run_metadata.metrics = metrics_id

    def push_examples(self, artifact_id: Optional[str] = None):
        examples_id = self._push_single_file('examples', artifact_id=artifact_id)
        self.run_metadata.example_results = examples_id
