...
class Polygon(BaseArea):
    ...
    def render_multi_polygon_on_axis(self, multi_polygon, ax: Axis) -> None:
        """
    ...
    def render_simple_polygon_on_axis(self, polygon, ax: Axis) -> None:
        """
    ...
    def __raster__(self, coordinate: numpy.ndarray) -> numpy.ndarray:
        ...
        :rtype:     numpy.ndarray
        """
        Exterior = self.contains_points(coordinate=coordinate)
        ...
        if isinstance(hole, Iterable):
            for sub_hole in hole.geoms():
                hole_contained *= sub_hole.contains_points(coordinate)
        else:
            hole_contained = hole.contains_points(coordinate)
