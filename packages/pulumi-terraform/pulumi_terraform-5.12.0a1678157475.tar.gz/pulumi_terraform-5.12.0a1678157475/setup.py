...
class InstallPluginCommand(install):
    def run(self):
        install.run(self)
        try:
            check_call(['pulumi', 'plugin', 'install', 'resource', 'terraform', '5.12.0-alpha.1678157475+1cdcb0bd'])
            ...
            if error.errno == errno.ENOENT:
                ...
                `pulumi plugin install resource terraform 5.12.0-alpha.1678157475+1cdcb0bd`
                """)
