...
def _create_defaultdict(cls,default_factory,items,state):A=cls.__new__(cls);A.default_factory=default_factory;A.update(items);A.__dict__.update(state);return A
@pickle.reducer(defaultdict,_create_defaultdict,subclasses=True)
def pickle_defaultdict(obj):A=obj;return type(A),A.default_factory,dict(A),A.__dict__