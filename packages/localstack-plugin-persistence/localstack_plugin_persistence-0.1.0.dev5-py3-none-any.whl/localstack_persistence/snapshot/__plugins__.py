...
def do_load_strategy():
	A=get_load_strategy();from localstack_persistence.snapshot.manager import LoadOnRequestHandler as B
	match A:
		case LoadStrategy.ON_STARTUP:LOG.info('registering ON_STARTUP load strategy, restoring state now');get_service_state_manager().load_all()
		case LoadStrategy.ON_REQUEST:LOG.info('registering ON_REQUEST load strategy');C=B(get_service_state_manager());serve_custom_service_request_handlers.append(C.on_request)
