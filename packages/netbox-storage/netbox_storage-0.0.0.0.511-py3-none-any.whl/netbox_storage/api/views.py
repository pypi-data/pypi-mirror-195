...
from netbox_storage.api.serializers import (
    DriveSerializer,
    FilesystemSerializer, PartitionSerializer, LinuxVolumeSerializer, StorageConfigurationSerializer
)
from netbox_storage.filters import DriveFilter, FilesystemFilter, \
    PartitionFilter, LinuxVolumeFilter
...
class StorageConfigurationViewSet(NetBoxModelViewSet):
    queryset = StorageConfiguration.objects.all()
    serializer_class = StorageConfigurationSerializer
    filterset_class = StorageConfigurationFilter

    @action(detail=True, methods=["get"])
    def storageconfiguration(self, request, pk=None):
        storageconfiguration = StorageConfiguration.objects.filter(storageconfiguration__id=pk)
        serializer = StorageConfigurationSerializer(storageconfiguration, many=True, context={"request": request})
        return Response(serializer.data)
