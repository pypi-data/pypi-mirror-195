...
class MountedVolumeViewSet(NetBoxModelViewSet):
    ...
    @action(detail=True, methods=["get"])
    def mounted_volume(self, request, pk=None):
        mounted_volume = MountedVolume.objects.filter(mountedvolume__id=pk)
        serializer = MountedVolumeSerializer(mounted_volume, many=True, context={"request": request})
        return Response(serializer.data)
