...
class StorageConfiguration(NetBoxModel):
...
class Drive(NetBoxModel):
    ...
    storage_configuration = models.ForeignKey(
        StorageConfiguration,
        on_delete=models.CASCADE,
        related_name='storage_configuration_linux_volume',
    )

    clone_fields = ["cluster", "size", "identifier", "description", "storage_configuration"]
    ...
    def save(self, *args, **kwargs):
        ...
        if is_already_implemented == 0:
            number_of_hard_drives = Drive.objects.filter(storage_configuration=self.storage_configuration).\
...
class LogicalVolume(NetBoxModel):
    ...
    storage_configuration = models.ForeignKey(
        StorageConfiguration,
        on_delete=models.CASCADE,
        related_name='storage_configuration_logical_volume',
    )
