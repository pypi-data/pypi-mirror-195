...
def EKOX(x,n=INFO,pref="", color=False, check=False, withprint=True, llevel=0, pattern="EKOX",
    ...
    if prt :
        ...
        if (ff[4] != None) :
            ...
            if '\n' in strng :
                strng = '\n' + strng
            r = print1(preamb + tt(loc, n=n) + lab(n) + tkk + '.', color=color, noeko=not withprint)
        else:
            r = print1(preamb + tt(loc, n=n) + lab(n) + str(ff[3]) + '=' + inl + pref + strng + '.', color=color, noeko= not withprint)
