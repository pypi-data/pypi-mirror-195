...
template <typename T>
__global__ void SparseSegmentNMeanKernel(const Cuda2DLaunchConfig config,
                                         int64* output_total_size,
                                         int64* data_inner_dim, T** output,
                                         int32** seg_lens) {
  CUDA_AXIS_KERNEL_LOOP(g_idx, config.virtual_thread_count.y, Y) {
    CUDA_AXIS_KERNEL_LOOP(g_offset, config.virtual_thread_count.x, X) {
      if (g_offset < output_total_size[g_idx]) {
        const int64 output_row = g_offset / data_inner_dim[g_idx];
        T count = T(ldg(seg_lens[g_idx] + output_row));
        output[g_idx][g_offset] /= (count > T(0.0) ? count : T(1.0));
      }
    }
  }
}
...
template <typename T>
__global__ void SparseSegmentSqrtNNKernel(const Cuda2DLaunchConfig config,
                                          int64* output_total_size,
                                          int64* data_inner_dim, T** output,
                                          int32** seg_lens) {
  CUDA_AXIS_KERNEL_LOOP(g_idx, config.virtual_thread_count.y, Y) {
    CUDA_AXIS_KERNEL_LOOP(g_offset, config.virtual_thread_count.x, X) {
      if (g_offset < output_total_size[g_idx]) {
        const int64 output_row = g_offset / data_inner_dim[g_idx];
        T count = T(ldg(seg_lens[g_idx] + output_row));
        output[g_idx][g_offset] /=
            static_cast<T>(std::sqrt(count > T(0.0) ? count : T(1.0)));
      }
    }
  }
}
...
void SparseSegmentReductionNFunctorBase<T, Index>::operator()(
  ...
  for (int i = 0; i < valid_num_columns; ++i) {
    ctx->allocate_temp(DT_INT32, TensorShape{output_row_sizes[i]},
                       const_cast<Tensor*>(seg_lengths_vec[i]));
  }

  prep_segids_functor(ctx, seg_lengths_ptr_heads, seg_lengths_ptr_heads_dev,
                      &seg_lengths_vec, valid_num_columns);
  functors.sum_lengths(max_output_row_size, valid_num_columns,
                       output_row_sizes_dev, seg_lengths_ptr_heads_dev,
                       max_indices_num, indices_num_dev, segids_ptr_heads_dev,
                       d);
  Reduce(max_output_total_size, valid_num_columns, output_total_sizes_dev,
         outputs_ptr_heads_dev, data_inner_dims_dev, seg_lengths_ptr_heads_dev,
         d);
...
#define DEFINE_SEGMENT_REDUCTION_OPS(T)                         \
  template struct SparseSegmentReductionNFunctorBase<T, int32>; \
  template struct SparseSegmentReductionNFunctorBase<T, int64>;
TF_CALL_SEGMENT_REDUCTION_TYPES(DEFINE_SEGMENT_REDUCTION_OPS);
#undef DEFINE_SEGMENT_REDUCTION_OPS

template <typename T, typename Index>
void SparseSegmentMeanNFunctor<T, Index>::Reduce(
    int64 max_output_total_size, int32 valid_num_columns,
    int64* output_total_sizes_dev, int8* outputs_ptr_heads_dev,
    int64* data_inner_dims_dev, int8* seg_lengths_ptr_heads_dev,
    const Eigen::GpuDevice& d) {
  Cuda2DLaunchConfig conf2d =
      GetCuda2DLaunchConfig(max_output_total_size, valid_num_columns, d);
  SparseSegmentNMeanKernel<T>
      <<<conf2d.block_count, conf2d.thread_per_block, 0, d.stream()>>>(
          conf2d, output_total_sizes_dev, data_inner_dims_dev,
          reinterpret_cast<T**>(outputs_ptr_heads_dev),
          reinterpret_cast<int32**>(seg_lengths_ptr_heads_dev));
}

#define DEFINE_SEGMENT_REDUCTION_OPS(T)                \
  template struct SparseSegmentMeanNFunctor<T, int32>; \
  template struct SparseSegmentMeanNFunctor<T, int64>;
TF_CALL_SEGMENT_REDUCTION_TYPES(DEFINE_SEGMENT_REDUCTION_OPS);
#undef DEFINE_SEGMENT_REDUCTION_OPS

template <typename T, typename Index>
void SparseSegmentSqrtNNFunctor<T, Index>::Reduce(
    int64 max_output_total_size, int32 valid_num_columns,
    int64* output_total_sizes_dev, int8* outputs_ptr_heads_dev,
    int64* data_inner_dims_dev, int8* seg_lengths_ptr_heads_dev,
    const Eigen::GpuDevice& d) {
...
#define DEFINE_SEGMENT_REDUCTION_OPS(T)                 \
  template struct SparseSegmentSqrtNNFunctor<T, int32>; \
  template struct SparseSegmentSqrtNNFunctor<T, int64>;
TF_CALL_SEGMENT_REDUCTION_REAL_TYPES(DEFINE_SEGMENT_REDUCTION_OPS);
#undef DEFINE_SEGMENT_REDUCTION_OPS
