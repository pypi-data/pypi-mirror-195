...
class Cvt_RtMap_SetIPv6NxtHopVrfy(DiffConvert_RtMap_Entry):
    cmd = tuple()
    ext = "set", "ipv6-next-hop-verify-availability"

    def remove(self, old, rtmap, seq):
        l = self.enter(rtmap, new, seq)
        l.append(" no set ipv6 next-hop verify-availability")
        return l

    def update(self, old, upd, new, rtmap, seq):
        l = self.enter(rtmap, new, seq)
        l.append(" set ipv6 next-hop verify-availability")
        return l


class Cvt_RtMap_SetIPNxtHopVrfyTrk(DiffConvert_RtMap_Entry):
    cmd = tuple()
    ext = "set", "ipv6-next-hop-verify-availability", None

    def _cmd(self, seq, nexthop):
        return ("set ipv6 next-hop verify-availability %s %s track %d"
                     % (nexthop["addr"], seq, nexthop["track-obj"]))

class Cvt_RtMap_SetIPv6NxtHopVrfyTrk_Del(Cvt_RtMap_SetIPNxtHopVrfyTrk):
    block = "rtmap-del"
...
class Cvt_RtMap_SetIPv6NxtHopVrfyTrk_Add(Cvt_RtMap_SetIPNxtHopVrfyTrk):
    block = "rtmap-add"
