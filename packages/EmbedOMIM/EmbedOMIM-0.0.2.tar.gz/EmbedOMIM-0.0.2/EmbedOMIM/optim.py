...
class AutoEncoderOptimizer:
    ...
    def __init__(self,vae_model,dataset_training_state,optimizationParameters={'maxLearningRate': 0.05,'maxEpochs': 5000,'numParticles':10},computeConfiguration={'device':None,'numDataLoaders':0}, **kwargs):
        """
        ...
        **kwargs : See source code.
        ...
        if 'OneCycleParams' not in allKeywordArgs:
            ...
            assert set(self.KLAnnealingParams.keys())==set(['pctCycleIncrease','initLRDivisionFactor','finalLRDivisionFactor']),"OneCycleParams must be dictionary with the following keys: 'pctCycleIncrease','initLRDivisionFactor','finalLRDivisionFactor'."
    ...
    def _contstructSVI(self,total_steps):
        """Builds SVI function for model inference.
        

        -------
        TYPE dict
            Dictionary containing the SVI function and scheduler for the learning rate.
        """
        ...
        return {'svi':SVI(self.vae_model.model,self.vae_model.guide,scheduler, loss=Trace_ELBO(num_particles=self.numParticles)),'scheduler':scheduler}


    def BatchTrain(self,batch_size:int,errorTol:float = 1e-3,verbose=True,logFile=None,errorComputationWindow=None,early_stop_patience=5):
        """
        ...
        sviFunction = self._contstructSVI(len(torchTrainingData)*self.maxEpochs)
        ...
        for i,data_batch in enumerate(validationDataLoader):
            ...
            avg_epoch_val_loss+=sviFunction['svi'].evaluate_loss(*data_batch,minibatch_scale = (numTotalValidationSamples/data_batch[0].shape[0]),annealing_factor=self.KLAnnealingParams['maxTemp'])
        ...
        for epoch in range(self.maxEpochs):
            ...
            elif avg_epoch_val_loss<bestModelScore:
                bestModelState = self.vae_model.package_state()
