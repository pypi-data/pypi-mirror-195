...
class Fixed_Linear_Decoder_wTranslator(nn.Module):
    def __init__(self, decoding_matrix,n_output):
        """Decodes a latent space into the paramters defining the logit-probabilities for a set of diagnoses. The dimension of the diagnosis space is determined from the decoding_matrix provided by the user (dim=1). Note, the decoding matrix is NOT updated during inference. 
        ...
        """
        super(Fixed_Linear_Decoder_wTranslator,self).__init__()
        self.linear_latent=FixedWeights_Linear(decoding_matrix.shape[1],decoding_matrix.shape[0],decoding_matrix,bias=False)
        self.linear_translator=nn.Linear(decoding_matrix.shape[0],n_output,bias=True)

    def _set_bias(self,bias_vec):
        with torch.no_grad():
            self.linear_translator.bias[:]=bias_vec[:]

    def forward(self,latent_var):
        return self.linear_translator(self.linear_latent(latent_var))
