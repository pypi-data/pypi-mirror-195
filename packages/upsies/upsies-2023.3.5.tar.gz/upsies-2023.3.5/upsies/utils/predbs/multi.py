...
class MultiPredbApi:
    ...
    DEFAULT_PREDB_NAMES = ('corruptnet', 'srrdb')
    ...
    async def search(self, *args, **kwargs):
        """
        Search multiple predbs concurrently and combine the results

        See :meth:`~.PredbApiBase.search`.
        """
        results = set()
        exceptions = []
        async for result in self._for_all_predbs('search', *args, **kwargs):
            if isinstance(result, Exception):
                exceptions.append(result)
            else:
                # Return first non-empty search results.  If a fast-but-
                # incomplete predb (e.g. corruptnet) finds search results, we
                # return them quickly. Otherwise,, we wait for a slow-but-
                # complete predb (e.g. srrdb) to maybe find results.
