...
class Experiment:
    ...
    reproduction_rules: ReproductionRules = ReproductionRules()
...
def run(experiment: Experiment, spark_context: SparkContext) -> Generation:
    ...
    logging.info('Breeding initial generation...')
    ...
    while not _is_final_generation(
        ...
        logging.info(f'Generation #{generation.generation_number} is not final, experiment continues...')
        generation = breed_next_generation(
            spark_context=spark_context,
            chromosome_factories=experiment.chromosome_factories,
            fitness_evaluator=experiment.fitness_evaluator,
    ...
    logging.info(f'Experiment completed at generation #{generation.generation_number}!')
    return generation
