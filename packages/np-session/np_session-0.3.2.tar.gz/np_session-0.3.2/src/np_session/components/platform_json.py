...
def update_platform_json_fields(pj: PlatformJson, session) -> None:
    ...
    with pj.write_disabled():
        update('operatorID', session.user.id)
        update('sessionID', session.id)
        update('mouseID', session.mouse.id)
        update('stimulus_name', session.lims['stimulus_name'])
        if pj.script_name:
            with contextlib.suppress(Exception):
                update('script_name', np_config.local_to_unc(session.rig.stim, pj.script_name))
        update('foraging_id', session.foraging_id)
