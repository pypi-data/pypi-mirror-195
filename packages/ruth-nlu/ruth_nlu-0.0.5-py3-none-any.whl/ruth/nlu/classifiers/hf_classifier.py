...
class HFClassifier(IntentClassifier):
    ...
    def required_element(self):
        """Return the required element for the classifier."""
        return [HFTokenizer]
    ...
    def get_input_ids(message: RuthData) -> Dict[Text, List[int]]:
        """Get the input ids from the message."""
    ...
    def get_attention_masks(message: RuthData) -> Dict[Text, List[int]]:
        """Get the attention masks from the message."""
    ...
    def persist(self, file_name: Text, model_dir: Path) -> dict:
        """Persist the model. save the model and the label encoder."""
    ...
    def predict_probabilities(self, input_ids, attention_masks) -> ndarray:
...
class HFDatasetLoader(Dataset):
    ...
    def __getitem__(self, idx) -> Dict[Text, Any]:
        """Return the item of the dataset."""
