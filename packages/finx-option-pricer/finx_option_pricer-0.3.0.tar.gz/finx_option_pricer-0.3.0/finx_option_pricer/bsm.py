...
def bs_call_value(S, K, T, r, sigma):
    """calc (dollar) value of call option

    Args:
        S (float): Spot price
        K (float): Strike price
        T (float): time, annualized (180 days => 0.5 years)
        r (float): risk free rate for T
    ...
    try:
        ...
        print("(np.log(S / K) + (r + sigma ** 2 / 2) * T) / (sigma * np.sqrt(T))")
        print(f"S = {S}")
        print(f"K = {K}")
        print(f"r = {r}")
        print(f"sigma = {sigma}")
