...
def datatables_view(cursor_connection:str = None, logger:str = None, tablename:str = None, target_column:list = None, column_name:list = None, values:list = None, limit:int = None, page:int = None, order:str = None, 
                        order_tipe:str = None, tipe:int = None, group_values:list = None, disable_row_count:bool = None, menu_name:str = None, extra_condition:list = None):
    ...
    extra_condition => sebuah list yang berisikan string code untuk menentukan kodisi sebuah query misalnya beetween di tandai dengan 'bw', is null ditandai dengan 'inull', is not null ditandai dengan 'inotnull'
            jika tidak ada kondisi khusus atau lisnya kosong, maka akan diisi dnegan '=' jika ada kombinasi antara keempatnya ex : ['bw','inull','inostnull',False], false disini dianggap menjadi = 
    """
    data = None
    rows_count = 0
    ...
    spesific_list = []
    if extra_condition:
        __lencolumn = len(column_name) + extra_condition.count('bw')
        if __lencolumn != len(values):
            if logger:
    ...
    for idx, column in enumerate(column_name):
        try:
            extra_con = extra_condition[idx]
            if str(extra_con).lower() == 'bw':
                spesific_list.append(f" {column} between %s and %s ")
            elif str(extra_con).lower() == 'inull':
                spesific_list.append(f" {column} is %s ")
            elif str(extra_con).lower() == 'inotnull':
                spesific_list.append(f" {column} is not %s ")
            else:
                spesific_list.append(f" {column} = %s ")
        except Exception as e:
            spesific_list.append(f" {column} = %s ")
            continue
        spesific_list.append(f" {column} = %s ")

    valid_columns = " AND ".join(spesific_list) if column_name else ""
...
action_return_of_response = return_of_response
action_additional_response = additional_response
action_build_message_detail = build_message_detail
action_generate_headers = generate_headers
action_generate_footers = generate_footers
action_datatables_view = datatables_view
action_check_existing_data = check_existing_data
action_update_data = update_data
action_insert_data = insert_data
