...
class Infer:
    def __init__(self,output_dir='output',print_args=False,save_result=True):
        # paddle.enable_static()
        parser = argsparser()

        parser.add_argument("--output_dir", type=str, default=output_dir, help="Directory of output visualization files.")
        parser.add_argument("--model_dir", type=str, default='../model', help='')
        parser.add_argument("--image_dir", type=str, default=None, help="Dir of image file, `image_file` has a higher priority.")
        ...
        FLAGS.save_results = save_result
        self.detector = eval(self.detector_func)(
            FLAGS.model_dir,
            device=FLAGS.device,
            run_mode=FLAGS.run_mode,
            batch_size=FLAGS.batch_size,
    ...
    def infer(self,img_file):
        # self.parser.add_argument("--image_file", type=str, default=img_file, help="Path of image file.")

        # img_list = get_test_images(FLAGS.image_dir, FLAGS.image_file)
        img_list = [img_file]
        return self.detector.predict_image(
            self.FLAGS,
            img_list,
            self.FLAGS.run_benchmark,
            repeats=100,
            visual=self.FLAGS.save_images,
            save_results=self.FLAGS.save_results)["boxes"]

if __name__ == '__main__':
    i = Infer(save_result = False)
    r = i.infer("H:\\AI\\ddpg\\images\\36.jpg")
    for dt in r:
