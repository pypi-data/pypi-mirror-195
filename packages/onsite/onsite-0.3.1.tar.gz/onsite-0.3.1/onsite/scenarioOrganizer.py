...
class ScenarioOrganizer():
    ...
    def load(self, input_dir: str, output_dir: str) -> None:
        ...
        if 'visualize' not in self.config['test_settings'].keys():
            self.config['test_settings']['visualize'] = False

        # 如果测试模式 == 'replay'，读取文件夹下所有待测场景，存在列表中
        if self.test_mode == 'replay':  # 判断测试模式是否为replay
            if 'skip_exist_scene' not in self.config['test_settings'].keys():
                self.config['test_settings']['skip_exist_scene'] = False
            for item in os.listdir(input_dir):
                if item.split(".")[-1] != 'py':
                    if item != "__pycache__" and item[0] != '.':
                        if self.config['test_settings']['skip_exist_scene'] and os.path.exists(os.path.join(output_dir, item+'.csv')):
                            continue
