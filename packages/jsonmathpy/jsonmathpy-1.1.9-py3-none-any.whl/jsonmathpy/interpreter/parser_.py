...
class Parser:
    ...
    def limit_node(self):
        ...
        if self.current_token.type != TokenType.COMMA:
            self.raise_error("Syntax Error, expecting a COMMA token.")
        self.advance()
        independent_variable = self.object()
        if self.current_token.type != TokenType.COMMA:
            self.raise_error("Syntax Error, expecting a COMMA token.")
        self.advance()
        limit = self.object()
        if self.current_token.type != TokenType.RPAREN:
            self.raise_error("Syntax Error, expecting a RPAREN token.")
        return LimitNode(expression_to_integrate, independent_variable, limit)
    ...
    def series_node(self):
        ...
        if self.current_token.type != TokenType.RPAREN:
            self.raise_error("Syntax Error, expecting a RPAREN token.")
        return SeriesNode(expression_to_integrate, x0, order)
