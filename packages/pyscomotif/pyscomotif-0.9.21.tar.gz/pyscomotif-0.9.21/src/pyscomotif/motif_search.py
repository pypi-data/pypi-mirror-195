...
def get_all_pairs_of_residues_in_motif_MST(motif_MST: nx.Graph, order: str = 'dfs') -> Dict[Tuple[str,str], Residue_pair_data]:
    """
    Returns all the residue pairs in motif_MST in an ordered manner such that, for all i, residue pairs between 0 and i form a connected graph. The order can 
    be that of a Depth First Search (dfs) of Breadth First Search (bfs). If the residue pairs were randomly ordered, we wouldn't be able to do the connectivity check 
    in update_map_of_PDBs_with_all_residue_pairs, because the logic requires that new residue pairs are connected to the previous nodes.
    """
    MST_edges: List[Tuple[str, str]] # Ex: [('A1G', 'A3K'), ('A3K', 'A8N'), ...]
    if order == 'dfs':
        MST_edges = list(nx.dfs_edges(motif_MST))
    elif order == 'bfs':
        MST_edges = list(nx.bfs_edges(motif_MST))
    else:
        raise ValueError(f" The 'order' parameter must be either 'dfs' or 'bfs', but {order} was given.")
...
def update_map_of_PDBs_with_all_residue_pairs(
    ...
    for PDB_ID in list(map_of_PDBs_with_all_residue_pairs.keys()):
        # Remove PDBs that no longer have all the residue pairs
        ...
        # Connectivity check. If there isn't any residue pair in the new residue pairs that is connected to already visited nodes, then we can also drop this PDB
        if not any(residue_pair_is_connected_to_visited_nodes(residue_pair, visited_nodes_map[PDB_ID]) for residue_pair in new_residue_pair_data[PDB_ID]):
            del map_of_PDBs_with_all_residue_pairs[PDB_ID]
            del visited_nodes_map[PDB_ID]
            continue

        list_of_residue_pairs = new_residue_pair_data[PDB_ID]
        map_of_PDBs_with_all_residue_pairs[PDB_ID].extend(list_of_residue_pairs) # Update the list of residue pairs of the PDBs that have all the residue pairs
        visited_nodes_map[PDB_ID].update(set(itertools.chain.from_iterable(list_of_residue_pairs)))
...
def find_PDBs_with_all_residue_pairs(
    ...
    all_pairs_of_residues_to_check = get_all_pairs_of_residues_in_motif_MST(motif_MST, order='dfs')
    ...
    for future in submitted_futures:
        ...
        if map_of_PDBs_with_all_residue_pairs_initialized_flag == False:
            ...
            visited_nodes_map = {
                PDB_ID:set(itertools.chain.from_iterable(list_of_residue_pairs))
...
def solve_motif_MST(
    ...
    map_of_PDBs_with_all_residue_pairs_initialized_flag: bool = False
    visited_nodes_map: Dict[str, Set[str]] # Used for connectivity check
    for pair_of_full_residue_IDs, residue_pair_data in get_all_pairs_of_residues_in_motif_MST(motif_MST, order='dfs').items():
