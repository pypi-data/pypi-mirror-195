...
comment_xrefs = _read_json(Path(STATIC_PATH, COMMENT_XREF_FILE))
strategy_xrefs = _read_json(Path(STATIC_PATH, STRATEGY_XREF_FILE))
comments = _read_json(Path(STATIC_PATH, LOCALE, COMMENTS_FILE_NAME))
strategies = _read_json(Path(STATIC_PATH, LOCALE, STRATEGIES_FILE_NAME))
...
def strategy_html(call_id):
    """Return the basic i18n code for the comment associated with the call_id."""
    comment_id = comment_xrefs[call_id]
    if not comment_id in strategy_xrefs:
