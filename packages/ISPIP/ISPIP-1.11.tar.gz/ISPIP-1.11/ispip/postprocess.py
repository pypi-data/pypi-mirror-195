...
def analyses(params) -> tuple:
    ...
    fscores = test_frame.groupby(["protein"]).apply( lambda x: compute_fscore(x, annotated_col, pred))
    mccs= test_frame.groupby(["protein"]).apply( lambda x: compute_mcc(x, annotated_col, pred))
