...
THIRD_TYPE_CONFIG = {
    ...
    "dingtalk": {
        ...
        # 获取飞书登录授权的地址（钉钉的）
        "api": "https://login.dingtalk.com/oauth2/auth?redirect_uri={redirect_uri}&response_type=code&client_id={appid}&scope=openid&state={state}&prompt=consent",

        # 确认登录的跳转地址（自己的）
        "confirm": "http://{local_ip}/api/dvadmin_third/confirm/dingtalk_confirm_login/",

        # 获取用户token的地址（钉钉的）
        "token_api": "https://api.dingtalk.com/v1.0/oauth2/userAccessToken",

        # 获取用户信息的地址（钉钉的）
        "userinfo_api": "https://api.dingtalk.com/v1.0/contact/users/{unionId}/",
