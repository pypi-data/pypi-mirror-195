...
def z_projection(path_to_tiff:str, color:str):
    ...
    ret, projection = cv2.threshold(projection, (np.mean(projection) + np.std(projection)), np.max(projection), cv2.THRESH_BINARY)
    
    normalized = cv2.normalize(projection, None, 0, 150, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    ...
    projection = clahe.apply(normalized)
