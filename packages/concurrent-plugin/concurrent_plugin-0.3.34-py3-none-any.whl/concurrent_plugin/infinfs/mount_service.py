...
def fetch_upload_pod_status_logs(k8s_client:client.CoreV1Api, run_id, pod_name, pod_namespace):
    try:
        ...
        print(f"pod_info: {pod_info}", flush=True)
        print(f"pod_phase: {pod_phase}", flush=True)
        ...
        print(f"task_container_status={task_container_status}")
        container_statuses = task_container_status.container_statuses
        print(f"container_statuses={container_statuses}")
        task = container_statuses[task_index]
        print(f"task={task}")
        task_container_state = task.state
        print(f"task_container_state={task_container_state}")
        if 'running' in task_container_state and task_container_state['running']:
            print(f"Task container is in running state. Continuing to loop")
            return True
        elif 'terminated' in task_container_state and task_container_state['terminated']:
            print(f"Task container is in terminated state. Exiting loop")
            return True
        elif 'waiting' in task_container_state and task_container_state['waiting']:
