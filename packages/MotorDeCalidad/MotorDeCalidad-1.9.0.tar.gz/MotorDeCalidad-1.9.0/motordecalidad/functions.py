...
print("Motor de Calidad Version Release 1.9.0")
...
def startValidation(inputspark,config,dfltPath=""):
    ...
    writeDf(validationData, output,"resumen.csv")
...
def writeDf(object:DataFrame,output,name:str):
    ...
    if type == "prod_csv":
        ...
        try:
            if len(partitions) > Zero :
                object.coalesce(One).write.partitionBy(*partitions).mode("overwrite").option("partitionOverwriteMode", "dynamic").option("delimiter",str(output.get(JsonParts.Delimiter))).option("header",header).format("com.databricks.spark.csv").save(str(output.get(JsonParts.Path).format( 
                    country = dbutils.widgets.get('country'),
                    year = dbutils.widgets.get('year'),
                    month = dbutils.widgets.get('month')))+name)
            else:
        ...
        try:
            if len(partitions) > Zero :
                object.coalesce(One).write.partitionBy(*partitions).mode("overwrite").option("partitionOverwriteMode", "dynamic").option("delimiter",str(output.get(JsonParts.Delimiter))).option("header",header).format("com.databricks.spark.csv").save(str(output.get(JsonParts.Path))+name)
            else:
                object.coalesce(One).write.mode("overwrite").option("delimiter",str(output.get(JsonParts.Delimiter))).option("header",header).format("com.databricks.spark.csv").save(str(output.get(JsonParts.Path))+name)
        except:
            object.coalesce(One).write.mode("overwrite").option("delimiter",str(output.get(JsonParts.Delimiter))).option("header",header).format("com.databricks.spark.csv").save(str(output.get(JsonParts.Path))+name)
        ...
        if rules[code].get(JsonParts.Fields) not in [0,["0"],"0"] :
            ...
            if code[0:3] == Rules.Pre_Requisites.code:
                ...
                if column[0] == "*" :
                    ...
                    writeDf(res,rules[code].get(JsonParts.Output),"estadisticas.csv")
                else:
