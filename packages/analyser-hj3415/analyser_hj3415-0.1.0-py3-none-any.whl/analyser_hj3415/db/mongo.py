...
class MongoBase:
    ...
    def validate_db(client, db_name: str) -> bool:
        ...
        if db_name in client.list_database_names():
            ...
            raise Exception(f'Invalid database name : {db_name}')
    ...
    def validate_col(self) -> bool:
        """
        col_name 이 실제로 db_name 안에 있는지 확인한다.
        """
        if self.validate_db(self.client, self.db_name):
            if self.col_name in self.client[self.db_name].list_collection_names():
                return True
            else:
                raise Exception(f'Invalid collection name : {self.col_name}')
...
class Corps(MongoBase):
    ...
    def __init__(self, client, code: str, page: str):
        self.code = code
        self.page = page
        super().__init__(client=client, db_name=self.code, col_name=self.page)
    ...
    @staticmethod
    def drop_all_corps(client):
        corp_list = Corps.get_all_corps(client)
    ...
    def pick_rnd_x_code(client, count: int) -> list:
        ...
        """
        return random.sample(Corps.get_all_corps(client), count)
...
class DateBase(MongoBase):
    ...
    def __init__(self, client, db_name: str, date: str):
        self.date = date
        super().__init__(client=client, db_name=db_name, col_name=self.date)
...
class EvalByDate(DateBase):
    ...
    def __init__(self, client, date: str):
        super(EvalByDate, self).__init__(client, self.EVAL_DBNAME, date)
        # 인덱스 설정
