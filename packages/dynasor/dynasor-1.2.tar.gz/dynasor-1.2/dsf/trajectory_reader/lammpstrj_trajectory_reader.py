...
class lammpstrj_trajectory_reader(abstract_trajectory_reader):
        ...
        while True:
            ...
            if m.group(1) == 'TIMESTEP':
                ...
                if x.shape == (3, 3):
                    ...
                    raise IOError('TRJ_reader: Malformed box bounds')
    ...
    def _get_first(self):
        ...
        if self._x_map is None:
            self._x[:, II] = data[:, self._x_I].transpose()
        else:
            self._x[:, II] = self._x_map(data[:, self._x_I].transpose())
        if self._v_I is not None:
            self._v = zeros((3, N), order='F')
            self._v[:, II] = data[:, self._v_I].transpose()
