...
from .utils import load_dataset, data_preprocess
...
class TrainModel:
    ...
    def get_train_valid_dataset(self):
        ...
        valid_dataset = self.get_normalization(train_dataset,
                                               valid_dataset,
                                               action_tranform=True)
        # convert to tensor
        train_x, train_y = self.create_sequence(train_dataset,
                                                self.n_in,
                                                self.n_out)
        valid_x, valid_y = self.create_sequence(valid_dataset,
                                                self.n_in,
                                                self.n_out)
        (self.X['train'], self.X['valid']) = (torch.from_numpy(train_x).float(), 
                                              torch.from_numpy(valid_x).float())
        (self.y['train'], self.y['valid']) = (torch.from_numpy(train_y).float(), 
