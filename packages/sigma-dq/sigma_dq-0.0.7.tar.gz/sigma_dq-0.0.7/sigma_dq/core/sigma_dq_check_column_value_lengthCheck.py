...
def sigma_dq_check_column_value_lengthCheck(target_column,api_response,target_table, Execution_Type='',from_date=0,to_date=0,meta={}):
    column = target_column
    dq_rule = 'lengthCheck'
    api_response = api_response.split('#AND#')

    if Execution_Type == 'Incremental':  
        StrSQl = "select "+target_column+", CASE WHEN LENGTH("+target_column+") >= "+api_response[0]+" AND LENGTH("+target_column+") <= "+api_response[1]+" THEN 'PASS' ELSE 'FAIL' END as DQ_Status from " +target_table +" where UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from "+target_table+") "
        ...
        StrSQL = "select "+target_column+", CASE WHEN LENGTH("+target_column+") > "+api_response[0]+" AND LENGTH("+target_column+") < "+api_response[1]+" THEN 'PASS' ELSE 'FAIL' END as DQ_Status from " +target_table + between_condition
