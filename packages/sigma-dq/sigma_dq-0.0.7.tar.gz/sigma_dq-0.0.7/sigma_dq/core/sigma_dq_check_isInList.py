...
def sigma_dq_check_isInList(target_column,list_of_values,target_table,Execution_Type='',from_date=0,to_date=0,meta={}):  
  column = target_column
  dq_rule = 'isInList'
  
  if Execution_Type == 'Incremental':
    StrSQl = "select "+target_column+",case when "+target_column+"  in ('"+"','".join(list_of_values)+"') then 'PASS' else 'FAIL' end as DQ_Status from  "+target_table + ' WHERE UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from '+target_table+') '
    ...
    StrSQL = "select "+target_column+",case when "+target_column+" in ('"+"','".join(list_of_values)+"') then 'PASS' else 'FAIL' end as DQ_Status from "+target_table + between_condition
  else:
