...
# sigma_dq_check_NegativeNumber(target_column,target_table)
def sigma_dq_check_isNegativeNumber(target_column,target_table,Execution_Type='',from_date=0,to_date=0,meta={}):
  column = target_column
  dq_rule = 'isNegativeNumber'
  
  if(Execution_Type == 'Incremental'):
      StrSQl = "select "+target_column+", CASE WHEN " +target_column+" < 0 THEN 'PASS' ELSE 'FAIL' END as DQ_Status from " +target_table+" WHERE UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from "+target_table+") "
      ...
      StrSQL = "select "+target_column+", CASE WHEN " +target_column+" < 0 THEN 'PASS' ELSE 'FAIL' END as DQ_Status from " + target_table + between_condition
