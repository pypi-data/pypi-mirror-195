...
# sigma_dq_check_isEmail(target_column,api_response,target_table)
def sigma_dq_check_isEmail(target_column,target_table,Execution_Type='',from_date=0,to_date=0,meta={}):
  column = target_column
  dq_rule = 'isEmail'

  if Execution_Type == 'Incremental':
    StrSQl = "select "+target_column+", CASE WHEN " +target_column+ "  not LIKE '%_@__%.__%' THEN 'FAIL' ELSE 'PASS' END as DQ_Status from " +target_table+ " where UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from "+target_table+") "
    ...
    StrSQL = "select "+target_column+", CASE WHEN " +target_column+ "  not LIKE '%_@__%.__%' THEN 'FAIL' ELSE 'PASS' END as DQ_Status from " +target_table + between_condition
