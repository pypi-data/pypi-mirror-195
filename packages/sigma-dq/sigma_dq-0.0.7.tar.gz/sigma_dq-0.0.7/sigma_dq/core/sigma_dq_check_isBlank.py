...
def sigma_dq_check_isBlank(target_column,target_table,Execution_Type='',from_date=0,to_date=0,meta={}):
    dq_rule = 'isBlank'
    column = target_column

    if(Execution_Type == 'Incremental'):
        StrSQl = "select "+target_column+",case when "+target_column+" is not null and TRIM("+target_column+") <> ''  then 'PASS' else 'FAIL' end as DQ_Status from "+target_table + " WHERE UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from "+target_table+") "
        ...
        StrSQl = "select "+target_column+",case when "+target_column+" is not null and TRIM("+target_column+") <> '' then 'PASS' else 'FAIL' end as DQ_Status from "+target_table  + between_condition
    else: 
