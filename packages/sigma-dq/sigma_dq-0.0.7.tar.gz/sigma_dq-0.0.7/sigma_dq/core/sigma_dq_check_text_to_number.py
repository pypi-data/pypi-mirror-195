...
def sigma_dq_check_text_to_number(target_column,target_table,Execution_Type='',from_date=0,to_date=0,meta = {}):
  ...
  if Execution_Type == 'Incremental':
    StrSQl = f"select {target_column}, case when cast(replace(replace({target_column}, ',', ''), ' ', '') " \
             ...
             f"then 'PASS' else 'FAIL' end as DQ_Status from {target_table}  where UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from "+target_table+") "
    ...
    StrSQL = f"select {target_column}, case when cast(replace(replace({target_column}, ',', ''), ' ', '') " \
          f"as INT) is not null and cast(replace(replace({target_column}, ',', ''), ' ', '') as FLOAT)%1 == 0 " \
          f"then 'PASS' else 'FAIL' end as DQ_Status from {target_table}" + between_condition
