...
def sigma_dq_check_isSpace(target_column,target_table,Execution_Type='',from_date=0,to_date=0,meta={}):
  ...
  if(Execution_Type == 'Incremental'):
    StrSQl = "select "+target_column+", CASE WHEN length(" +target_column+") - length(REPLACE("+ target_column +" , ' ', '')) <= 1 then 'PASS' ELSE 'FAIL' END as DQ_Status from " +target_table+" where UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from "+target_table+") "  
    ...
    StrSQL = "select "+target_column+", CASE WHEN length(" +target_column+") - length(REPLACE("+ target_column +" , ' ', '')) <= 1 then 'PASS' ELSE 'FAIL' END as DQ_Status from " +target_table + between_condition
  else:
