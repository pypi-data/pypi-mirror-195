...
# sigma_dq_check_isTimestamp(target_column,api_response,target_table)
def sigma_dq_check_isTimestamp(target_column,target_table,Execution_Type='',from_date=0,to_date=0,meta={}):
    column = target_column
    dq_rule = 'isTimestamp'

    if Execution_Type == 'Incremental': 
      StrSQl = "select "+target_column+", CASE WHEN " +target_column+" = to_timestamp("+ target_column +") THEN 'PASS' ELSE 'FAIL' END as DQ_Status from "                +target_table + " where UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from "+target_table+") "
      ...
      StrSQL = "select "+target_column+", CASE WHEN " +target_column+" = to_timestamp("+ target_column +") THEN 'PASS' ELSE 'FAIL' END as DQ_Status from " +target_table + between_condition
