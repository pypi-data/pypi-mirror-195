...
# sigma_dq_check_isNumber(target_column,target_table)
def sigma_dq_check_isNumber(target_column,target_table,Execution_Type='',from_date=0,to_date=0,meta={}):
    column = target_column
    dq_rule = 'isNumber'

    if(Execution_Type == 'Incremental'):
          StrSQl = "SELECT " + target_column + ", CASE WHEN " + target_column + " REGEXP '^[0-9]+$' " + \
          "THEN 'PASS' ELSE 'FAIL' END AS DQ_Status FROM " + target_table + " where UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from edap_transform.purchase_order) OR dqAction = 'NA' "
