...
# check_upper_case(target_column,target_table)
def sigma_dq_check_isUpperCase(target_column,target_table,Execution_Type='',from_date=0,to_date=0,meta={}):
  column = target_column
  dq_rule = 'isUpperCase'
  
  if(Execution_Type == 'Incremental'):
    StrSQl = "SELECT " + target_column + ", CASE WHEN " + target_column + " = upper(" + target_column + ") " \
           "THEN 'PASS' ELSE 'FAIL' END AS DQ_Status FROM " + target_table + " where UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from "+target_table+") "
