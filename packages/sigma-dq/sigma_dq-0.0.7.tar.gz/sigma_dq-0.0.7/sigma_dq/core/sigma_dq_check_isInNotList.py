...
def sigma_dq_check_isInNotList(target_column,list_of_values,Execution_Type='',from_date=0,to_date=0,meta={}):
  column = target_column
  dq_rule = 'isInNotList'
  
  if Execution_Type == 'Incremental':
    StrSQl = "select "+target_column+",case when "+target_column+" not in ('"+"','".join(list_of_values)+"') then 'PASS' else 'FAIL' end as DQ_Status from  "+target_table + ' WHERE UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from '+target_table+')  '
    ...
    StrSQL = "select "+target_column+",case when "+target_column+" not in ('"+"','".join(list_of_values)+"') then 'PASS' else 'FAIL' end as DQ_Status from  "+target_table  + between_condition
