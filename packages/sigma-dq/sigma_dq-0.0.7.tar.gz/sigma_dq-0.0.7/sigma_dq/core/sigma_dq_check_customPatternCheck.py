...
def sigma_dq_check_customPatternCheck(target_column,api_response,Execution_Type='',from_date=0,to_date=0,meta={}):
  column = target_column
  dq_rule = 'sigma_dq_check_expect_column_values_to_match_regex'

  if(Execution_Type == 'Incremental'):
    StrSQl = "select "+target_column+", CASE WHEN " +target_column+" REGEXP '"+api_response+"' THEN 'PASS' ELSE 'FAIL' END as DQ_Status from " +target_table+" where UPDATE_RUN_TS = (select MAX(UPDATE_RUN_TS) from "+target_table+") "
    ...
    StrSQL = "select "+target_column+", CASE WHEN " +target_column+" REGEXP '"+api_response+"' THEN 'PASS' ELSE 'FAIL' END as DQ_Status from " +target_table + between_condition
