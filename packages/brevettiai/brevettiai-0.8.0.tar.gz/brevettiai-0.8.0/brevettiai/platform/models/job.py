...
class Job(BaseModel):
    ...
    def prepare_start(self, resolve_access_rights: bool = True, cache_remote_files: bool = True, set_credentials: bool = True):
        ...
        self._credentials.set_credentials(type="JobCredentials",
                                          user=self.id, secret=self.api_key, platform=self.backend)
        #if cache_remote_files:
        #    self.io.set_cache_root(self.temp_path)

        if set_credentials:
            self.io.minio.credentials = self._credentials
