...
class TopicCommitter(IPFSPubSub, StorageBase, HasherBase, IPFSClient):
    """
        ...
        [#] Sync with a Topic Listener via Listener-swarm
    """
    ...
    def __handle_subscription_topic_updates_from_listener(self, data, seqno, topic_ids, cid, subscription):
        ...
        listener = data["caller"]
        # get the status and rff from the listener
        status = data["status"]
        rff = data["rff"]
        # get the swarm key hash from the listener
        swarm_key_hash = data["swarm_key"]
        # TODO: check if the listener is a valid listener for the topic
        # TODO: check if status and rff are valid
    ...
    def listen_to_topic_updates(self, topic_id:int=None, interval:int=3600) -> str:
        assert topic_id is not None
