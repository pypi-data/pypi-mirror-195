...
def peak_picking(df1, threshold=15, i_threshold=500, SN_threshold=3,
                 ...
                 mz_error_alignment=0.015, enable_progress_bar=True):
    """
...
def split_peak_picking(ms1, profile=True, split_n=20, threshold=15, i_threshold=500,
    ...
    if profile is True:
        ...
        t2 = time.time()
        print(f'\r Optimized time: {round(t2 - t1, 0)} s     ')
    else:
...
def first_process(file, company, profile=True, i_threshold=200, SN_threshold=3,
    ...
    if profile is True:
        if company.lower() == 'waters':
            df1, raw_info = gen_df(ms1, mz_round, profile=profile, raw_info=True)
        ...
        if company.lower() == 'waters':
            df1 = gen_df(ms1, mz_round, profile=profile, raw_info=False)
    ...
    if len(ms2) == 0:
        ...
        if ms2_analysis is True:
            if ('control' in file.lower()) | ('blank' in file.lower()) | ('methanol' in file.lower()) | (
                ...
                if profile is True:
                    if company.lower() == 'waters':
                        df2 = gen_df(ms2, mz_round, profile=profile, raw_info=False)  # 不分析二级碎片
                        peak_all2 = peak_picking(df2, isotope_analysis=False)
                    else:
                        peak_all2 = split_peak_picking(ms2, profile=profile, i_threshold=i_threshold)
                else:
                    if company.lower() == 'waters':
                        df2 = gen_df(ms2, mz_round, profile=profile, raw_info=False)
                        peak_all2 = peak_picking(df2, isotope_analysis=False)
                    else:
                        peak_all2 = split_peak_picking(ms2, profile=profile, i_threshold=i_threshold)
...
def second_process(file, ref_all, company, profile=True):
    ...
    ms1, ms2 = sep_scans(file, company)
    if company.lower() == 'waters':
        df1 = gen_df(ms1, ms_round=4, profile=profile, raw_info=False)
