...
class Workload:
    """Class representing a workload

    This class contains all information about an execution of a program including logs, run numbers and input/output files.

    Attributes:
        _path (str): The file path of the workload.
        _program (Program): Program object associated with this workload.
        _workload_uuid (str): Unique identifier for the workload.
        _module_uuid (str): Unique identifier for the module.
        _nb_config (dict): Dictionary containing configuration related to this workload.
        _run_num (int): Numerical value corresponding to which run of the given program this workload represents.
        _initializing_lines (list[tuple]): List of tuples containing data related to initializing stages of the workload.
        _preparing_lines (list[tuple]): List of tuples containing data related to preparing stages of the workload.
        _running_lines (list[tuple]): List of tuples containing data related to running stages of the workload.
        _input_files (CloudStorage): Cloud storage instance containing the input files associated with the workload.
        _update_lines (list[tuple]): List of tuples containing update related data for the workload.
        _output (list): Output lines from the workload.
        _monitoring (list): List of monitoring data related to the workload.
        _last_patches (int): Last patch number seen by the workload.
        _output_files (CloudStorage): Cloud storage instance containing output files associated with the workload.
        _machine (Machine): Machine object associated with this workload.
        _token (str): Authentication token used to access session.
        _last_refresh (float): Time since last refresh.
    """
    ...
    def __init__(self, path, program, workload_uuid, module_uuid, config, run_num):
        """This function initializes the Workload object.
    ...
    def wait(self, progress="summary"):
        """This function waits for a workload to complete by continuously checking the status.

        Args:
            progress (str, optional): Defines the level of verbosity in reporting the workload status. Can be "summary", "detail" or "silent". Defaults to "summary".
        """
    ...
    def stop(self, wait=True):
        """Stops the current workload.

        Args:
            wait (bool, optional): If set to True, the function will wait till the workload is terminated before returning. Defaults to True.
        """
        from .Workloads import Workloads

        if (
            optumi.utils.is_dynamic()
    ...
    @property
    def status(self):
        """Returns the current status of the workload: either launching, running, or completed.

        Returns:
            str: The current status of the workload: either launching, running, or completed.
        """
        self._refresh()
    ...
    def detailed_status(self):
        """Returns the current detailed status of a workload.

        Returns:
            str: the current detailed status of a workload.
        """
        self._refresh()
    ...
    def input_files(self):
        """Gets the input files for the given workload.

        Returns:
            CloudStorage: A list of input files.
        """
        self._refresh()
    ...
    def output_files(self):
        """Gets the output files for the given workload.

        Returns:
            CloudStorage: A list of output files.
        """
        self._refresh()
        if self._program == None:
            for workload in json.loads(
    ...
    def name(self):
        ...
        Returns:
            str: the name of the workload
        """
        return self._path.split("/")[-1] + " (Run #" + str(self._run_num) + ")"

    @property
    def path(self):
        """Returns the path of the workload

        Returns:
            str: the path of the workload
        """
        return optumi.utils.normalize_path(self._path, strict=False)

    @property
    def machine(self):
        """Returns the machine this workload ran/is running on

        Returns:
            Machine: the machine this workload ran/is running on
        """
