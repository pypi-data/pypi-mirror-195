...
def install_modules(modules=[], reinstall=False, options=[], verbose=False):
    ...
    if available:
        ...
        for module in modules:
            if "=" in module:
            ...
            elif module in to_install or module == "ampl":
                continue
                ...
                for entry in bundles:
                    bundle, includes = entry["module"], entry["includes"]
                    if module in includes:
                        raise Exception(
                            f"AMPL module '{module}' "
                            f"is not available but it is "
                            f"included in module '{bundle}'."
                        )
                else:
                    raise Exception(f"AMPL module '{module}' is not available.")

    modules = _normalize_modules(modules=to_install, add_base=True)
