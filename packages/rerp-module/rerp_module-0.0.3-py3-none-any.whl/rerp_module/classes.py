...
conditions = [240, 241, 242, 243, 244, 245, 246, 247]
...
class subject:
    ...
    def load_unique_codes(self, uc1_xlsx='Unique_Codes_List1.xlsx', uc2_xlsx='Unique_Codes_List2.xlsx'):
        if self.list_num == 1: return pd.read_excel(uc1_xlsx).to_numpy()
        elif self.list_num == 2: return pd.read_excel(uc2_xlsx).to_numpy()
        else: return None
        
        
    def load_predictors(self, pred1_xlsx='List1_Full.xlsx', pred2_xlsx='List2_Full.xlsx', drop_col=[]):
        if self.list_num == 1:
    ...
    def check_elist(self, condition_ecodes=conditions):   
        unique_codes = self.load_unique_codes()
        prefinal_codes = []
    ...
    def drop_marked_epochs(self, condition_ecodes=conditions, col=None):
        predictors = self.load_predictors(drop_col=col)
        ep_all = []
    ...
    def sort_predictors(self, condition_ecodes=conditions, col=[]):
        unique_codes = self.load_unique_codes()
        predictors = self.load_predictors(drop_col=col)
        ep_all = []
        for i in self.split_elist():
        ...
        dm = np.zeros((0, np.shape(predictors)[1]))
        prefinal_codes = self.check_elist()
        index = None
        for i in range(len(unique_codes)):
    ...
    def AD_compiler(self, condition_ecodes=conditions):          
...
class iteration(subject):
    def __init__(self, list_num, eeglab_elist, mne_raw, col_todrop=[]):
        super().__init__(list_num, eeglab_elist, mne_raw)
        self.col_todrop = col_todrop
        self.unique_codes = self.load_unique_codes()
        self.predictors = self.load_predictors(drop_col=col_todrop)
        self.chan_dict = self.create_chan_dict()
        
        
    def get_rerp_outputs(self):
        ecodes = self.drop_marked_epochs(col=self.col_todrop).Epoch_Ecodes
        ...
        self.mne_raw.set_annotations(self.AD_compiler())
        all_epoched_bc = mne.Epochs(self.mne_raw, ev, event_id=all_ev, tmin=-0.1, tmax=1.0, baseline=(-0.1,0.0),
                                    reject_by_annotation=True)
        
        if self.check_elist() == None:
            X = self.drop_marked_epochs(col=self.col_todrop).Dmatrix
            
        else: 
            print('\nError: Unique codes do not match !!! Sorting...\n')
            X = self.sort_predictors(col=self.col_todrop)
            
        all_trials = rERP(all_epoched_bc, ecodes, [240, 241, 242, 243, 244, 245, 246, 247])
...
class grand:
    def __init__(self, subj_outputs):
        self.outputs = subj_outputs
        
    def compute_grand_averages(self):
        num_bins = np.shape(subj_outputs[0].Regr_Res.Bins.Coeffs['coeffs_arr'])[0]
        num_pred = np.shape(subj_outputs[0].Regr_Res.rERP.Coeffs['coeffs_arr'])[0]
        num_ch = np.shape(subj_outputs[0].Regr_Res.rERP.Coeffs['coeffs_arr'])[1]
        num_t = np.shape(subj_outputs[0].Regr_Res.rERP.Coeffs['coeffs_arr'])[2]
        ...
        for i in range(np.shape(subj_outputs)[0]):
            rerp_coeffs = subj_outputs[i].Regr_Res.rERP.Coeffs['coeffs_arr']
            rerp_cond_means = np.array([subj_outputs[i].Cond_Means.rERP.Mean_Fitted,
                                        subj_outputs[i].Cond_Means.rERP.Mean_Resids], dtype=object)
            
            gnd_avgs = np.add(gnd_avgs, np.array([subj_outputs[i].Regr_Res.Bins.Coeffs['coeffs_arr'],
                                                  rerp_coeffs, rerp_cond_means],
                              dtype=object), dtype=object)
            
        gnd_avgs = np.multiply(gnd_avgs, 1/np.shape(subj_outputs)[0])
...
class eeg_graphs:
    def __init__(self, timeline, grand_averages=None, single_subj_rerp_outputs=None):
    ...
    def plot_grand_average_rERP_effects(self, ch_names_arr, fig_size,
        ...
        if np.shape(ch_names_arr) == (1,1):
            ...
            for i in range(num_coeffs-1):
                axis.plot(self.timeline, muV(self.grand_averages.rERP_Effects[i+1][chan_dict[ch]]),
            ...
            for ax, ch in zip(axis.copy().flatten(), ch_names_arr.copy().flatten()):
                for i in range(num_coeffs-1):
                    ax.plot(self.timeline, muV(self.grand_averages.rERP_Effects[i+1][chan_dict[ch]]),
                            color=effect_colors[i+1], label=labels[i+1])
    ...
    def plot_grand_average_ERPs(self, conditions, ch_names_arr, fig_size,
        ...
        if np.shape(ch_names_arr) == (1,1):
            ...
            for i in range(len(conditions)):
                axis.plot(self.timeline, muV(self.grand_averages.ERP_Bins[cond_dict[conditions[i]]][chan_dict[ch]]),
            ...
            for ax, ch in zip(axis.copy().flatten(), ch_names_arr.copy().flatten()):
                for i in range(len(conditions)):
                    ax.plot(self.timeline, muV(self.grand_averages.ERP_Bins[cond_dict[conditions[i]]][chan_dict[ch]]),
                            color=bins_colors[i], label=conditions[i])
                ax.axvline(x=0, color='black', linewidth=0.5)
                ax.axhline(y=0, color='black', linewidth=0.5)
                ax.set_title(ch)
    ...
    def plot_grand_average_rERPs(self, conditions, ch_names_arr, fig_size,
        ...
        if np.shape(ch_names_arr) == (1,1):
            ...
            figure, axis = plt.subplots(figsize=fig_size)
            for i in range(len(conditions)):
                axis.plot(self.timeline, muV(self.grand_averages.rERP_Estimates[cond_dict[conditions[i]]][chan_dict[ch]]),
            ...
            for ax, ch in zip(axis.copy().flatten(), ch_names_arr.copy().flatten()):
                for i in range(len(conditions)):
                    ax.plot(self.timeline, muV(self.grand_averages.rERP_Estimates[cond_dict[conditions[i]]][chan_dict[ch]]),
                            color=conditions_colors[i], label=conditions[i])
                ax.axvline(x=0, color='black', linewidth=0.5)
                ax.axhline(y=0, color='black', linewidth=0.5)
                ax.set_title(ch)
