...
class AnomalyDetection(beam.PTransform):
    ...
    def encode_and_normalize(self, bq_row, num_fields=None, id_field='Id'):
        logging.info('Step 1: Normalize')
        logging.info('Step 1: Input type expected: {}')
        logging.info(f'Step 1: Input type: {type(bq_row)} has {len(bq_row._fields)} fields')

        logging.info(f'Step 1: Input fields: {bq_row._fields}')
        ...
        logging.info(f'Step 1: Input data encoded.')
        ...
        logging.info(f'Step 1: Continuous columns shape: {cont_cols.shape}. Expected: ')
        logging.info(f'Step 1: Categorical columns shape: {cont_cols.shape}. Expected: ')

        result = (getattr(bq_row, id_field),
                  {'cont_cols': cont_cols,
                   'cat_cols': cat_cols})

        logging.info(f'Step 1: Output type {type(result)} with length {len}')
        return result
