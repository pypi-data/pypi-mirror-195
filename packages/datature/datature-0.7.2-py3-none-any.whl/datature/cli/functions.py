...
def list_projects():
    ...
    for project_name in project_names:
        ...
        try:
            ...
            if float(asset_annotated) == 0 or float(asset_total) == 0:
                percentage = '0.0%'
            else:
                percentage = f"{round(100 * float(asset_annotated) / float(asset_total),2)}%"

            output.append([project_name, asset_annotated, percentage])
        except error.ForbiddenError:
            print((
...
def find_all_assets(path: Path) -> [str]:
    ...
    for root, _, _ in os.walk(path):
        file_paths.extend(glob.glob(os.path.join(root, '*.jpg')))
        file_paths.extend(glob.glob(os.path.join(root, '*.png')))
        file_paths.extend(glob.glob(os.path.join(root, '*.jpeg')))
        file_paths.extend(glob.glob(os.path.join(root, '*.mp4')))
...
def upload_assets(path: Optional[Path] = None, groups: Optional[str] = None):
    ...
    if not groups:
        groups_res = inquirer.text(messages.ASSETS_GROUPS_MESSAGE,
                                   default="main")
        groups = [
...
def print_table(data: [[str]], column_width: int = 12):
    """
...
def assets_group(group: Optional[str] = None):
    ...
    if not group:
        ...
        table = [[
            "NAME", "ASSETS", "ANNOTATED", "REVIEW", "TOFIX", "COMPLETED"
        ]]
        table.append([
