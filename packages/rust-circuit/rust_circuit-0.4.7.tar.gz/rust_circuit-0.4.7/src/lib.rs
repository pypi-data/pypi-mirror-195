...
fn _rust(py: Python<'_>, m: &PyModule) -> PyResult<()> {
    ...
    m.add_class::<rr_util::shape::PySize>()?;

    m.add_function(wrap_pyfunction!(rr_util::shape::broadcast_shapes_py, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(rr_util::shape::symbolic_sizes, m)?)?;
