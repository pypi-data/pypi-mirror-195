...
impl ModuleSpec {
    ...
    pub fn check_expand_valid_shapes(
        ...
        for (arg_spec, node) in izip!(self.arg_specs.iter(), nodes) {
            ...
            if !batchable && node.info().rank() > arg_spec.symbol.info().rank() {
    ...
    pub fn check_no_ident_matching_issues(&self) -> Result<()> {
        ...
        let syms = sym_single_map
            .as_ref()
            .unwrap_or_else(|| self.circuit.info().get_raw_free_symbols());
        ...
        fn recurse(circ: CircuitRc, symbols: &SymbolSetRc) -> Result<Option<CircuitRc>> {
            ...
            let new = if let Some(m) = circ.as_module() {
                let mut removed: BTreeSet<_> = symbols
                    .iter()
                    .filter(|x| m.info().get_raw_free_symbols().contains(&x.crc()))
                    .cloned()
                    .collect();
...
pub enum FreeSymbolsWrapper<'a> {
    Sym(IndexSet<CircuitRc>),
    Other(&'a IndexSet<CircuitRc>),
}

impl<'a> Deref for FreeSymbolsWrapper<'a> {
    type Target = IndexSet<CircuitRc>;
    fn deref(&self) -> &Self::Target {
        match self {
            Self::Sym(x) => x,
            Self::Other(x) => x,
...
pub fn get_free_symbols(circuit: &CircuitRc) -> FreeSymbolsWrapper<'_> {
    ...
    if circuit.is_symbol() {
        FreeSymbolsWrapper::Sym([circuit.clone()].into_iter().collect())
    } else {
...
pub fn has_free_sym(circuit: &CircuitRc, sym: &Symbol) -> bool {
    if let Some(other) = circuit.as_symbol() {
        ...
        circuit.info().get_raw_free_symbols().contains(&sym.crc())
    }
}
