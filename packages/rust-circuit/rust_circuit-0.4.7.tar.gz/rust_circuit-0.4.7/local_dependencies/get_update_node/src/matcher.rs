...
impl From<MatcherFromPyBase> for Matcher {
    fn from(m: MatcherFromPyBase) -> Self {
        match m {
            ...
            MatcherFromPyBase::Name(x) => {
                MatcherData::Name(x.into_many().into_iter().map(NameOrdById).collect())
            }
...
impl MatcherData {
    ...
    fn item_hash(&self, hasher: &mut blake3::Hasher) {
        match self {
            ...
            Self::Name(x) => {
                for s in x {
                    hasher.update(s.0.str().as_bytes());
        ...
        match &self.data {
            MatcherData::Name(names) => {
                let found_names = matched
                    .into_iter()
                    .filter_map(|x| x.info().name.map(NameOrdById))
                    .collect();
    ...
    pub fn call(&self, circuit: CircuitRc) -> Result<bool> {
        let ret = match &self.data {
            ...
            MatcherData::Name(names) => circuit
                .info()
                .name
                .map(|x| names.contains(&NameOrdById(x)))
                .unwrap_or(false),
