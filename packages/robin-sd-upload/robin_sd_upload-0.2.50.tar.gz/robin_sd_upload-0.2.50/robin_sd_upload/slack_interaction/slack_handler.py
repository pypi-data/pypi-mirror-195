...
def zip_files(log_dir):
    ...
    try:
        ...
        logger.log(message=f"Error zipping files: {e}", to_terminal=True, log_level='error')
...
def send_slack(file_to_send, customer_name: str = None):
    ...
    try:
        ...
        logger.log(message=f"Slack error: {e.response['error']}", to_terminal=True, log_level='error')
