...
def write_dump_traj(h5_path='aimd.h5', jpath=None, ai=None, ele=None, index=None, dumpfile='aimdTraj.dump'):
    ...
    with open(dumpfile, 'w') as f:
        for n in range(Nstep):
            box_bounds = get_lammps_non_orthogonal_box(lats[n])
            f.write('ITEM: TIMESTEP\n%d\n' % n)
            f.write('ITEM: NUMBER OF ATOMS\n%d\n' % (len(eles)))
            f.write('ITEM: BOX BOUNDS xy xz yz xx yy zz\n')
            f.write('%f %f %f\n%f %f %f\n %f %f %f\n' % (box_bounds[0][0], box_bounds[0][1], box_bounds[0][2], box_bounds[1][0],  box_bounds[1][1],  box_bounds[1][2], box_bounds[2][0], box_bounds[2][1], box_bounds[2][2]))
...
def get_lammps_non_orthogonal_box(lat:np.ndarray):
    """计算用于输入lammps的盒子边界参数
    ...
    zlo_bound = zlo
    zhi_bound = zhi
    box_bounds = np.array([[xlo_bound, xhi_bound, xy],[ylo_bound, yhi_bound, xz],[zlo_bound, zhi_bound, yz]])
