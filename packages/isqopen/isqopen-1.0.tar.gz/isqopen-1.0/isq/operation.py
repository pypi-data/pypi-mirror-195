...
from .globalVar import get_jax_env, jax_support
...
def X():
    if np_mod: return jnp.array([[0,1], [1,0]], dtype=complex)
    return anp.array([[0,1], [1,0]], dtype=complex)
def Y():
    if np_mod: return jnp.array([[0,-1j], [1j,0]], dtype=complex)
    return anp.array([[0,-1j], [1j,0]], dtype=complex)
def Z():
    if np_mod: return jnp.array([[1,0], [0,-1]], dtype=complex)
    return anp.array([[1,0], [0,-1]], dtype=complex)
def H():
    if np_mod: return 1/jnp.sqrt(2) * jnp.array([[1,1], [1,-1]], dtype=complex)
    return 1/anp.sqrt(2) * anp.array([[1,1], [1,-1]], dtype=complex)
def T():
    if np_mod: return jnp.array([[jnp.exp(-1j*jnp.pi / 8),0], [0,jnp.exp(1j*jnp.pi / 8)]], dtype=complex)
    return anp.array([[anp.exp(-1j*anp.pi / 8),0], [0,anp.exp(1j*anp.pi / 8)]], dtype=complex)
def S():
    if np_mod: return jnp.array([[1,0], [0,1j]], dtype=complex)
...
def RX(theta):
    theta /= 2
    if np_mod: return jnp.array([[jnp.cos(theta),-1j*jnp.sin(theta)],[-1j*jnp.sin(theta),jnp.cos(theta)]], dtype=complex)
    return anp.array([[anp.cos(theta),-1j*anp.sin(theta)],[-1j*anp.sin(theta),anp.cos(theta)]], dtype=complex)
def RY(theta):
    theta /= 2
    if np_mod: return jnp.array([[jnp.cos(theta),-1*jnp.sin(theta)],[jnp.sin(theta),jnp.cos(theta)]], dtype=complex)
    return anp.array([[anp.cos(theta),-1*anp.sin(theta)],[anp.sin(theta),anp.cos(theta)]], dtype=complex)
def RZ(theta):
    theta /= 2
    if np_mod: return jnp.array([[jnp.exp(-1j*theta),0],[0,jnp.exp(1j*theta)]], dtype=complex)
    return anp.array([[anp.exp(-1j*theta),0],[0,anp.exp(1j*theta)]], dtype=complex)
def X2P():
    if np_mod: return jnp.sqrt(2)/2 * jnp.array([[1, -1j], [-1j, 1]])
    return anp.sqrt(2)/2 * anp.array([[1, -1j], [-1j, 1]])
def X2M():
    if np_mod: return jnp.sqrt(2)/2 * jnp.array([[1, 1j], [1j, 1]])
    return anp.sqrt(2)/2 * anp.array([[1, 1j], [1j, 1]])
def Y2P():
    if np_mod: return jnp.sqrt(2)/2 * jnp.array([[1, -1], [1, 1]])
    return anp.sqrt(2)/2 * anp.array([[1, -1], [1, 1]])
def Y2M():
    if np_mod: return jnp.sqrt(2)/2 * jnp.array([[1, 1], [-1, 1]])
    return anp.sqrt(2)/2 * anp.array([[1, 1], [-1, 1]])
...
def single_gate(state, gate, qnum, target):
    ...
    if np_mod: return jnp.ravel(state)
