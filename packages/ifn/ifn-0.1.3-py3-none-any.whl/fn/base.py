...
from .type import Ta, Tb, Keys, NestedDict, NestedItem, NestedKeys, NestedValue, OptStr, Unknown
...
class Data(t.Generic[Ta]):
    ...
    def get(self, *keys: str, default: Unknown = None) -> t.Union[NestedValue, Unknown]:
        try:
...
class Base(t.Generic[Ta]):
    ...
    __key__: str = 'default'
    __sep__: str = '.'
    __def__: t.Optional[Ta] = None

    def __init__(self, value: t.Optional[Ta] = None) -> None:
        self._value = self.__def__ if value is None else value
    ...
    def new(cls, key: str, default: t.Optional[Ta] = None, sep: OptStr = None) -> 'Self':
        name = ''.join(map(str.capitalize, cls._keys(key)))
        return type(name, (cls, ), {
            '__key__': key, '__def__': default,
            '__sep__': sep or cls.__sep__,
        })
...
class Protocol:
    ...
    def __init__(self, **kwargs: Unknown) -> None:
        for key, value in kwargs.items():
            setattr(self, key, value)

    def setup(self, group: 'Group') -> 'Self':
        self._group = group
        return self

    @property
    def group(self) -> 'Group':
        assert getattr(self, '_group', None) is not None
