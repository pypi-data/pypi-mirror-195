...
def test_import_nodes():
    data = parse('''
$source nodes = tests/blocks/nodes.dip
...
def test_source_inject():
    data = parse('''
file str = tests/blocks/nodes.dip
$source nodes = {?file}
{nodes?fruits}              
    ''')
    np.testing.assert_equal(data,{
        'file':   StringType('tests/blocks/nodes.dip'),
        'fruits': IntegerType(0),
    })

def test_source_import():
    data = parse('''
$source file = tests/blocks/nodes.dip
$source {file?blocks}
{blocks?energy}              
    ''')
    np.testing.assert_equal(data,{
...
$source {file?books}
    ...
    with pytest.raises(Exception) as e_info:
        parse('''
$source blocks = tests/blocks/nodes.dip
...
def test_injection_remote():
    data = parse('''
$source query = tests/blocks/query.dip
...
def test_injection_array():
    data = parse('''
$source matrix = tests/blocks/matrix.txt
$source query = tests/blocks/query.dip
...
def test_injection_table():
    data = parse('''
$source table = tests/blocks/table.txt  
$source query = tests/blocks/query.dip
...
def test_injection_text():
    data = parse('''
$source text = tests/blocks/text.txt
$source query = tests/blocks/query.dip
