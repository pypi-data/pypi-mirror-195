...
from owl_model_extractor.config import constants_metadata
...
class OwlModel:
    ...
    def get_license(self):
        for license in constants_metadata.LICENSE:
            for metadata in self.__ontology.metadata:
                if isinstance(metadata, str):
                    if license.__eq__(metadata):
                        license_list = list(
                            default_world.sparql(constants_metadata.SPARQL_STR.replace("##PH1##", license)))
    ...
    def get_preferred_namespace_prefix(self):
        annotation_list = []
        for namespace_prefix in constants_metadata.PREFERRED_PREFIX:
            for metadata in self.__ontology.metadata:
                if isinstance(metadata, str):
                    if namespace_prefix.__eq__(metadata):
                        preferredPrefix = list(default_world.sparql(constants_metadata.SPARQL_STR.replace("##PH1##", namespace_prefix)))
    ...
    def get_preferred_namespace_uri(self):
        for namespace_uri in constants_metadata.PREFERRED_PREFIX:
            for metadata in self.__ontology.metadata:
                if isinstance(metadata, str):
                    if namespace_uri.__eq__(metadata):
                        namespace_uri_list = list(
                            default_world.sparql(constants_metadata.SPARQL_STR.replace("##PH1##", namespace_uri)))
    ...
    def get_version_iri(self):
        version_iri = default_world._get_obj_triple_sp_o(self.__ontology.storid,
                                           default_world._abbreviate("http://www.w3.org/2002/07/owl#versionIRI"))
