...
class CostExplorerPayerBillings(CostBillingReports):
            ...
            for results_by_time in cost_data.get('ResultsByTime'):
                ...
                for group in results_by_time.get('Groups'):
                    ...
                    if name and amount:
                        if name not in data:
                            if cost_center:
                                acc_cost_center, account_budget, years, owner = self.update_to_gsheet.get_cost_center_budget_details(account_id=name, dir_path='/tmp')
    ...
    def filter_forecast_data(self, cost_forecast_data: list, cost_usage_data: dict, account_id: str, cost_center: int, account: str):
        acc_cost_center, account_budget, years, owner = self.update_to_gsheet.get_cost_center_budget_details(account_id=account_id, dir_path='/tmp')
