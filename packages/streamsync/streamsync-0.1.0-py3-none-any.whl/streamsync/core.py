...
class StateProxy:
    ...
    """
    The root state and its children (nested states) are instances of this class.
    ...
    def mutations_as_dict(self):
        ...
        for key, value in self.state.items():
            ...
            if isinstance(value, StateProxy):
                child_mutations = value.mutations_as_dict()
...
class StreamsyncState():
    ...
    def change_page(self, active_page_key: str):
...
class EventDeserialiser:
    ...
    def transform(self, ev):
        ...
        # Ignore non-custom events, they don't need to be deserialised

        if not ev["type"].startswith("ss-"):
            return
        ...
        for c in descendents:
            ...
            if c.type == "fileinput":
                tf_payload[key] = list(map(
                    lambda dataurl: urllib.request.urlopen(dataurl), payload[key]))
                continue
...
class StateSerialiser:
    ...
    def serialise(self, v: Any):
        ...
        if hasattr(v, "to_dict") and callable(v.to_dict):
            ...
            # Covers StateProxy, Pandas dataframes, Altair charts.
