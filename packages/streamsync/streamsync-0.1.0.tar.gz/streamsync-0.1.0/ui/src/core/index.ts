...
import { Component, InstancePath, MailItem } from "../streamsyncTypes";
import {
...
export function generateCore() {
	...
	const renderedComponents: Record<Component["id"], ComponentPublicInstance> =
		{};
	...
	function forwardEvent(event: Event, instancePath: InstancePath) {
		...
		if (event instanceof CustomEvent) {
			eventPayload = event.detail?.payload ?? {};
	...
	function sendComponentUpdate() {
		...
		sendFrontendMessage("componentUpdate", payload, (r) => {
			if (r.ok == true) {
				return;
			}
			console.error("Components couldn't be updated.");
		});
	}
	...
	function setActivePageFromKey(targetPageKey: string) {
		...
		activePageId.value = matches[0].id;
	}
	...
	function getContainableTypes(componentId: Component["id"]) {
		return typeHierarchy.getContainableTypes(components, componentId);
	}
