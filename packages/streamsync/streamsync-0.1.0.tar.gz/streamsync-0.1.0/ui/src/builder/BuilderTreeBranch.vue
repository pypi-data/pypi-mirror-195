<template>
	<div class="BuilderTreeBranch">
		...
		>
			...
			<span class="type">{{ name }} {{ component.position }}</span>
...
const name = computed(() => {
	const { name } = componentDefinition.value;
...
const handleDragStart = (ev: DragEvent) => {
	...
	ev.dataTransfer.setData(
		`application/json;${component.value.type},${component.value.id}`,
		"{}"
	);
};
...
const handleDragOver = (ev: DragEvent) => {
	const { draggedType: childType } = getComponentInfoFromDrag(ev);
