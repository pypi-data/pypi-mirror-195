...
class UpsertData:
    """
        ...
        Attributes:
            ...
            timezone: the Timezone of the rows. Must e the OriginalTimezone or, when Hourly, must be ""UTC""
            downloadedAt: the UTC timestamp at which this assessment has been acquired/generated
            version: the Version to operate on
            rows: the timeserie data in OriginalTimezone or, when Hourly, must be ""UTC""
            marketAssessment: The Market Data Identifier to upsert. 
                                (LocalDateTime key is The Report timestamp in the MarketData OriginalTimezone but when Hourly must be UTC)
            bidAsk: the Bid Ask
            auctionRows: the timeserie data in OriginalTimezone or, when Hourly, must be UTC.
            deferCommandExecution: flag to choose between synchronous and asynchronous command execution
            deferDataGeneration: flag to choose between synchronous and asynchronous precomputed data generation
            keepNulls: flag to choose between syncronoys and asyncronous precomputed data generation
    """
