        ...
        while "MORPHEUS PING" not in ping:
            while timeout < 90:
                try:
                    ...
                    if "Could not reach node:" in ping:
                        logger.info(f'Unable to reach the node. Assuming it is still initializing. Waiting 30 seconds to check again.')
                        time.sleep(30)
                        timeout = timeout + 1
                    if "404" in str(ping):
                        logger.info(f'Encountered a 404. Assuming it is still initializing. Waiting 30 seconds to check again.')
                        time.sleep(30)
                        timeout = timeout + 1
