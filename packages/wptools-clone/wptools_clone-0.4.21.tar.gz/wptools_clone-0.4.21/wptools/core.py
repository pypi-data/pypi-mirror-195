...
class WPTools(object):
    ...
    def _get(self, action, show, proxy, timeout=30):
        """
        ...
        if proxy:
            response = self._session.get(url=qstr, params=qobj.status, proxies=proxy, timeout=timeout or 30)
        else:
            response = self._session.get(url=qstr, params=qobj.status, timeout=timeout or 30)
