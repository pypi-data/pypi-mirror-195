...
class TestGridView(TestCase):
    ...
    def test_select_context_menu_item(self):
        SAPGUI.open_new_session(SECRETS['connection_string'], SECRETS['user_id'], SECRETS['password'], SECRETS['client'], SECRETS['language'])
        SAPGUI.run_transaction('su3')
        SAPGUI.select('wnd[0]/usr/tabsTABSTRIP1/tabpPARAM')
        SAPGUI.grid_view.select_context_menu_item(r'wnd[0]/usr/tabsTABSTRIP1/tabpPARAM/ssubMAINAREA:SAPLSUID_MAINTENANCE:1104/cntlG_PARAMETER_CONTAINER/shellcont/shell', '&XXL')