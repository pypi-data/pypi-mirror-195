...
class Client:
    ...
    Args:
        domain: str
            The base domain or IP address of the RIME service.
        api_key: str
            The API key used to authenticate to RIME services.
        channel_timeout: float
            The amount of time in seconds to wait for responses from the RIME cluster.
    ...
    def create_project(
        ...
        Returns:
            A ``Project`` object.
            The ``project_id`` attribute of this ``Project`` object is used
            by the ``start_stress_test()`` and ``list_stress_testing_jobs()``
            calls.
    ...
    def delete_project(self, project_id: str) -> None:
        ...
        Example:
        .. code-block:: python
           project = rime_client.delete_project("123-456-789")
        """
    ...
    def create_managed_image(
        ...
        Args:
            ...
            package_requirements: Optional[List[ManagedImagePackageRequirement]]
                An optional List of additional package requirements to install
                on the managed image. Currently only supports requirements for `apt`
                packages. Create a  ``ManagedImage.PackageRequirement`` parameter with
                the``Client.apt_requirement`` helper method .
