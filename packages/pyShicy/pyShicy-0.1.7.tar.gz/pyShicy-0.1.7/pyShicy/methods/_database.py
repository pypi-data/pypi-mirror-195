...
try:
    from pyShicy.exceptions import DependencyMissingError
except ImportError:
    from pyShicy.exceptions import DependencyMissingError
...
class SqlDB:
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS pyShicy (pyShicyCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('pyShicy')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'pyShicy'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM pyShicy")
    ...
    def set_key(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE pyShicy DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE pyShicy ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO pyShicy ({key}) values (%s)", (str(value),))
        return True
    ...
    def del_key(self, key):
        if key in self._cache:
            del self._cache[key]
        try:
            self._cursor.execute(f"ALTER TABLE pyShicy DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE pyShicy")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS pyShicy (pyShicyCli varchar(70))"
        )
        return True
