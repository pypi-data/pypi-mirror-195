...
class Helpers(object):
    ...
    def gen_chlog(self, repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Shicy Ubot {shicy_ver} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Shicy Ubot {shicy_ver} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
    ...
    async def restart(xd=None):
        ...
        if Var.HEROKU_APP_NAME and Var.HEROKU_API:
            ...
            if len(sys.argv) == 1:
                os.execl(sys.executable, sys.executable, "-m", "pyShicy")
            else:
                os.execl(
                    sys.executable,
                    sys.executable,
