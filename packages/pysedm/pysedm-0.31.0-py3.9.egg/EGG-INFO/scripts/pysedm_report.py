...
def build_image_report(specfile):
    ...
    # ADR
    try:
        img_adr  = pil.Image.open(pysedm.io.get_night_files(date, "re:^adr_", filesourcename, extention=".png")[0])
    except:
        img_adr= pil.get_buffer([9, 4], "ADR image missing" , **prop_missing)

    # Output Spectra
    all_spectra_files = pysedm.io.get_night_files(date, "re:^spec_", filesourcename, extention=".png")
    extention = "%s.png" % object_name.split("+")[-1] if "+" in object_name else "%s.png" % object_name
    pysedm_spec_file = pysedm.io.get_night_files(date, "re:^spec_", filesourcename, extention=extention)
    if not STD:
