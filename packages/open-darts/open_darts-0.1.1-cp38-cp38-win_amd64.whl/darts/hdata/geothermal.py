...
class Geothermal(PhysicsBase):
    ...
    def __init__(self, property_container, timer, n_points, min_p, max_p, min_e, max_e, mass_rate=False, grav=False,
        ...
        if grav:
            ...
            self.acc_flux_etor = acc_flux_gravity_evaluator_python(self.property, self.n_ops)
            self.acc_flux_etor_well = acc_flux_gravity_evaluator_python_well(self.property, self.n_ops)
        else:
            self.n_ops = 2 * self.n_components + self.thermal * 6
            self.engine = eval("engine_nce_%s%d" % (platform, self.n_components))()
            self.acc_flux_etor = acc_flux_custom_iapws_evaluator_python(self.property)
            self.acc_flux_etor_well = acc_flux_custom_iapws_evaluator_python_well(self.property)
        ...
        if mass_rate:
            self.rate_etor = geothermal_mass_rate_custom_evaluator_python(self.property)
        else:
            self.rate_etor = geothermal_rate_custom_evaluator_python(self.property)
    ...
    def set_nonuniform_initial_conditions(self, mesh, pressure_grad, temperature_grad, temp_anomaly):
        ...
        for i in range(tbl_len):
            state = value_vector([tbl_pres[i], 0])
