...
class ImageModelTraining:
    ...
    def train_model(self, model, criterion, optimizer):
        ...
        for epoch in range(self.num_epochs):
            ...
            running_loss = 0.0
            with tqdm(total=len(self.train_loader)) as progress_bar:
                for i, (inputs, labels) in enumerate(self.train_loader, 0):
                    ...
                    progress_bar.set_description(f"Training loss: {running_loss / (i + 1):.4f}")

            evaluate_accuracy = self.evaluate_model(model=model, data_loader=self.val_loader, device=self.device)
            print(
                f"Train Loss: {running_loss / len(self.train_loader):.4f} - Evaluate Accuracy: {evaluate_accuracy:.4f}")
        return running_loss / len(self.train_loader), evaluate_accuracy
