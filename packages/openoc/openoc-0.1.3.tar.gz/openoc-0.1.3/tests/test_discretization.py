...
def test_D_x():
    mesh = np.array([0, 0.1, 0.3, 0.4, 0.7, 0.8, 0.85, 1], dtype=np.float64)
    num_point = np.array([6, 7, 5, 6, 7, 5, 6], dtype=np.int32)
    continuity_x = np.array([True, False, True, False], dtype=bool)
    t_c, _ = xw_c(mesh, num_point)
    t_nc, _ = xw_nc(mesh, num_point)
    x = np.concatenate([t_c ** 2, t_nc ** 2, t_c ** 3, t_nc ** 3])
    d_x = np.concatenate([2 * t_nc, 2 * t_nc, 3 * t_nc ** 2, 3 * t_nc ** 2])
    assert np.allclose(D_x(mesh, num_point, continuity_x) @ x, d_x)

def test_aug():
    mesh = np.array([0, 0.1, 0.3, 0.4, 0.7, 0.8, 0.85, 1], dtype=np.float64)
    num_point = np.array([6, 7, 5, 6, 7, 5, 6], dtype=np.int32)
    continuity_x = np.array([True, False, True, False], dtype=bool)
    continuity_u = np.array([True, False], dtype=bool)
    t_c, _ = xw_c(mesh, num_point)
    t_nc, _ = xw_nc(mesh, num_point)
    t_nc_aug, _ = xw_nc(mesh, num_point + 1)
    x = np.concatenate([t_c ** 2, t_nc ** 2, t_c ** 3, t_nc ** 3])
    xu = np.concatenate([t_c ** 2, t_nc ** 2, t_c ** 3, t_nc ** 3, t_c ** 4, t_nc ** 4])
    v_xu = np.concatenate([t_nc_aug ** 2, t_nc_aug ** 2, t_nc_aug ** 3, t_nc_aug ** 3, t_nc_aug ** 4, t_nc_aug ** 4])
    d_x = np.concatenate([2 * t_nc_aug, 2 * t_nc_aug, 3 * t_nc_aug ** 2, 3 * t_nc_aug ** 2])
    assert np.allclose(V_xu_aug(mesh, num_point, continuity_x, continuity_u) @ xu, v_xu)
    assert np.allclose(D_x_aug(mesh, num_point, continuity_x) @ x, d_x)
