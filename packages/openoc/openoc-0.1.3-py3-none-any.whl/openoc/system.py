...
class System:
    def __init__(self, static_parameter: Union[int, List[str]], /, simplify: bool = False, parallel: bool = False,
        ...
        self._basic_value_hasher = xxhash.xxh64()
    ...
    def _basic_value(self, x):
        self._basic_value_hasher.reset()
        self._basic_value_hasher.update(x)
        if self._basic_value_hash != self._basic_value_hasher.hexdigest():
            ...
            self._basic_value_hash = self._basic_value_hasher.hexdigest()
    ...
    def constraints(self, x):
        ...
        d_est_phase = np.concatenate([p.D_x @ v[:p.L_x] / dt for p, v, dt in zip(self.p, V, DT)])
        d_func_phase = np.concatenate([f_d.F(m, p.l0_m) for p, m in zip(self.p, M) for f_d in p.F_d])
        c_dynamics = d_est_phase - d_func_phase

        c_phase = np.concatenate([f_c.F(m, p.l0_m) for p, m in zip(self.p, M) for f_c in p.F_c])
