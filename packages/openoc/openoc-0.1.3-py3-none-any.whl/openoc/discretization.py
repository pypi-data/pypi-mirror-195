...
def D_x(mesh: NDArray[np.float64], num_point: NDArray[np.int32], continuity_x: NDArray[bool]) \
        -> NDArray[np.float64]:
...
def D_lgl_aug(num_point: int) -> NDArray[np.float64]:
    """Compute the derivative matrix of the Legendre-Gauss-Lobatto nodes for error check, i.e. x_aug = V @ x"""
    x, _ = xw_lgl(num_point)
    x_aug, _ = xw_lgl(num_point + 1)
    D = []
    for i in range(num_point):
        ...
        poly = scipy.interpolate.lagrange(x, y)
        deriv_poly = np.polyder(poly)
        D.append(np.polyval(deriv_poly, x_aug))
    return np.array(D, dtype=np.float64).T
...
def D_c_aug(mesh: NDArray[np.float64], num_point: NDArray[np.int32]) -> NDArray[np.float64]:
    """Compute the derivative matrix of continuous variables for error check"""
    data = []
    row = []
    col = []
    l_row, r_row = lr_nc(num_point + 1)
...
def D_nc_aug(mesh: NDArray[np.float64], num_point: NDArray[np.int32]) -> NDArray[np.float64]:
    """Compute the derivative matrix of non-continuous variables for error check"""
    data = []
    row = []
    col = []
    l_row, r_row = lr_nc(num_point + 1)
...
def V_xu_aug(mesh: NDArray[np.float64], num_point: NDArray[np.int32], continuity_x: NDArray[bool],
             continuity_u: NDArray[bool]) -> NDArray[np.float64]:
    """Compute the value matrix of all state variables for error check"""
    V_c_ = V_c_aug(mesh, num_point)
    V_nc_ = V_nc_aug(mesh, num_point)
    diag = [V_c_ if c else V_nc_ for c in continuity_x] + [V_c_ if c else V_nc_ for c in continuity_u]
    return scipy.sparse.block_diag(diag, format='csr')


def D_x_aug(mesh: NDArray[np.float64], num_point: NDArray[np.int32], continuity_x: NDArray[bool]) \
        -> NDArray[np.float64]:
    """Compute the derivative matrix of all state variables for error check"""
    D_c_ = D_c_aug(mesh, num_point)
    D_nc_ = D_nc_aug(mesh, num_point)
    diag = [D_c_ if c else D_nc_ for c in continuity_x]
    return scipy.sparse.block_diag(diag, format='csr')
