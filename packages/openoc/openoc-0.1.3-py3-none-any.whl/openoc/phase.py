...
class Phase:
    ...
    def set_discretization(self, mesh: Union[int, Iterable[float]], num_point: Union[int, Iterable[int]],
        ...
        self._V_xu_aug = V_xu_aug(self._mesh, self._num_point, self._continuity_state, self._continuity_control)
        self._D_x_aug = D_x_aug(self._mesh, self._num_point, self._continuity_state)
    ...
    def _error_estimation_data_continuous(self, variable, static_parameter: Iterable[float] = None):
        ...
        d_x_aug = (self._D_x_aug @ variable.data[:self.r_v[self.n_x - 1]]).reshape(self.n_x, self.l0_m + self.N) / dt
        f_d_aug = np.array([f_d.F(m_aug, self.l0_m + self.N) for f_d in self.F_d], dtype=np.float64)
        return d_x_aug, f_d_aug
    ...
    def _error_check_interval_continuous(self, d_x_aug, f_d_aug, atol, rtol, mtol):
        ...
        for i_n in range(self.N):
            ...
            ec_c[i_n] = np.allclose(d_x_aug[:, l:r], f_d_aug[:, l:r], atol=atol, rtol=rtol)
        return ec_c
    ...
    def check(self, variable, static_parameter: Iterable[float] = None, /,
        ...
        """
        d_x_aug, f_d_aug = self._error_estimation_data_continuous(variable, static_parameter)
        f_b = self._error_estimation_data_discontinuous(variable, static_parameter)
        ec_c = self._error_check_interval_continuous(
            d_x_aug, f_d_aug, absolute_tolerance_continuous, relative_tolerance_continuous, tolerance_mesh)
        ...
        if not np.all(ec_c):
            ...
            for i_n in range(self.N):
                if ec_c[i_n]:
                    mesh_num_point.append((self._mesh[i_n], self._num_point[i_n]))
                    continue
                d_x_aug_n = d_x_aug[:, self._l_nc_aug[i_n]:self._r_nc_aug[i_n]]
                f_d_aug_n = f_d_aug[:, self._l_nc_aug[i_n]:self._r_nc_aug[i_n]]
                relative_error_max = np.max(np.abs(d_x_aug_n - f_d_aug_n) / np.abs(f_d_aug_n))
