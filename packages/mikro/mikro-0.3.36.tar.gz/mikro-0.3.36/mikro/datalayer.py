...
class DataLayer(KoiledModel):
    ...
    async def astore_bigfile(self, file: BigFile) -> None:
        """Store a DataFrame in the DataLayer"""
        if not self._connected:
            if self.auto_connect:
                await self.aconnect()

        print(self.secret_key, self.access_key, self.session_token)
        ...
        async with session.create_client('s3', region_name='us-west-2',
                                   ...
                                   aws_session_token=self.session_token) as client:
            
            resp = await client.put_object(Bucket="mikromedia",
                                                Key=key,
                                                Body=file.value)

            print(resp)
        print(f"mikromedia/{key}")
