...
def refine_data(data: dict, refine_words: list) -> dict:
    """
    주어진 딕셔너리에서 refine_words에 해당하는 키를 삭제해서 반환하는 유틸함수. c10346에서 사용
    """
    ...
    def _find(self, title: str, refine_words: list) -> Tuple[List[dict], int]:
        ...
        if title in titles:
            ...
            for data in self.my_col.find({'항목': {'$eq': title}}):
                count += 1
                data_list.append(refine_data(data, refine_words))
...
class C101(MongoBase):
    def __init__(self, client, code: str):
        super().__init__(client=client, db_name=code, col_name='c101')
    ...
    def save_dict(self, c101_dict: dict) -> bool:
        ...
        Note:
            ...
            '유통비율', 'intro'\n
        """
        c101_template = ['date', '코드', '종목명', '업종', '주가', '거래량', 'EPS', 'BPS', 'PER', '업종PER', 'PBR', '배당수익률',
                      '최고52주', '최저52주', '거래대금', '시가총액', '베타52주', '발행주식', '유통비율']
        # 리스트 비교하기
...
class C106(MongoBase):
    ...
    def find(self, title: str) -> dict:
        ...
        if title in titles:
            ...
            return refine_data(data, ['_id', '항목'])
        else:
            raise Exception(f'{title} is not in {titles}')