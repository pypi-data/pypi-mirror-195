...
def login(key: str) -> (bool, str):
    rsp = http_interface.login(get_http_host(), key, get_use_https())
    if rsp["result"] == "success":
        ...
        if use_grpc_tls:
            channel_opt = ()
            # channel_opt += (('grpc.ssl_target_name_override', 'bokchi.com'),)
            channel = grpc.secure_channel(grpc_url, grpc.ssl_channel_credentials(), channel_opt)
        else:
