...
# s.headers = {'User-Agent':'Mozilla/5.0 (Linux; U; Android 4.4.2; zh-cn; GT-I9500 Build/KOT49H) AppleWebKit/537.36(KHTML, like Gecko)Version/4.0 MQQBrowser/5.0 QQ-URL-Manager Mobile Safari/537.36'}
# s = rq.Session()
# s.cookies = http.cookiejar.MozillaCookieJar(
    # 
path_cookies=".data/cookies.txt"
...
def get_videos_bg_link(url:str,page_number:int,query:str) -> list[VideoData]:
    ...
    load_cookies(asession,path_cookies)
    url_html = br.get(url)
    if 'Enable JavaScript and cookies to continue' in url_html.text:
        save_cookies(asession,path_cookies)
