...
class XmlChunk(ChunkInformation):
    ...
    def __init__(self, reader: FileIO = None, size: int = None, offset: int = None, name: str = None):
        ...
        #   Set the default values for the various items
        self._version = None
        self._project = None
        self._scene = None
        self._take = None
        self._user_bits = None
        self._file_uid = None
        self._note = None
        self._speed_note = None
        self._speed_master_speed = None
        self._speed_current_speed = None
        self._speed_timecode_flag = None
        self._speed_timecode_rate = None
        self._speed_file_sample_rate = None
        self._speed_audio_bit_depth = None
        self._speed_digitizer_sample_rate = None
        self._speed_timestamp_sample_rate = None
        self._speed_timestamp_samples_since_midnight_hi = None
        self._speed_timestamp_samples_since_midnight_lo = None
        self._history = None
        self._file_set = None
        ...
        if (reader is not None) and (offset is not None) and (size is not None):
            ...
            for child in tree:
                if child.tag == "IXML_VERSION":
                    self._version = float(child.text)
                elif child.tag == "PROJECT":
                    self._project = child.text
                elif child.tag == "SCENE":
                    self._scene = child.text
                elif child.tag == "TAKE":
                    self._take = child.text
                elif child.tag == "UBITS":
                    self._user_bits = child.text
                elif child.tag == "FILE_UID":
                    self._file_uid = child.text
                elif child.tag == "NOTE":
                    self._note = child.text
                elif child.tag == "SPEED":
                    for node in child:
                        if node.tag == "NOTE":
                            self._speed_note = node.text
                        elif node.tag == "MASTER_SPEED":
                            self._speed_master_speed = node.text
                        elif node.tag == "CURRENT_SPEED":
                            self._speed_current_speed = node.text
                        elif node.tag == "TIMECODE_FLAG":
                            self._speed_timecode_flag = node.text
                        elif node.tag == "TIMECODE_RATE":
                            self._speed_timecode_rate = node.text
                        elif node.tag == "FILE_SAMPLE_RATE":
                            self._speed_file_sample_rate = float(node.text)
                        elif node.tag == "AUDIO_BIT_DEPTH":
                            self._speed_audio_bit_depth = float(node.text)
                        elif node.tag == "DIGITIZER_SAMPLE_RATE":
                            self._speed_digitizer_sample_rate = float(node.text)
                        elif node.tag == "TIMESTAMP_SAMPLE_RATE":
                            self._speed_timestamp_sample_rate = node.text
                        elif node.tag == "TIMESTAMP_SAMPLES_SINCE_MIDNIGHT_HI":
                            self._speed_timestamp_samples_since_midnight_hi = int(node.text)
                        elif node.tag == "TIMESTAMP_SAMPLES_SINCE_MIDNIGHT_LO":
                            self._speed_timestamp_samples_since_midnight_lo = int(node.text)
                elif child.tag == "HISTORY":
                    self._history = child.text
                elif child.tag == "FILE_SET":
                    self._file_set = child.text
                elif child.tag == "TRACK_LIST":
                    self._track_list = child.text
    ...
    @property
    def user_bits(self):
        return self._user_bits

    @user_bits.setter
    def user_bits(self, value):
        self._user_bits = value

    @property
    def unique_file_id(self):
        return self._file_uid

    @unique_file_id.setter
    def unique_file_id(self, value):
        self._file_uid = value
    ...
    @property
    def speed_note(self):
        return self._speed_note

    @speed_note.setter
    def speed_note(self, value):
        self._speed_note = value

    @property
    def master_speed(self):
        return self._speed_master_speed

    @master_speed.setter
    def master_speed(self, value):
        self._speed_master_speed = value

    @property
    def current_speed(self):
        return self._speed_current_speed

    @current_speed.setter
    def current_speed(self, value):
        self._speed_current_speed = value

    @property
    def timecode_flag(self):
        return self._speed_timecode_flag

    @timecode_flag.setter
    def timecode_flag(self, value):
        self._speed_timecode_flag = value

    @property
    def timecode_rate(self):
        return self._speed_timecode_rate

    @timecode_rate.setter
    def timecode_rate(self, value):
        self._speed_timecode_rate = value

    @property
    def sample_rate(self):
        return self._speed_file_sample_rate

    @sample_rate.setter
    def sample_rate(self, value):
        self._speed_file_sample_rate = value

    @property
    def audio_bit_depth(self):
        return self._speed_audio_bit_depth

    @audio_bit_depth.setter
    def audio_bit_depth(self, value):
        self._speed_audio_bit_depth = value

    @property
    def digitizer_sample_rate(self):
        return self._speed_digitizer_sample_rate

    @digitizer_sample_rate.setter
    def digitizer_sample_rate(self, value):
        self._speed_digitizer_sample_rate = value

    @property
    def timestamp_sample_rate(self):
        return self._speed_timestamp_sample_rate

    @timestamp_sample_rate.setter
    def timestamp_sample_rate(self, value):
        self._speed_timestamp_sample_rate = value
...
class WaveFile(Waveform):
    ...
    def save(self, path):
        ...
        with open(path, 'wb') as file:
            ...
            if self.xml_chunk is not None or isinstance(self.start_time, datetime):
                self.xml_chunk
