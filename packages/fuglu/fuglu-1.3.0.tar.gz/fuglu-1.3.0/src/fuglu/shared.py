...
def default_template_values(suspect, values=None):
    ...
    values['subject'] = suspect.get_message_rep()['subject'] if suspect.has_message_rep() else \
        suspect.build_headeronly_message_rep()['subject']
...
class Suspect(SimpleTimeoutMixin):
    ...
    def to_localpart(self) -> tp.Optional[str]:
        ...
        try:
            ...
            logging.getLogger('suspect').error(f'{self.id} could not extract localpart from recipient address {self.to_address}')
            return None
    ...
    def to_domain(self) -> tp.Optional[str]:
        ...
        try:
            ...
            logging.getLogger('suspect').error(f'{self.id} could not extract domain from recipient address {self.to_address}')
            return None
    ...
    def from_localpart(self) -> tp.Optional[str]:
        ...
        if not self.from_address:
            ...
            try:
                ...
                logging.getLogger('suspect').error(f'{self.id} could not extract localpart from sender address {self.from_address}')
                return None
    ...
    def from_domain(self) -> tp.Optional[str]:
        ...
        if not self.from_address:
            ...
            try:
                ...
                logging.getLogger('suspect').error(f'{self.id} could not extract domain from sender address {self.from_address}')
                return None
    ...
    def debug(self, message:str) -> None:
        ...
        try:
            ...
            logging.getLogger('suspect').error(f'{self.id} Could not write to logfile: {e.__class__.__name__}: {str(e)}')
    ...
    def get_original_source(self, maxbytes:int=None) -> bytes:
        ...
        try:
            ...
            logging.getLogger('fuglu.suspect').error(f'{self.id} Cannot retrieve original source from tempfile {self.tempfilename()} due to {e.__class__.__name__}: {str(e)}')
            raise e
        return source
    ...
    def __init_socket__(self):
        ...
        if unixsocket:
            ...
            try:
                ...
                raise Exception(f'Could not reach {self.enginename} server using unix socket {path_or_port}')
        else:
            host = self.config.get(self.section, 'host', resolve_env=True)
            try:
                sock = socket.create_connection((host, port), timeout)
            except socket.error:
                raise Exception(f'Could not reach {self.enginename} server using network ({host}, {path_or_port})')
    ...
    def strip_text(self, content, remove_tags=None, replace_nbsp=True, use_bfs=True, fugluid: str = "<>"):
        ...
        if HAVE_BEAUTIFULSOUP and use_bfs:
            try:
                ...
                self.logger.warning(f"{fugluid} Problem creating BeautifulSoup object: {str(e)}")
            else:
