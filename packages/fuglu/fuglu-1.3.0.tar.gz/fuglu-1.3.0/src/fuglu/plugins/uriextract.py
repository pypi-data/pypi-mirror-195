...
class EmailExtract(URIExtract):
    ...
    def _run(self, suspect):
        ...
        hrefs = self._check_skiplist(hrefs, extractfuncname='extractemails', use_hacks=usehacks)
        ...
        for mail in body_emails_final:
            if mail and "@" in mail:
                try:
                    ...
                    self.logger.error(f"{suspect.id} (bodydomain) Couldn't split {mail} in localpart & domain: {str(e)}")
        ...
        for mail in hdr_emails_final:
            if mail and "@" in mail:
                try:
                    ...
                    self.logger.error(f"{suspect.id} (headerdomain) Couldn't split {mail} in localpart & domain: {str(e)}")
                ...
                try:
                    ...
                    for identifier, humanreadable in iter(listings.items()):
                        if identifier in welcomelists:
                            self.logger.debug(f'{suspect.id} skipping lookup of {domain} (welcomelisted)')
                            welcomelisted = True
                            continue
    ...
    def lint(self):
        ...
        if allok:
            ...
            if not blconf:
                ...
                allok = False
                print('ERROR: blocklistconfig %s not found' % blconf)
            else:
                self._init_rbllookup()
                if self.rbllookup is None:
                    allok = False
...
class EmailAction(DomainAction):
    ...
    def __init__(self, config, section=None):
        ...
        self.requiredvars['blacklistconfig']['default'] = ''
        self.requiredvars['blocklistconfig']['default'] = '${confdir}/rblemail.conf'
