    ...
    def _send_sync(self, suspect: Suspect, target_host: str, target_args: dict, from_address: str, to_address: str, retry: int = 3):
        try:
            ...
            try:
                ...
                self._log_response(f'{suspect.id} sent quit to {target_host}', quit_resp)
            except Exception as e:
                self.logger.debug(f'{suspect.id} error sending quit to {target_host}: {e.__class__.__name__}: {str(e)}', level=LOGLEVEL_EXTREME)
        except Exception as e:
