...
class ElasticBackend(object):
    ...
    def log_to_elastic(self, suspect: Suspect, logdata: tp.Dict[str, tp.Any], rcpt_id: int, reload=False, retry: int = 3) -> tp.Dict[str, str]:
        ...
        try:
            ...
            self.logger.info(f'{suspect.id} indexed in elastic {indexname}: {r}')
        except Exception as e:
            if retry > 0:
...
class ElasticLogger(AppenderPlugin):
    ...
    def _get_suspect_fields(self, suspect: Suspect) -> tp.Dict[str, str]:
        ...
        for field in suspect_fields:
            ...
            if field_tag and field_tag.startswith('low:') and value:
                value = value.lower()
    ...
    def process(self, suspect: Suspect, decision: str) -> None:
        ...
        for recipient in suspect.recipients:
            ...
            try:
                ...
                self.logger.info(f'{suspect.id} processed with result {result} in {processtime:.2f}s')
            except Exception as e:
                processtime = time.time()-starttime
...
class ElasticImport(object):
    ...
    def _log_to_elastic(self, indexname, suspectid, logdata):
        ...
        if not '_id' in r:
            ...
            success = False
        print(f'{suspectid} indexed in elastic: {r}')
        return success
