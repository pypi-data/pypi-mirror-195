...
class FuzorMixin(object):
    ...
    def report(self, suspect):
        ...
        try:
            self.logger.debug(
                f"{suspect.id} to={suspect.to_address} hash={fuhash.digest} usable_body={fuhash.bodytext_size} predigest={fuhash.predigest[:50]} subject={msg.get('Subject')}")
        except Exception:
            pass
        ...
        if fuhash.digest is not None:
            ...
            try:
                ...
                self.logger.error(f'{suspect.id} failed to connect to backend: {str(e)}')
                have_backend = False
            ...
            while attempts:
                ...
                try:
                    ...
                    self.logger.info(f"{suspect.id} hash {fuhash} seen {count-1} times before")
                    attempts = 0
