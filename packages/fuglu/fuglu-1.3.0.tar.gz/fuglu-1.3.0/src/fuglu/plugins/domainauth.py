...
class SPFPlugin(ScannerPlugin, BasicMilterPlugin, BMPMailFromMixin, BMPRCPTMixin, BMPEOBMixin):
    ...
    def _check_skip_on_tag(self, suspect):
        ...
        for tag in tags:
            if suspect.get_tag(tag, False) is True:
                return tag
        return None
    ...
    def _get_clientinfo(self, suspect:Suspect):
        clientinfo = suspect.get_client_info(self.config)
...
Requires python dmarc library
    ...
    def _do_dmarc_check(self, dmarc_record, header_from_domain, aspf, adkim, suspectid):
        ...
        try:
            ...
            self.logger.info(f'{suspectid} invalid DMARC record: {str(e)}')
        except ValueError:
            if aspf is not None:
    ...
    def examine(self, suspect):
        ...
        self.logger.debug(
            f'{suspect.id} dmarc eval with result={result} and dispo={dispo} input spf={spf_result} dkim_result={dkim_result} dkim_domain={dkim_domain} dkim_selector={dkim_selector} ')
...
Only mail that would not sender domain's SPF policy will be allowed to pass.
    ...
    def __init__(self, config, section=None):
        ...
        self.requiredvars['create_received_spf']['default'] = 'False'
...
class NoHaveAuth(ScannerPlugin):
    ...
    def examine(self, suspect: Suspect):
        ...
        spf_checked = suspect.get_tag("SPF.status") not in [None, 'skipped']
