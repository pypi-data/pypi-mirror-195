...
class BasicTCPServer(object):
    ...
    def serve(self):
        ...
        threading.current_thread().name = '%s Server on Port %s' % (
        ...
        while self.stayalive:
            try:
                ...
                self.logger.debug('(%s) Incoming connection  [incoming server port: %s, prot: %s]' %
                                  (createPIDinfo(), self.port, self.protohandlerclass.protoname))
