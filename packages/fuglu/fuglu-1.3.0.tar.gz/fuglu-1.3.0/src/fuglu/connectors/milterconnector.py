...
class MilterHandler(ProtocolHandler):
    ...
    def get_suspect(self, **kwargs):
        ...
        if temp_filename and not os.path.exists(temp_filename):
            self.logger.warning(f"{id} File '{temp_filename}' not found for suspect creation! from: {from_address}, to: {recipients}")
            return None
        ...
        suspect = Suspect(from_address, recipients, temp_filename, att_cachelimit=self._att_mgr_cachesize,
                          ...
                          queue_id=sess.queueid, id=id, milter_macros=sess.milter_macros)
        ...
        logging.getLogger('fuglu.MilterHandler.queueid').info(f'"{id}" "{sess.queueid if sess.queueid else "NOQUEUE"}"')
