    ...
    def re_inject(self, suspect):
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f'{suspect.id} Exception while quitting re-inject session to {targethost}: {e.__class__.__name__}: {str(e)}')

        if responsecode is None:
            self.logger.warning(f'{suspect.id} Re-inject: could not get server response code from {targethost}')
            responsecode = 451
        if serveranswer is None:
            self.logger.warning(f'{suspect.id} Re-inject: could not get server answer from {targethost}')
            serveranswer = ''
