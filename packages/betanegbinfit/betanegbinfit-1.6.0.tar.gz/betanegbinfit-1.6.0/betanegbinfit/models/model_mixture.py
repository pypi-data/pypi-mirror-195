...
class ModelMixture(Model):
    ...
    def mean(self, params=None, return_modes=False):
        ...
        if self.dist == 'BetaNB':
            k = self.get_param('k', params)
            l1 = dist.LeftTruncatedBetaNB.mean(p1, k, r, self.left, r_transform=self.r_transform)
            if self.bad == 1:
                l2 = l1
            else:
                l2 = dist.LeftTruncatedBetaNB.mean(p2, k, r, self.left, r_transform=self.r_transform)
        elif self.dist == 'NB':
