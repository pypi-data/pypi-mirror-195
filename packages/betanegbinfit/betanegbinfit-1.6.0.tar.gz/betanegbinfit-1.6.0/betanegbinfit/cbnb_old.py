...
def get_hats(aux, aefg, aefg_1, aefg_2, aefg_3, i):
    ...
    # call(lambda x: print('------', i), i)
    # call(lambda x: print(*x), aux.at[i - 1].get())
    # call(lambda x: print(*x), (g_3_hat / a_3_hat * d_n, f_2_hat / a_2_hat * c_n, e_1_hat / a_1_hat * b_n))
    beta_hat = beta - beta_3_hat / a_3_hat * d_n - beta_2_hat / a_2_hat * c_n - beta_1_hat / a_1_hat * b_n
    # beta_hat = jnp.abs(beta_hat)
    ...
    g_hat = g_n
    return jnp.abs(a_hat), e_hat, jnp.abs(f_hat), g_hat, beta_hat
...
def logprob_recurrent(r, p, k, upper, max_sz):
    ...
    b1 = 2.
    b2 = 2. - k * (1. - p)  - r
    aux = jnp.zeros((max_sz + ahead_comp, 5), dtype=float)
