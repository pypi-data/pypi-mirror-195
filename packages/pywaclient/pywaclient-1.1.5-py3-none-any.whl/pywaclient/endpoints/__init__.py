...
from pywaclient.exceptions import AccessForbidden, ResourceNotFound, InternalServerException, \
    UnexpectedStatusException, ConnectionException, UnprocessableDataProvided
...
def _parse_response(path, response) -> Dict[str, Any]:
    if response.ok:
        ...
        if data['success']:
            ...
            raise UnexpectedStatusException(response.status_code, data['error'], path)
...
class Endpoint:
    ...
    def _scroll_collection(self, path: str, params: Dict[str, str], collection_tag: str, parentName: str = '', parentId: str = '') -> Iterable[Dict[str, Any]]:
        limit = 50
        offset = 0
        if '.' in collection_tag:
            first, second = collection_tag.split('.')
        if parentName != '':
