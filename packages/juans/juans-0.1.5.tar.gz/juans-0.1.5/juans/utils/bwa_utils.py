"""
 Author: yican.yc
 Date: 2022-08-23 19:24:42
 Last Modified by:   yican.yc
 Last Modified time: 2022-08-23 19:24:42
"""
import warnings
import torch
import math
...
class SWALR(_LRScheduler):
    ...
    def __init__(self, optimizer, swa_lr, anneal_epochs=10, anneal_strategy="cos", last_epoch=-1):
        ...
        if not isinstance(anneal_epochs, int) or anneal_epochs < 0:
            raise ValueError("anneal_epochs must be equal or greater than 0, got {}".format(anneal_epochs))
        ...
        if isinstance(swa_lrs, (list, tuple)):
            if len(swa_lrs) != len(optimizer.param_groups):
                raise ValueError(
                    ...
                    "optimizer.param_groups has {}".format(len(swa_lrs), len(optimizer.param_groups))
                )
            return swa_lrs
        else:
