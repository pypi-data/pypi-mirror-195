...
class TestShift:
    @mark.parametrize(
        ("n", "expected_v"),
        [
            param(1, [nan, 0.0, 1.0], id="n=1"),
            param(2, [nan, nan, 0.0], id="n=2"),
            param(-1, [1.0, 2.0, nan], id="n=-1"),
            param(-2, [2.0, nan, nan], id="n=-2"),
        ],
    )
    @mark.parametrize("dtype", [param(float), param(int)])
    def test_1d(self, n: int, expected_v: list[float], dtype: type[Any]) -> None:
