...
class pd_shape_dp(ExternalOpImplementation):
    ...
    async def call(  # type: ignore[override]
        ...
        max_mul = MaxMultiplicityOp(
            parent_ds,
            epsilon,
            tasks[0].noise_user_count,
            tasks[0].noise_multiplicity,
            tasks[0].max_max_multiplicity,
        ).value(random_generator)

        n_rows = CountOp(
            parent_ds,
            noise=t.cast(GenericTask, tasks[1]).parameters['noise'],
    ...
    async def call(  # type: ignore[override]
        ...
        **kwargs: t.Any,
        ...
        for index, column_name in enumerate(dataframe.columns):
            subtasks = [unwrap(task) for task in tasks[index + 1].subtasks]
