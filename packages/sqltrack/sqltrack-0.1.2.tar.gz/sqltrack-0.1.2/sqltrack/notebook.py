...
def init_notebook_mode():
    ...
    """
    path = Path(__file__).parent / "notebook.css"
...
def format_dataframe(
    ...
    is selected based based on dtype:
        - Any `datetime`-like: `format_datetime_relative`
            or `format_datetime` if `relative_datetimes` is False
        - Any `str`-like: `format_string` with the given
    ...
    Parameters:
        ...
        relative_datetimes: if True (default), use
            `format_datetime_relative` for columns with
            datetime-like dtype, else `format_datetime`
        string_ellipsis: passed as `ellipsis` parameter to
            `format_string` for columns with str-like dtype
    """
    mapping = dict(DEFAULT_MAPPING, **(formatting or {}))
    mapping = {
        name: _partial_function(
            func,
            na_rep=na_rep,
