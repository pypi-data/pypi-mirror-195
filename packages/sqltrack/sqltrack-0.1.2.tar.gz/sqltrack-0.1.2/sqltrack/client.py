...
class Client:
    """
    A simple wrapper that creates psycopg.Connection objects
    ...
    Parameters passed directly during object creation take priority,
    but they may also be passed as environment variables `SQLTRACK_<PARAM>`,
    e.g., `SQLTRACK_USER`,
    or loaded from a config file, by default `./sqltrack.conf`

    Experiment and Run objects obtain a connections as required.
    ...
    Parameters:
        config_path: Path to config file, defaults to `./sqltrack.conf`
