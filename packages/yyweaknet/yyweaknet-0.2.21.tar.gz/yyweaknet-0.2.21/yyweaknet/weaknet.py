...
class WeakNetworkConfig(object):
    ...
    def __init__(self, atc_host="http://172.29.182.152:8000", stf_url=None, sft_token=None, wifi_name="yy_net",
                 wifi_password="", weak_network_ip="192.168.1"):
        """
    ...
    def _connect_android(self, serial):
        ...
        while True:
            ...
            if str(ip).startswith(self.weak_network_ip):
                self.set_auth(ip)
                return ip
            else:
                time.sleep(1)
    ...
    def _connect_ios(self, device):
        ...
        while True:
            ...
            if device_ip:
                ...
                if device_ip.startswith(self.weak_network_ip):
    ...
    def _disconnect_android(self, serial):
        ...
        ip = ret.get("ip", None)
        if not str(ip).startswith(self.weak_network_ip):
            return True
        else:
            return False
    ...
    def _disconnect_ios(self, serial):
        ...
        while True:
            ...
            if device_ip:
                # 弱网断网现在是192.168.0 如果网段变化则需要修改下面的逻辑
                if not device_ip.startswith(self.weak_network_ip):
