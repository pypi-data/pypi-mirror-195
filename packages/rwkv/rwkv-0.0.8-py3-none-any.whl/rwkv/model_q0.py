...
class RWKV(MyModule):
    def __init__(self, model, strategy):
        ...
        with torch.no_grad():
            ...
            print()
                            ...
                            # print(w[x+'_mx'].numpy())
                            # print(w[x+'_rx'].numpy())
                            # print(w[x+'_my'].flatten().numpy())
                            # print(w[x+'_ry'].flatten().numpy())
                            w[x] = torch.round(w[x] * 255.0).to(dtype=torch.uint8)
                            # print(w[x])
                            # bb = self.get_uint8_to_type(x, ATYPE)
                            # print(bb)
                            # print('err', torch.sqrt(torch.mean((aa-bb)*(aa-bb))))
                            # exit(0)
            ...
            for x in keys:
                ...
                if '.time_decay' in x: # need fp32 for this
                    ...
                    if (len(w[x].shape) == 2) and ('emb' not in x) and (not IS_STREAM):
                        if WTYPE != torch.uint8:
                            ...
                            if 'ffn' in x:
                                ...
                                aa = ww.clone()
                                print(aa)
                                w[x+'_rx'], _ = torch.max(torch.abs(ww), dim=0)
                                ww = ww / w[x+'_rx']
                                w[x+'_ry'], _ = torch.max(torch.abs(ww), dim=1)
                                ww = ww / w[x+'_ry'].unsqueeze(1)
                                print(w[x+'_rx'].numpy())
                                print(w[x+'_ry'].numpy())
                                w[x] = torch.round(ww * 127.0 + 127.0).to(dtype=torch.uint8)
                                print(w[x])
                                bb = self.get_uint8_to_type(x, ATYPE)
                                print(bb)
                                print('err', torch.sqrt(torch.mean((aa-bb)*(aa-bb))))
                ...
                if 'emb.' in x:
                    pass
                elif (IS_STREAM) and (x.endswith('key.weight') or x.endswith('value.weight') or x.endswith('receptance.weight') or x.endswith('output.weight')):
                    try:
                ...
                if layer_id == 0 or layer_id >= args.n_layer-1:
                    ...
                    dt = dt.replace('float32', 'fp32').replace('bfloat16', 'bf16').replace('float16', 'fp16').replace('uint8', 'int8')
    ...
    def get_uint8_to_type(self, x, dtype):
        w = self.w
        return (w[x].to(dtype=dtype) - 127.0) * w[x+'_rx'] * w[x+'_ry'].unsqueeze(1) / 127.0       
    ...
    def forward(self, tokens, state, full_output=False):
        with torch.no_grad():
            ...
            for i in range(args.n_layer):
                ...
                if dd.stream:
                    ...
                    if wtype != torch.uint8:
                        ...
                        kw = self.get_uint8_to_type(f'{att}key.weight', atype)
                        vw = self.get_uint8_to_type(f'{att}value.weight', atype)
                        rw = self.get_uint8_to_type(f'{att}receptance.weight', atype)
                        ow = self.get_uint8_to_type(f'{att}output.weight', atype)
                ...
                if dd.stream:
                    ...
                    if wtype != torch.uint8:
                        ...
                        kw = self.get_uint8_to_type(f'{ffn}key.weight', atype)
                        vw = self.get_uint8_to_type(f'{ffn}value.weight', atype)
                        rw = self.get_uint8_to_type(f'{ffn}receptance.weight', atype)
                x, state[i*5+4] = FFN(
                    x, sx=state[i*5+4],
            ...
            atype = self.strategy[args.n_layer].atype
            x = x[-1,:] if (seq_mode and (not full_output)) else x
            x = x.to(dtype=atype, device=self.strategy[args.n_layer].device)
            
            x = F.layer_norm(x, (args.n_embd,), weight=w['ln_out.weight'], bias=w['ln_out.bias'])
            if w['head.weight'].dtype != torch.uint8:
                x = x @ w['head.weight']
            else:
                x = x @ self.get_uint8_to_type('head.weight', atype)
