...
# SIZE = 4096
# torch.manual_seed(0)
# a1 = torch.rand((1, SIZE), dtype=torch.bfloat16, device=torch.device('cuda')) - 0.5
# a2 = torch.rand((1, SIZE), dtype=torch.bfloat16, device=torch.device('cuda')) - 0.5
# bb = torch.rand((SIZE, SIZE), dtype=torch.bfloat16, device=torch.device('cuda')) - 0.5
# cc = (torch.cat((a1,a2)) @ bb).float()
# d1 = cc[0]
# d2 = cc[1]
# c1 = (a1 @ bb).float()
# c2 = (a2 @ bb).float()
# print(torch.sum((d1-c1)*(d1-c1)))
# print(torch.sum((d2-c2)*(d2-c2)))
# exit(0)


# print(f'Loading model - {MODEL_NAME}')
# model = RWKV(model=MODEL_NAME, strategy='cuda bf16')
# out, state = model.forward([187], None)
# print('out0', out.detach().cpu().numpy())
# vx0 = model.vx.clone()
# vw = model.vw.clone()
# out, state = model.forward([510], state)
# print('out1', out.detach().cpu().numpy())
# vx1 = model.vx.clone()

# out, state = model.forward([187, 510], None, full_output=True)
...
# vv1 = (vx1 @ vw).float()
# vv = (vx @ vw).float()
# print(torch.sum(torch.square(vv0 - vv[0])))
# print(torch.sum(torch.square(vv1 - vv[1])))
# exit(0)
