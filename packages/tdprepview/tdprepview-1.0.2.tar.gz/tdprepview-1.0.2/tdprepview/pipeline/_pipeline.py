...
class Pipeline:
    ...
    steps : list
        ...
        The first element is the input columns, which can be a string, a list of strings or a dictionary.
            ...
            If it is a dictionary, it can have the keys 'prefix', 'suffix', 'pattern', 'dtype_include', and/or
                'dtype_exclude'. The value of each key must be a string or a list of strings. These key-value pairs will
                be used to get all matching columns from the current step
    ...
    def _check_and_reformat_steps(self, steps):
        ...
        for i, step in enumerate(steps):
            ...
            if isinstance(input_columns,dict):
                assert all([(k in ["prefix", "suffix", "pattern", "dtype_include", "dtype_exclude"])
                            for k in input_columns.keys()])
                for key, val in input_columns.items():
                    if key in ["prefix", "suffix", "pattern"]:
                        assert isinstance(val, str)
                    elif key in ["dtype_include", "dtype_exclude"]:
