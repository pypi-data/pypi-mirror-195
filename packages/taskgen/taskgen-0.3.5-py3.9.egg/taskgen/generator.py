...
def gen_subs_data(task_folder, n):
    ...
    lst = glob.glob(os.path.join(task_folder, '*.ipynb'))
    if len(lst) == 1:
        path = lst[0]
    elif len(lst) > 0:
        path = lst[0]
        logging.warning(
            'В папке задачи "' + task_folder + '" лежит несколько файлов с расширением .ipynb. \n'
            'В качестве файла параметризации выбран файл "' + path + '".')
    else:
        logging.error(
            'Файл параметризации не найден для задачи: "' + task_folder + '". \n'
...
def gen_subs(n=1, folder=config.get('GENERAL', 'bank_folder')):
    ...
    '''
    for ipynb_file in glob.glob(os.path.join(folder, '**', '*.ipynb'),
                                recursive=True):
