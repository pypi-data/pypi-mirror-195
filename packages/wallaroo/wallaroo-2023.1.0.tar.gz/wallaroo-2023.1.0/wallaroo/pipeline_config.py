...
class ModelWeight:
    ...
    @classmethod
    def from_tuple(cls, tup: Tuple[float, Model]):
        (weight, model) = tup
...
class PipelineConfigBuilder:
    ...
    def as_standalone(pipeline_name: str):
        return PipelineConfigBuilder(None, pipeline_name, standalone=True)
    ...
    def add_model_step(self, model: Model) -> "PipelineConfigBuilder":
    ...
    def add_multi_model_step(self, models: Iterable[Model]) -> "PipelineConfigBuilder":
    ...
    def _create_validation(self, name: str, validation: Expression) -> "Check":
        configured_models = set(m.model().name() for m in self._model_configs())
