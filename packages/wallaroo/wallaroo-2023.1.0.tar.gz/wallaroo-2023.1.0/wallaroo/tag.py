...
class Tag(Object):
    ...
    def _fill(self, data: Dict[str, Any]) -> None:
        ...
        # Optional
        self._model_tags = (
            [Model(self.client, model_tag["model"]) for model_tag in data["model_tags"]]
    ...
    def model_tags(self) -> List[Model]:
        return cast(List[Model], self._model_tags)
    ...
    def _repr_html_(self) -> str:
        def row(tag):
            ...
            return (
                ...
                + f"<td>{[(p.name(), [pv.name() for pv in p.variants()]) for p in pipelines]}</td>"
                + "</tr>"
            )
