...
from django.db.models import CASCADE, SET_NULL, ForeignKey, JSONField, fields
...
class Device(TimeStampedModel, ActivatorModel):
    ...
    def __str__(self):
        return "Device(id=%s, user=%s, service=%s, manufacturer=%s, model=%s, platform=%s, " \
               "serial=%s, token=%s, uuid=%s, version=%s)" % \
