        ...
        for consumer in itertools.cycle(self.consumers):
            ...
            try:
                try:
                    ...
                    self._yellow(f'[{timezone.now()}]: Job {consumer_name:<40} .. [error]:', ending='')
                    self._red(e)
                else:
                    try:
                        self._yellow(f'[{timezone.now()}]: Starting job {consumer_name:<40} ...')
                        consumer.process(record)
                    except Exception as e:
                        self._yellow(f'[{timezone.now()}]: Job {consumer_name:<40} .. [error]: ', ending='')
                        self._red(e)
                    else:
                        self._yellow(f'[{timezone.now()}]: Job {consumer_name:<40} .. [OK]')
