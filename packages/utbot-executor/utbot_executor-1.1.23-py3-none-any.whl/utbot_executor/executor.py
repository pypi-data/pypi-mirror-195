...
class PythonExecutor:
    ...
    def run_function(self, request: ExecutionRequest) -> ExecutionResponse:
        ...
        logging.debug("Module `%s`", request.function_module)
...
def run_calculate_function_value(
    ...
    logging.debug("State before...")
    _, _, _, state_before = _serialize_state(args, kwargs)
    logging.debug("State before OK")
    ...
    # )
    # __cov.start()
    logging.debug("Run function")
    try:
        with suppress_stdout():
            logging.debug("Inside suppress_stdout")
            __result = function(*args, **kwargs)
            logging.debug("Result %s", __result)
    except Exception as __exception:
        logging.debug("Exception %s", __exception)
        __result = __exception
        __is_exception = True
    # __cov.stop()
    logging.debug("Exception success OK")

    (__sources, __start, ) = inspect.getsourcelines(function)
    __end = __start + len(__sources)
    (_, __stmts, _, __missed, _, ) = 0, [], 0, [__start, __start+1, __start+2], 0  #__cov.analysis2(fullpath)
    # __cov.erase()
