...
def shell(cmd, keyword='', ignore_error=False, output=True, s3=False):
    if s3 is True:
        cmd += ' --output yaml'
    if output is False:
...
def get_file_size(file_path, provider):
    if provider == 's3':
        get_size_cmd = f'aws s3 ls --summarize --human-readable {file_path}'
        info = shell(cmd=get_size_cmd, keyword='Total Size', s3=True)
        size = 0
        if 'MiB' in info:
        ...
        get_size_cmd = f'gsutil du {file_path}'
        size = shell(cmd=get_size_cmd, s3=False)
        return int(size.split('gs://', 1)[0].strip()) / 1024


def get_instance_type(pbf, sp, ssp, provider):
    instance_type = 'r5.large'
    coefficient = 10

    size = get_file_size(pbf, provider) + get_file_size(sp, provider)
    if ssp != '':
        size += get_file_size(ssp, provider)
...
def get_volume_size(pbf, sp, ssp, provider):
    size = get_file_size(pbf, provider) + get_file_size(sp, provider)
    if ssp != '':
        size += get_file_size(ssp, provider)
