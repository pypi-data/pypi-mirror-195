...
class TransformerModelForCoreferenceResolutionBase(transformers.BertPreTrainedModel):
    """
    ...
    def __init__(self, pretrained_name_or_config, max_span_width=30, ffnn_hidden_size=1000, top_span_ratio=0.4,
                 ...
                 coref_dropout=0.3):
        if isinstance(pretrained_name_or_config, transformers.PretrainedConfig):
            base_config = pretrained_name_or_config
        else:
            base_config = transformers.AutoConfig.from_pretrained(pretrained_name_or_config)
        ...
        if isinstance(pretrained_name_or_config, transformers.PretrainedConfig):
            self.features_model = transformers.AutoModel.from_config(pretrained_name_or_config)
        else:
            self.features_model = transformers.AutoModel.from_pretrained(pretrained_name_or_config)
        base_emb_size = base_config.hidden_size
        self.span_attend_projection = torch.nn.Linear(base_emb_size, 1)
