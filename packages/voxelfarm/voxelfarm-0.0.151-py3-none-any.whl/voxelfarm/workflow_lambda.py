...
class workflow_lambda_host:
    ...
    def get_entity(self, id = None):
        entity = None
        framework_entity = self.lambda_framework.get_entity(id)
        if framework_entity:
            for property in framework_entity:
                entity[property] = framework_entity[property] 
    ...
    def _create_workflow_folders(self, vf, workflow, project_id, project_entity, parent_folder, fields, crs):
        ...
        if 'singletons' in workflow:
            existing_version = self.get_entity(parent_folder)
            ...
            for singleton in workflow['singletons']:
                ...
                if singleton_key in existing_singleton_ids:
                    ...
                    singleton_entity = self.get_entity(singleton_id)
        ...
        if 'tracks' in workflow:
            for track in workflow['tracks']:
                ...
                if field_id in project_entity:
                    ...
                    folder = self.get_entity(folder_id)
                    if folder != None:
    ...
    def set_workflow_definition(self, workflow_definition):
        ...
        if workflow_request.state == state.initialize:
            ...
            project_entity = self.get_entity(workflow_request.project_id)
        ...
        if 'success' in result and result['success']:
            if "complete" in result and result["complete"]:
                ...
                if (workflow_request.state != state.initialize) and ('activated' not in workflow_request.properties):
                    ...
                    product_entity = self.get_entity(workflow_request.product_folder_id)
    ...
    def get_product_singleton(self, product_id, singleton_id):
        ...
        if entity and 'project' in entity:
    ...
    def get_product_property(self, product_id, property):
        ...
        if entity and 'project' in entity:
            ...
            folder_id = 'workflow_folder_' + product_id
            if project_entity and folder_id in project_entity:
                product_folder_id = project_entity[folder_id]
                product_entity = self.get_entity(product_folder_id)
                if product_entity and 'version_active' in product_entity:
                    active_version_id = product_entity['version_active']
    ...
    def publish_reports(self, workflow_request: request, report_name:str, report_id: str, constant_spatial_id: str, constant_object_id: str, propertyIds:tuple, timestampID: str = None, spatialId: str = None, objectId: str = None):
        ...
        mesh_entity = self.get_entity(report_id)
        if not mesh_entity:
        ...
        iso_time = datetime.fromtimestamp(int(file_date)/1000.0).isoformat()
                        ...
                        try:
                            ...
                            if " " in timestamp:
                                if "AM" or "PM" in timestamp:
                                    entry_date = time.mktime(datetime.strptime(timestamp, "%d/%m/%Y %H:%M:%S %p").timetuple())
                                else: 
                                    entry_date = time.mktime(datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S").timetuple())
                            else:
                                entry_date = time.mktime(datetime.strptime(timestamp, "%Y-%m-%d").timetuple())
                            iso_time = datetime.fromtimestamp(entry_date).isoformat()
                        except:
    ...
    def notify_users(self, file_entity_id):
        ...
        file_entity = self.get_entity(file_entity_id)
        if file_entity != None:
            ...
            folder_entity = self.get_entity(file_entity['file_folder'])
            parent_entity = self.get_entity(folder_entity['file_folder'])
