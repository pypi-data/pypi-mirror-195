...
_batched_non_max_suppression_outputs = ["selected_indices", "selected_counts"]
_BatchedNonMaxSuppressionOutput = _collections.namedtuple(
    "BatchedNonMaxSuppression", _batched_non_max_suppression_outputs)


@_dispatch.add_dispatch_list
@tf_export('batched_non_max_suppression')
def batched_non_max_suppression(boxes, scores, box_counts, max_output_size, iou_threshold, score_threshold, pad_to_max_output_size=False, algorithm=-1, name=None):
  r"""TODO: add doc.

  Args:
    boxes: A `Tensor`. Must be one of the following types: `float32`.
    scores: A `Tensor`. Must have the same type as `boxes`.
    box_counts: A `Tensor` of type `int32`.
    max_output_size: A `Tensor` of type `int32`.
    iou_threshold: A `Tensor`. Must be one of the following types: `float32`.
    ...
    algorithm: An optional `int`. Defaults to `-1`.
    name: A name for the operation (optional).

  Returns:
    A tuple of `Tensor` objects (selected_indices, selected_counts).

    selected_indices: A `Tensor` of type `int32`.
    selected_counts: A `Tensor` of type `int32`.
  """
  _ctx = _context._context or _context.context()
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "BatchedNonMaxSuppression", name, _ctx.post_execution_callbacks,
        boxes, scores, box_counts, max_output_size, iou_threshold,
        score_threshold, "pad_to_max_output_size", pad_to_max_output_size,
        "algorithm", algorithm)
      _result = _BatchedNonMaxSuppressionOutput._make(_result)
      return _result
    except _core._FallbackException:
      try:
        return batched_non_max_suppression_eager_fallback(
            boxes, scores, box_counts, max_output_size, iou_threshold,
            score_threshold, pad_to_max_output_size=pad_to_max_output_size,
            algorithm=algorithm, name=name, ctx=_ctx)
      except _core._SymbolicException:
        pass  # Add nodes to the TensorFlow graph.
      except (TypeError, ValueError):
        result = _dispatch.dispatch(
              batched_non_max_suppression, boxes=boxes, scores=scores,
  ...
  try:
    _, _, _op = _op_def_lib._apply_op_helper(
        "BatchedNonMaxSuppression", boxes=boxes, scores=scores,
                                    box_counts=box_counts,
                                    max_output_size=max_output_size,
                                    iou_threshold=iou_threshold,
                                    score_threshold=score_threshold,
  ...
  _attrs = ("T", _op._get_attr_type("T"), "T_threshold",
            ...
            _op.get_attr("pad_to_max_output_size"), "algorithm",
            _op.get_attr("algorithm"))
  _execute.record_gradient(
      "BatchedNonMaxSuppression", _inputs_flat, _attrs, _result, name)
  _result = _BatchedNonMaxSuppressionOutput._make(_result)
  return _result

def BatchedNonMaxSuppression(boxes, scores, box_counts, max_output_size, iou_threshold, score_threshold, pad_to_max_output_size=False, algorithm=-1, name=None):
  return batched_non_max_suppression(boxes=boxes, scores=scores, box_counts=box_counts, max_output_size=max_output_size, iou_threshold=iou_threshold, score_threshold=score_threshold, pad_to_max_output_size=pad_to_max_output_size, algorithm=algorithm, name=name)
BatchedNonMaxSuppression.__doc__ = batched_non_max_suppression.__doc__
BatchedNonMaxSuppression = _doc_controls.do_not_generate_docs(_kwarg_only(BatchedNonMaxSuppression))
tf_export("raw_ops.BatchedNonMaxSuppression")(BatchedNonMaxSuppression)


def batched_non_max_suppression_eager_fallback(boxes, scores, box_counts, max_output_size, iou_threshold, score_threshold, pad_to_max_output_size=False, algorithm=-1, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  This is for function batched_non_max_suppression
  """
  _ctx = ctx if ctx else _context.context()
  if pad_to_max_output_size is None:
    pad_to_max_output_size = False
  ...
  if algorithm is None:
    algorithm = -1
  algorithm = _execute.make_int(algorithm, "algorithm")
  _attr_T, _inputs_T = _execute.args_to_matching_eager([boxes, scores], _ctx)
  (boxes, scores) = _inputs_T
  _attr_T_threshold, _inputs_T_threshold = _execute.args_to_matching_eager([iou_threshold, score_threshold], _ctx)
  (iou_threshold, score_threshold) = _inputs_T_threshold
  box_counts = _ops.convert_to_tensor(box_counts, _dtypes.int32)
  max_output_size = _ops.convert_to_tensor(max_output_size, _dtypes.int32)
  _inputs_flat = [boxes, scores, box_counts, max_output_size, iou_threshold, score_threshold]
  _attrs = ("T", _attr_T, "T_threshold", _attr_T_threshold,
  "pad_to_max_output_size", pad_to_max_output_size, "algorithm", algorithm)
  _result = _execute.execute(b"BatchedNonMaxSuppression", 2,
...
_generate_bounding_box_proposals_outputs = ["rois", "roi_probabilities"]
_GenerateBoundingBoxProposalsOutput = _collections.namedtuple(
    "GenerateBoundingBoxProposals", _generate_bounding_box_proposals_outputs)
...
def generate_bounding_box_proposals(scores, bbox_deltas, image_info, anchors, nms_threshold, pre_nms_topn, min_size, post_nms_topn=300, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "GenerateBoundingBoxProposals", name, _ctx.post_execution_callbacks,
        scores, bbox_deltas, image_info, anchors, nms_threshold, pre_nms_topn,
        min_size, "post_nms_topn", post_nms_topn)
  ...
  _result = _op.outputs[:]
  _inputs_flat = _op.inputs
  _attrs = ("post_nms_topn", _op.get_attr("post_nms_topn"))
  _execute.record_gradient(
      "GenerateBoundingBoxProposals", _inputs_flat, _attrs, _result, name)
  _result = _GenerateBoundingBoxProposalsOutput._make(_result)
  return _result

def GenerateBoundingBoxProposals(scores, bbox_deltas, image_info, anchors, nms_threshold, pre_nms_topn, min_size, post_nms_topn=300, name=None):
  return generate_bounding_box_proposals(scores=scores, bbox_deltas=bbox_deltas, image_info=image_info, anchors=anchors, nms_threshold=nms_threshold, pre_nms_topn=pre_nms_topn, min_size=min_size, post_nms_topn=post_nms_topn, name=name)
GenerateBoundingBoxProposals.__doc__ = generate_bounding_box_proposals.__doc__
GenerateBoundingBoxProposals = _doc_controls.do_not_generate_docs(_kwarg_only(GenerateBoundingBoxProposals))
tf_export("raw_ops.GenerateBoundingBoxProposals")(GenerateBoundingBoxProposals)


def generate_bounding_box_proposals_eager_fallback(scores, bbox_deltas, image_info, anchors, nms_threshold, pre_nms_topn, min_size, post_nms_topn=300, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  This is for function generate_bounding_box_proposals
  """
  _ctx = ctx if ctx else _context.context()
  if post_nms_topn is None:
    post_nms_topn = 300
