        ...
        if from_path not in black_list_dir:
            if os.path.isdir(from_file_path):
                ...
                if from_file_path.__contains__("/res/values"):
                    if is_allow:
                        merge_project_diff.merge_xml_diff.merge_diff(from_file_path, to_file_path)
                    else:
                        merge_project_diff.merge_xml_diff.merge_diff_attrs(from_file_path, to_file_path, target_project_path)
                if from_file_path.__contains__("AndroidManifest"):
                    if not is_allow:
                        merge_project_diff.manifest_diff.merge_manifest_diff(to_file_path, from_file_path)
