...
def compile_cuda_code(folder, cuda_filename, lib_name, *extra_compile_args, additional_cuda_files=None):
    ...
    # this check is only done if --lazy-cuda-compilation is given to setup.py
    # we then check if the shared library file exists and if it was modified after the source file
    # if the argument --force is given to setup.py, the CUDA code is always recompiled
    if "--lazy-cuda-compilation" in sys.argv and "--force" not in sys.argv and os.path.isfile(output_filename) and shared_lib_latest_version:
