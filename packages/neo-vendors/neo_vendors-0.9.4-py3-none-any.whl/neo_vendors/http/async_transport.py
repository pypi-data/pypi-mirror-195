...
class AsyncHttpProvider:
    ...
    async def make_http(
        ...
        if request_body is not None and not method.has_request_body_support():
            raise ValueError(f"request body is not supported for http method = {http_methods.HttpMethodsEnum.value}")
...
class RequestContextWrapper(types.SimpleNamespace):
    ...
    def __init__(self, trace_request_ctx: typing.Optional[types.SimpleNamespace] = None):
        self.context = request_context.RequestContext()
        super(RequestContextWrapper, self).__init__()


class AsyncRequestContexHandleTraceConfig(aiohttp.TraceConfig):
    request_context_handler: request_context.RequestContextHandlerProtocolAsync
    ...
    def __init__(
        ...
        self.request_context_handler = request_context_handler
    ...
    async def on_request_end_handler(
        ...
        await self.request_context_handler(trace_config_ctx.context)
