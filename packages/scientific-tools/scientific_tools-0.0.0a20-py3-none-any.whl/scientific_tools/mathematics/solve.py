...
def explicit_finite_difference_solver(Ctt:float, Ctx:float, Cxx:float,
    ...
    Initial conditions:
    U_t0 is the list of U values when t = min_t. len(U_t0) must be T.
    U_t1 is the list of U values when t = min_t + 1*step_t. len(U_t1) must be T.
    
    Limit conditions:
    U_min_x is the list of U values when x=min_x. len(U_min_x) must be X-2.
    U_max_x is the list of U values when x=max_x. len(U_max_x) must be X-2.
