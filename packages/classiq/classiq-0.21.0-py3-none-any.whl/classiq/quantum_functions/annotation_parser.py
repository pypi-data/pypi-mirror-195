...
class AnnotationParser:
    ...
    def is_valid_generic_alias_of_qreg(obj: GenericAlias) -> bool:
        is_fixed_qreg = issubclass(obj.__origin__, QSFixed) and not issubclass(
            obj.__origin__, QSInt
        )
        valid_qreg_annotation = (
            isinstance(obj, GenericAlias)
            and issubclass(obj.__origin__, QReg)
            and not is_fixed_qreg
        ...
        valid_qfixed_annotation = (
            isinstance(obj, GenericAlias)
            and is_fixed_qreg
            and type(obj.__args__) is tuple
            and len(obj.__args__) == 2
            and type(obj.__args__[0]) is int
            and type(obj.__args__[1]) is int
        )
        return valid_qreg_annotation or valid_qfixed_annotation
