...
ParametersExpressionSupportedNodeTypes = Union[
    # binary operation
    ast.BinOp,
    ast.BitOr,
    ast.BitAnd,
...
def validate_expression_str(name: str, expr_str: str) -> None:
    ...
    arithmetic_expression_parser.validate_expression(
        ...
        supported_nodes=ParametersExpressionSupportedNodeTypes,
    )
    ...
    if expr is None:
        raise ClassiqValueError(f"{name}'s value cannot be None")
...
class FunctionParams(HashablePydanticBaseModel):
    ...
    def is_field_gen_param(self, name: str) -> bool:
        ...
        # note that exec-params are also gen-params (but not vice versa!)
        return isinstance(f, ModelField) and (
            "is_exec_param" in f.field_info.extra
            or "is_gen_param" in f.field_info.extra
        )
