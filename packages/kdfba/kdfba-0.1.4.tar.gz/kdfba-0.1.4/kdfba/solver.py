    ...
    def __calculate_derivatives(self, t, states_value):  # 测试一下这个函数传入self会不会影响ode求解器求解，如果不会就好办了  12.15: 可以的
        ...
        for model_name in model_names:
            ...
            for dreaction in dreactions:
                var['flux'] = 0
                exec("var['flux'] = (" + dreaction.equation + ') * (model_temp.volume / model_temp.weight)')  ## ？？测试一下state里没有eqution对应物质的情况
                # print(var['flux'])
                ##  修改对应反应的反应上下限
            ...
            try:
                flux_values = flux_variability_analysis(model_temp, reaction_list = sub_objectives, fraction_of_optimum = model_temp.fraction_of_optimum, loopless = self.loopless)
            except OptimizationError:
                rows = len(sub_objectives)
                data = [[0]*2]*rows
                flux_values = pd.DataFrame(data, columns=['minimum','maximum'], index=sub_objectives_str, dtype=float)
            # print(flux_values)
            ...
            for i in range(0, len(sub_objectives)):
                ...
                var[model_name + '_' +sub_objectives[i].id] = flux_values.at[sub_objectives[i].id, sub_objective_directions[i]]
        ...
        for state_name in states_names:
            ...
            if state_name.startswith("x_"):  # 计算菌落生长率对应时间导数
                ...
                if model_mu in rxnsFluxValue_keys:  # 计算了这个菌的生长率, derivatives就不为0
                    ...
                    derivative['d'] = self.states[state_name] * var[model_mu] - self.states_var[state_name] * dilution_rate
