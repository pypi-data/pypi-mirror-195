...
from skipole import SectionData, ValidateError, FailPage, ServerError, GoTo

from .... import editpage, editfolder, fromjson, utils
...
def retrieve_edited_folder(skicall):
    ...
    info = utils.item_info(editedprojname, folder_number)
    ...
    if info.itemnumber == 0:
        ...
        parent = utils.item_info(info.project, info.parentfolder_number)
    ...
    default_page_name, page_names = utils.folder_page_names(info.project, info.itemnumber)
...
def move_to_folder(skicall):
    ...
    # call function in utils.editfolder
    try:
...
def choose_edit_action(skicall):
    ...
    if action.startswith('edit_folder_'):
        ...
        try:
            ...
            pinfo = utils.page_info(project, itemnumber)
        except Exception:
...
def choose_remove_action(skicall):
    ...
    try:
        ...
        project = get_field_list[-2]
        itemnumber = int(get_field_list[-1])
        info = utils.item_info(project, itemnumber)
    except Exception:
...
def copy_page(skicall):
    ...
    pd['pastepage', 'input_text'] = str(utils.next_ident_number(project))
...
def paste_page(skicall):
    ...
    if ('pastepage', 'input_text') not in call_data:
        pagenumber = utils.next_ident_number(project)
    else:
        try:
    ...
    item = utils.item_info(project, pagenumber)
    if item is not None:
        raise FailPage(message = "This page number is already used (%s)" % (item.path,), widget='pastepage')
    # get pages in the folder
    default_page_name, page_names = utils.folder_page_names(project, itemnumber)
...
def _retrieve_new_copypage(skicall):
    ...
    parent_url = utils.page_path(editedprojname, call_data['InFolder'])
...
def delete_item(skicall):
    ...
    if info.item_type == 'Folder':
        # call utils.editfolder.delete_folder
    ...
    # call utils.editpage.delete_page
...
def submit_rename_folder(skicall):
    ...
    # Rename the folder
    # call utils.editfolder.rename_folder which returns a new fchange
    try:
...
def submit_folder_brief(skicall):
    ...
    # call utils.editfolder.folder_description which returns a new fchange
    try:
...
def recursivedelete(skicall):
    ...
    folderinfo = utils.folder_info(editedprojname, folder_number)
...
def _foldertree(projectname, foldernumber):
    ...
    finfo = utils.folder_info(projectname, foldernumber)
    folder_path = utils.item_info(projectname, foldernumber).path
...
def _show_pages(contents, projectname, foldernumber, dragrows, droprows, indent):
    ...
    for pinfo in utils.pages(projectname, foldernumber):
...
def _show_folders(contents, projectname, foldernumber, dragrows, droprows, indent):
    ...
    for finfo in utils.folders(projectname, foldernumber):
