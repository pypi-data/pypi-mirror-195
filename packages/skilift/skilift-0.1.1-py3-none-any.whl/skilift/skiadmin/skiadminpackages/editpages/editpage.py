...
from .... import fromjson, editpage, utils

from ....utils import part_contents
...
def set_html_lang(skicall):
    ...
    # call utils.editpage.page_language which returns a new pchange
    try:
...
def move_up_in_page_dom(skicall):
    ...
    part_tuple = utils.part_info(editedprojname, pagenumber, None, location)
...
def move_up_right_in_page_dom(skicall):
    ...
    new_parent_tuple = utils.part_info(editedprojname, pagenumber, None, new_parent_location)
...
def move_in_page_dom(skicall):
    ...
    # get part_tuple from project, pagenumber, section_name, location
    part_to_move_tuple = utils.part_info(editedprojname, pagenumber, None, location_to_move)
    ...
    # get part_tuple from project, pagenumber, section_name, location
    target_part_tuple = utils.part_info(editedprojname, pagenumber, None, target_location)
...
def edit_page_dom(skicall):
    ...
    if len(location_list) == 1:
        ...
        call_data['part_tuple'] = utils.part_info(editedprojname, pagenumber, None, [location_list[0], None, ()])
        raise GoTo(target = 53007, clear_submitted=True)

    # utils.part_info requires a location which is a tuple or list consisting of three items:
    ...
    part_tuple = utils.part_info(editedprojname, pagenumber, None, [location_string, None, location_integers])
    if part_tuple is None:
