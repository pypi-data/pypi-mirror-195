...
def confidence_intervals_of_the_mean(df):
    ...
    for confidence_level in confidence_levels:
        ...
        series = {}
        for column in df:
            # Workaround dealing with the new parameter name in scipy.stats.t.interval()
...
def harmonic_mean(df):
    ...
    for column in df:
        try:
            if 'nan_policy' in inspect.signature(scipy.stats.hmean).parameters:
                res[column] = scipy.stats.hmean(df[column], nan_policy='omit')
            else:
                # workaround for the fact that nan_policy was only introduced in scipy 1.9.0
                # (released July 2022)
                res[column] = scipy.stats.hmean(
                    ut.utilities.remove_nans_from_array(df[column])
                )  # pragma: no cover
...
def geometric_mean(df):
    res = {}
    for column in df:
        if 'nan_policy' in inspect.signature(scipy.stats.gmean).parameters:
            res[column] = scipy.stats.gmean(df[column], nan_policy='omit')
        else:
            # workaround for the fact that nan_policy was only introduced in scipy 1.9.0
            # (released July 2022)
            res[column] = scipy.stats.gmean(
                ut.utilities.remove_nans_from_array(df[column])
            )  # pragma: no cover
