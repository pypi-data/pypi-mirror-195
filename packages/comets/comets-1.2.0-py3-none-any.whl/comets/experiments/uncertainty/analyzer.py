...
class OnlineStatAnalyzer:
    ...
    def update(self, sample):
        ...
        # Convert the sample to a ParameterSet retaining only numeric data
        sample = ParameterSet.to_dataframe(sample, keep_numeric_only=True)
        # Transform to an 1-D numpy array
        samplenp = sample.iloc[0].to_numpy()
    ...
    def _create_empty_stats_df(sample, list_of_statistics, fill_value=np.nan):
        return pd.DataFrame(
            data=np.full((sample.size, len(list_of_statistics)), fill_value),
    ...
    def _conf_int_mean(self, confidence):
        ...
        # The arguments of scipy.stats.t.interval are constructed in this
        # way because there has been a change in the keyword name in
        # scipy.stats.t.interval() -- this was previously 'alpha', and is now
        # 'confidence' (this was changed in scipy 1.9.0, July 2022, so this
        # construction can be removed once support for older versions is dropped).
        # NOTE: this construction is also repeated twice in descriptivestats.py
        confidence_key_for_t_interval = (
            'alpha'
            if 'alpha' in inspect.signature(scipy.stats.t.interval).parameters
            else 'confidence'
        )
        kwargs_for_t_interval = {
            confidence_key_for_t_interval: confidence,
            'df': self.count() - 1,
            'loc': self.mean(),
            'scale': self.sem(),
        }
