...
class IOTModbus(IOTDriver):
    ...
    def read(self, **kwargs):
        ...
        for name in names:
            ...
            if point:
                ...
                data_order = point.get('point_data_order', 0)
                if device_address is not None and slave_id is not None and fun_code is not None and address is not None and data_type is not None and data_length is not None:
                    value = self.get_value(datas, name, device_address, slave_id, fun_code, address, data_length, data_type, data_order)
    ...
    def get_value(self, results: dict, name: str, address: str, slave_id: int, fun_code: int, start: int, length: int, type: int, order: int):
        try:
            values = []
            pos = 0
