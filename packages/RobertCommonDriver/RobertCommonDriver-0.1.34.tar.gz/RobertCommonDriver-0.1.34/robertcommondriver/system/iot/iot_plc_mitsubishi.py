...
class IOTPlcMitsubishi(IOTDriver):
    ...
    def read(self, **kwargs):
        ...
        for name in names:
            ...
            if point:
                ...
                type = point.get('point_type')
                if device_address is not None and address is not None and type is not None:
                    value = self._get_value(datas, name, device_address, address, type)
    ...
    def _send_read_cmd(self, address: str, results: dict, mitsubishi_list: list):
        try:
                                ...
                                if item.is_success:
                                    if item.contents[9] is True:
                                        if address not in results.keys():
                                            results[address] = {}
                                        results[address][f"{item.contents[0]}_{item.contents[7]}"] = item.contents[8]
                                    else:
                                        logging.error(f"plc({address}) read({item.contents[0]}) fail({item.contents[10]})")
                                else:
                                    logging.error(f"plc({address}) read({item.contents[0]}) fail({item.msg})")
                    else:
                        raise Exception(f"{result.msg}")
        except Exception as e:
    ...
    def _get_value(self, results: dict, name: str, device_address: str, address: str, type: int):
        try:
            return results.get(device_address, {}).get(f"{address}_{type}")
        except Exception as e:
            logging.error(f"get value({name}) fail({e.__str__()})")
        return None

    def _send_write_cmd(self, address: str, results: dict, mitsubishi_list: list):
        try:
            if self._get_client(address) is not None and len(mitsubishi_list) > 0:
                            ...
                            if isinstance(item, IOTBaseCommon.IOTNetResult):
                                if item.is_success:
                                    if item.contents[9] is True:
                                        if address not in results.keys():
                                            results[address] = {}
                                        results[address][f"{item.contents[0]}_{item.contents[7]}"] = item.contents[9]
                                    else:
                                        logging.error(f"plc({address}) write({item.contents[0]}) fail({item.contents[10]})")
                                else:
                                    logging.error(f"plc({address}) write({item.contents[0]}) fail({item.msg})")
                    else:
                        raise Exception(f"{result.msg}")
        except Exception as e:
            logging.error(f"plc({address}) write fail({e.__str__()})")
    ...
    def _write_address(self, *args, **kwargs) -> dict:
        ...
        units = IOTBaseCommon.chunk_list(mitsubishis, self.configs.get('multi_read'))
        for unit in units:
            self._send_write_cmd(address, results, unit)
            self.delay(self.configs.get('cmd_interval', 0.3))
        return results
