...
class IOTPlcAllenBradley(IOTDriver):
    ...
    def _send_read_cmd(self, address: str, results: dict, ab_item_list: list):
        try:
                                ...
                                if item.is_success:
                                    if item.contents[9] is True:
                                        if address not in results.keys():
                                            results[address] = {}
                                        results[address][f"{item.contents[0]}_{item.contents[2]}"] = item.contents[8]
                                    else:
    ...
    def _send_write_cmd(self, address: str, results: dict, ab_item_list: list):
        try:
            if self._get_client(address) is not None and len(ab_item_list) > 0:
                            ...
                            if isinstance(item, IOTBaseCommon.IOTNetResult):
                                if item.is_success:
                                    if item.contents[9] is True:
                                        if address not in results.keys():
                                            results[address] = {}
                                        results[address][f"{item.contents[0]}_{item.contents[2]}"] = item.contents[9]
                                    else:
                                        logging.error(
