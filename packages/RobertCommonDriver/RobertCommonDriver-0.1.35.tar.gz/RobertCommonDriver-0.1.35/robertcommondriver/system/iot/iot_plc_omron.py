...
class IOTPlcOmronFins(IOTDriver):
    ...
    def _send_read_cmd(self, address: str, results: dict, omron_item_list: list):
        try:
    ...
    def _send_write_cmd(self, address: str, results: dict, omron_item_list: list):
        try:
            if self._get_client(address) is not None and len(omron_item_list) > 0:
                            ...
                            if isinstance(item, IOTBaseCommon.IOTNetResult):
                                if item.is_success:
                                    if item.contents[9] is True:
                                        ...
                                        logging.error(
                                            f"plc({address}) write({item.contents[0]}) fail({item.contents[10]})")
                                else:
