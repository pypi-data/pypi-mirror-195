...
class BacnetClient:
    ...
    def read(self, target_address: str, objct_propertys: list, max_per_request: Optional[int] = None, use_read_multiple: bool = True, ping_check: bool = False):
        self.logging(content=f"read({target_address}-{len(objct_propertys)}-{max_per_request}-{use_read_multiple})")
        target_address, device_id, bind_ip = self._parse_adderss(target_address)
    ...
    def _send_request(self, request, wait_result: bool = True, context: Optional[IOCBContext] = None):
        ...
        if self._get_application():
            ...
            self.logging(content=f"send({self.process}) {request.apduInvokeID} {request.__class__.__name__}")
    ...
    def _parse_response(self, request, iocb):
        if iocb is not None:
            ...
            self.logging(content=f"recv({self.process}) {apduInvokeID} {results.__class__.__name__}")
    ...
    def _create_object(self, object: IOTSimulateObject):
        try:
            ...
            logging.error(f"bacnet({self}) create object({object.get('point_type')} {object.get('point_address')}) fail({e.__str__()})")
    ...
    def refresh_objects(self, objects: List[IOTSimulateObject]):
        for object in objects:
            ...
            try:
                ...
                logging.error(f"bacnet({self}) refresh object({object.get('point_type')} {object.get('point_address')}) fail({e.__str__()})")
...
class IOTBacnet(IOTDriver):
    ...
    def read(self, **kwargs):
        ...
        for name in names:
            ...
            if point:
                ...
                logging.error(f"bacnet({self.configs.get('address')}) read point({name}) fail(UnExist)")
    ...
    def _get_value(self, name: str, target_address: str, object_type: str, instance_number: int, point_property: str):
        if self._get_client():
            ...
            if quality == 'Good':
                ...
                logging.error(f"bacnet({self.configs.get('address')}) read point({name}) fail({value})")
        return None
    ...
    def _write(self, name: str, target_address: str, object_type: str, instance_number: int, property_name: str, value):
        try:
            ...
            logging.error(f"bacnet({self.configs.get('address')}) write point({name}) fail({e.__str__()})")
        return False
