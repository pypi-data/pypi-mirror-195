...
from .crystal import SpinHamiltonian, read_params, zeeman_hamiltonian, \
    HARTREE2INVCM, isotropic_susceptibility, print_basis, SPIN_SYMBOLS, \
    ANGM_SYMBOLS, make_operator_storage, MagneticSusceptibilityFromFile, \
    EPRGtensorFromFile
...
def model_func(args, unknown_args):
    ...
    spin = np.sum([args.model_space.get_op(op, **args.coupling)
                   for op in args.model_space.coupling if op in SPIN_SYMBOLS],
                  axis=0)
    print(args.model_space.coupling)

    if spin.size == 1:
        warn("No spin momenta present assuming S=0!")

    angm = np.sum([args.model_space.get_op(op, **args.coupling)
                   for op in args.model_space.coupling if op in ANGM_SYMBOLS],
                  axis=0)
    ...
    def trafo(op):
        if args.quax:  # compute basis change from basis rotation
