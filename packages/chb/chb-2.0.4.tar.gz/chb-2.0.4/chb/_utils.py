...
class Timer:
    ...
    @staticmethod
    def datebetween(string1, string2, format_str='%Y-%m-%d %H:%M:%S'):
        """
        求两个
        :param date_:  datatime类 实例对象
        :param format_str: 字符型， string参数的时间格式
        :return:  整型时间戳
        """
        return datetime.datetime.strptime(string1, '%Y-%m-%d %H:%M:%S') - datetime.datetime.strptime(string2, '%Y-%m-%d %H:%M:%S')
...
class Open:
    ...
    def readlines(self, num_lines=10000):
        while True:
            lines = self.file_handle.readlines(num_lines)
            if not lines:
                break
            yield lines
