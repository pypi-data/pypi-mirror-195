...
class Api42:
    ...
    def __init__(self, requests_per_second: int = 4, log_lvl: str = 'WARNING',
        ...
        if not self.__log_lvl:
            ...
            self.log: logging.Logger = self.__get_logger()
            self.log.warning(
        ...
        if self.__requests_per_second < 1:
            msg: str = 'requests_per_second: '
            msg += f'invalid value {requests_per_second}, defaulting to 4'
            self.log.warning(msg)
            self.__requests_per_second = 4
        # logging Token
        self.log.debug(f'{self.__headers}')
    ...
    def log_lvl(self, lvl: str) -> None:
        ...
        Args:
            lvl (str): The log level to set.
        """
        # Setter for log level.
        if lvl not in set(config.log_lvls.keys()):
            # If given log level is not in config.log_lvls, log error and raise
            # exception if self.__raises is True.
            self.log.error(f'{lvl}: invalid value for log_lvl')
            if self.__raises:
        ...
        self.log.setLevel(self.__log_lvl)
    ...
    def requests_per_second(self, rps: int) -> None:
        """Sets the maximum number of requests per second to make to the 42API.
        ...
        """
        # Setter for self.__requests_per_second
        if rps < 1:
            # If given requests per second is less than 1, an exception is
            # raised if self.__raises is True.
            self.log.error(f'{rps}: invalid value for requests_per_second')
            if self.__raises:
                raise ValueError(
                    f'{rps}: invalid value for requests_per_second')
            return
    ...
    def get_token_info(self) -> dict:
        ...
        try:
            ...
            self.log.error(f'GET {resp.url} status: {resp.status_code}')
            if self.__raises:
    ...
    def get(self, url: str, params: dict = {}, handle_pagination: bool = True,
        ...
        self.log.debug(f'GET {resp.url} status: {resp.status_code}')
    ...
    def post(self, url: str, data: dict = {}) -> requests.Response:
        ...
        try:
            ...
            # Log the error and optionally raise an exception
            self.log.error(f'POST {resp.url} status: {resp.status_code}')
            if self.__raises:
                raise Api42RequestError(f'Failed: {e}') from e
            return []

        # Log the successful request
        self.log.debug(f'POST {resp.url} status: {resp.status_code}')
    ...
    def patch(self, url: str, data: dict = {}) -> requests.Response:
        ...
        try:
            ...
            # Log the error and optionally raise an exception
            self.log.error(f'PATCH {resp.url} status: {resp.status_code}')
            if self.__raises:
                raise Api42RequestError(f'Failed: {e}') from e
            return []

        # Log the successful request
        self.log.debug(f'PATCH {resp.url} status: {resp.status_code}')
    ...
    def put(self, url: str, data: dict = {}) -> requests.Response:
        ...
        try:
            ...
            # Log the error and optionally raise an exception
            self.log.error(f'PUT {resp.url} status: {resp.status_code}')
            if self.__raises:
                raise Api42RequestError(f'Failed: {e}') from e
            return []

        # Log the successful request
        self.log.debug(f'PUT {resp.url} status: {resp.status_code}')
    ...
    def delete(self, url: str, data: dict = {}) -> requests.Response:
        ...
        try:
            ...
            # Log the error and optionally raise an exception
            self.log.error(f'DELETE {resp.url} status: {resp.status_code}')
            if self.__raises:
                raise Api42RequestError(f'Failed: {e}') from e
            return []

        # Log the successful request
        self.log.debug(f'DELETE {resp.url} status: {resp.status_code}')
    ...
    def mass_request(self, method: str, requests: list,
       ...
       a list of requests.
        ...
        if not func:
            self.log.error(f'Method unrecognized: {method}')
    ...
    def handler(func):
        ...
        Returns:
            dict: The response content in JSON format, or an empty dictionary
                if there is no content.
        """

        def __handle(self, request: Api42Request):
            self.__refresh_token()
            resp: requests.Response = func(self, request)
    ...
    def __get(self, req: Api42Request) -> requests.Response:
        ...
        # Log the successful request
        self.log.debug(
            f'GET {resp.url} status: {resp.status_code}\n\t{req.params}')
        return resp
    ...
    def __patch(self, req: Api42Request) -> requests.Response:
        ...
        try:
            ...
            # Log the error and optionally raise an exception
            self.log.error(f'PACTH {resp.url} status: {resp.status_code}')
            if self.__raises:
                raise Api42RequestError(f'Failed: {e}') from e
            return resp
