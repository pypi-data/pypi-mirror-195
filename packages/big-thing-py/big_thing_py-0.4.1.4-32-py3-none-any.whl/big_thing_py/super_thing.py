...
class SoPZeroMQClient:
    def __init__(self, name: str, host: str, port: int, ipc_pipe: str = '') -> None:
        self.name = name
        self.host = host
        self.port = port
        ...
        # self.socket_send = self.context_send.socket(zmq.DEALER)
        self.socket_recv = self.context_recv.socket(zmq.DEALER)
        # self.socket_send.identity = self.name.encode()
        self.socket_recv.identity = self.name.encode()
    ...
    def connect(self):
        if self.ipc_pipe:
            # rc = self.socket_send.connect(f'ipc://{self.ipc_pipe}_recv')
            rc = self.socket_recv.connect(f'ipc://{self.ipc_pipe}_send')
        else:
            # self.socket_send.connect(f'tcp://{self.host}:{self.port}_recv')
            self.socket_recv.connect(f'tcp://{self.host}:{self.port}_send')
        SOPLOG_DEBUG(f'[ZMQ CONNECT] connect to ipc://{self.ipc_pipe}')

    def disconnect(self):
        if self.ipc_pipe:
            # self.socket_send.disconnect(f'ipc://{self.ipc_pipe}_recv')
            self.socket_recv.disconnect(f'ipc://{self.ipc_pipe}_send')
        else:
            # self.socket_send.disconnect(f'tcp://{self.host}:{self.port}_recv')
            self.socket_recv.disconnect(f'tcp://{self.host}:{self.port}_send')
        SOPLOG_DEBUG(f'[ZMQ DISCONNECT] disconnect from ipc://{self.ipc_pipe}')

    def publish(self, topic: str, payload: str):
        self.socket_recv.send_multipart(
    ...
    def receive(self) -> Tuple[str, str]:
        try:
            ...
            SOPLOG_DEBUG(
                f"[DEBUG ZMQ Receive] topic: {topic}, payload: {payload}")
        except Exception as e:
            print_error(e)
            SOPLOG_DEBUG(
                f"[DEBUG ZMQ Receive] msg: {msg}")

        try:
            topic_decode = topic.decode().strip()
            payload_decode = payload.decode().strip()
        except UnicodeDecodeError as e:
            try:
                topic_decode = topic[8:].decode().strip()
                payload_decode = payload[8:].decode().strip()
            except UnicodeDecodeError as e:
                print_error(e)
                SOPLOG_DEBUG(
                    f'[ZMQ Receive Error] Received message - topic: {topic}, payload: {payload}', 'red')
                raise e

        return topic_decode, payload_decode
...
class SoPSuperThing(SoPBigThing):
    ...
    def __init__(self, name: str, service_list: List[SoPService], alive_cycle: float, is_super: bool = False, is_parallel: bool = True,
        ...
        self._zmq_client = SoPZeroMQClient(
            name=self._name, host=self._ip, port=self._port, ipc_pipe='/mnt/ramdisk/super_ipc')
        ...
        self._thread_func_list += [
            ...
            self._listen_zmq_message_thread_func,]
    ...
    def _listen_zmq_message_thread_func(self, stop_event: Event):
        try:
            while not stop_event.wait(THREAD_TIME_OUT):
                topic, payload = self._zmq_receive()
                if topic:
                    self._receive_queue.put(
                        encode_MQTT_message(topic=topic, payload=payload))
        except Exception as e:
            stop_event.set()
            print_error(e)
            return False
    ...
    def _publish_message_thread_func(self, stop_event: Event):
        try:
            while not stop_event.wait(THREAD_TIME_OUT):
                ...
                if 'SM' in topic:
                    self._zmq_publish(topic, payload)
                    if 'SM/RESULT' in topic:
    ...
    # override
    def _zmq_subscribe(self, topic: str):
        self._zmq_client.subscribe(topic)
        self._zmq_client.subscribe_list.add(topic)

    # override
    def _zmq_unsubscribe(self, topic: str):
        self._zmq_client.unsubscribe(topic)
        self._zmq_client.subscribe_list.remove(topic)

    def _zmq_publish(self, topic: str, payload: str):
        self._zmq_client.publish(topic, payload)
        SOPLOG_DEBUG(f'==== [ZMQ Publish] ==== {self._name}', 'yellow')

        self._print_packet(topic, payload, mode=self._log_mode)

    def _zmq_receive(self) -> Tuple[str, str]:
        topic, payload = self._zmq_client.receive()
        SOPLOG_DEBUG(f'==== [ZMQ Receive] ==== {self._name}')
