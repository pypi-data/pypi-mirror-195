...
def select_mouse_and_operator_input(state_globals) -> None:
    ...
    try:
        ...
        logger.warning(
            "LIMS | Mouse %r could not be fetched from database: %r", mouse_input, exc
        )
    ...
    try:
        ...
        if npxc.is_mouse_in_lims is None:
            logger.warning(
                "LIMS | Mouse %r could not be fetched from database: %r", mouse_input
            )
        npxc.mouse = mouse_input
        npxc.is_mouse_in_lims = False
    else:
        ...
        npxc.set(
            state_globals, project=str(project), is_mouse_in_lims=npxc.is_mouse_in_lims
        )
    ...
    try:
        ...
        logger.warning(
            "LIMS | Operator %r could not be fetched from database: %r",
            operator_input,
            exc,
        )
    ...
    transition_result = (
        False if not npxc.is_mouse_in_lims else True
    )  # None isn't allowed in s_g['external']
    npxc.set(
        state_globals,
...
def select_experiment_input(state_globals) -> None:
    ...
    if not isinstance(npxc.experiment, Experiment):
        raise ValueError(
            f"Experiment {npxc.experiment!r} was not converted to a valid Experiment class"
        )
