...
class WithLims(abc.ABC):
    ...
    def photodoc(self, img_label: Optional[str] = None) -> pathlib.Path:
        ...
        if img_label and self.imager.data_files:
            views = 2 if isinstance(self.imager, np_services.Cam3d) else 1
            def files_with_label():
                return sorted([_ for _ in self.imager.data_files if img_label in _.name])
            while len(files_with_label()) > views:
                self.imager.data_files.remove(files_with_label()[0])
