...
"""Association inside the ARG: Tests all bitsets or random bitsets by mutation rate
...
`--sampling_rate` can function in 3 modes. If > 0, it uses that value as
a mutation rate. If = 0, it tests all branches / bitsets in the ARG for
association. If < 0, it uses the passed value to generate a range of
mutation rates for testing.
