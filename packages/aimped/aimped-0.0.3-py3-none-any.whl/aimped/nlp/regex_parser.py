...
def RegexNerParser(path, text):
    ...
    Args:
        path (list): json_regex file paths list.
        text (str): string text that comes from request object.
    Returns:
        list[dict[str, int]]: all the chunks with labels in gold standard
    """
...
def RegexModelNerChunkResults(regex_json_files_path_list, model_results,text):
    """
    Merges all the results from regex_json_files and model results.
    """
