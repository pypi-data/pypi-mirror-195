...
def generate_eog_template(eog_data: np.array = None,
    ...
    positive_peaks, _ = signal.find_peaks((eog_data * unitary_step(eog_data - _amp_threshold)).squeeze(),
                                          prominence=_amp_threshold.to_value(eog_data.unit),
                                          distance=int(fs * minimum_interval_width),
                                          width=int(fs * eog_peak_width))
    negative_peaks, _ = signal.find_peaks((-eog_data.value * unitary_step(-eog_data - _amp_threshold)).squeeze(),
                                          prominence=_amp_threshold.to_value(eog_data.unit),
                                          distance=int(fs * minimum_interval_width),
                                          width=int(fs * eog_peak_width))
