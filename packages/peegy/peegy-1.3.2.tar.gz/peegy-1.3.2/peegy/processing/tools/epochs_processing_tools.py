...
def et_subtract_oeg_template(data: np.array = np.array([]),
    ...
    for _idx_eog_ch in range(ref.shape[1]):
        ...
        reconstructed = reconstruct_eog(data=output_data,
                                        template=template,
...
def et_apply_spatial_filtering(z=np.array([]),
    ...
    print('DSS keeping {:} out of {:} components accounting for {:}% of the total variance'.format(
        n_components.size,
        pwr0.size,
        np.round(np.sum(p_ratio[n_components]) / np.sum(p_ratio) * 100, decimals=2)
        ))
