...
class CreateAndApplySpatialFilter(InputOutputProcess):
    ...
    def plot_component_power(self):
        ...
        ax.plot(self.pwr0, marker='o', label='Unbiased')
        ax.plot(self.pwr1, marker='o', label='Biased')
        ax.set_xlabel('Component rank')
        ax.legend(loc="upper right")
        ax.set_title('Power')

        ax = plt.subplot(gs[0, 1])
        ax.plot(self.p_ratio / np.sum(self.p_ratio), marker='o', label='Single')
        ax.plot(np.cumsum(self.p_ratio / np.sum(self.p_ratio)), marker='o', label='Accumulated')
        ax.axhline(self.sf_thr, color='black', label='Threshold')
        ax.set_ylim([0, 1])
        ax.set_xlabel('Component rank')
        ax.set_title('Biased_power / Unbiased_power')
        ax.legend(loc="upper right")
