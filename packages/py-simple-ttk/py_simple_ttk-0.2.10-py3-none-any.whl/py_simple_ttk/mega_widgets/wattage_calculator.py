...
class WattageCalculator(ttk.Frame):
    def __init__(self, parent: ttk.Frame):
        ...
        amperage_entry = LabeledEntry(
            self, labeltext="Amperage (A): ", on_keystroke=True, command=calc
        )
        default_pack(amperage_entry, padx=10)
        voltage_entry = LabeledEntry(
            self, labeltext="Voltage (V): ", on_keystroke=True, command=calc
        )
        default_pack(voltage_entry, padx=10)
        ttk.Label(
            self,
            text="Wattage (Rounded to multiple of 5): ",
            justify=tk.CENTER,
            anchor=tk.CENTER,
        ).pack(side=tk.TOP, fill="x", pady=(10, 0))
        (output := CopyBox(self, height=1)).pack(
            side=tk.TOP, pady=(0, 10), expand=False, fill="x", padx=10
        )
        output.disable()
