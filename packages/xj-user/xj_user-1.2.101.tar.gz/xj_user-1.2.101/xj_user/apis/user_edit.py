...
class UserEdit(generics.UpdateAPIView):  # 或继承(APIView)
    ...
    def post(self, request, *args, user_info=None, request_params=None, **kwargs):
        ...
        try:
            ...
            user_id = self.params.pop("user_id", None) or user_info.get("user_id")
            t_user = BaseInfo.objects.filter(id=user_id).first()
            if not t_user:
                return response.Response({
                    'err': 1002,
                    'msg': '找不到user_id为' + str(user_id) + "用户"
                })
