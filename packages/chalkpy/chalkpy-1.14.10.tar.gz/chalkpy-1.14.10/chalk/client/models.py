...
class QueryStatus(int, Enum):
    PENDING_SUBMISSION = 1  # Pending submission to the db
    SUBMITTED = 2  # Submitted to the db, but not yet running
    RUNNING = 3  # Running in the db
    ERROR = 4  # Error with either submitting or running the job
    EXPIRED = 5  # The job did not complete before an expiration deadline, so there are no results
    CANCELLED = 6  # Manually cancelled before it errored or finished successfully
    SUCCESSFUL = 7  # Successfully ran the job. Output is available at `output_uri`
