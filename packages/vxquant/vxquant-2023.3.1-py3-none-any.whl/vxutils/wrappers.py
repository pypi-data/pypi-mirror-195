...
class vxWrapper:
    ...
    def init_by_config(config: dict):
        ...
        if (
            not isinstance(config, (Mapping, "vxDict", "vxContext"))
            or "class" not in config
        ):
            return config
...
class vxAPIWrappers:
    ...
    # def __setitem__(self, key: str, providers: Any) -> None:
    #    self.register_providers(**{key: providers})
    ...
    def register_providers(self, **providers):
        ...
        for name, provider in providers.items():
            ...
            if name in self.__dict__:
                logger.warning("providers({name}) 已注册为: {self.__dict__[name]},忽略更新")
                continue
            ...
            try:
                ...
                logger.error(f"加载provider: {name}({provider})出错: {err}", exc_info=True)
