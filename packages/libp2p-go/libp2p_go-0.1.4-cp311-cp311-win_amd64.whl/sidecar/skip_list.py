...
class SkiplistAbstractBase:
    ...
    def _scan(self, key, key_compare: Callable[[Any, Any], bool] = None) -> tuple[Any, Any]:
        return_value = None
        ...
        for level in reversed(range(height)):
            ...
            if key_compare and key_compare(node.key, key):
                return_value = node
            elif not key_compare and node.key == key:
                return_value = node
            else:
                prevs[level] = node
    ...
    def _remove(self, key):
        ...
        with update.lock():
            for level in range(len(node.nxt)):
                if len(update) <= level:
                    continue
                if update[level].nxt[level] != node:
                    continue
...
class Skiplist(SkiplistAbstractBase, MutableMapping):
    ...
    def scan_by_compare(self, key, key_compare: Callable[[Any, Any], bool]):
        node, _ = self._scan(key, key_compare=key_compare)
        if node is not None:
            return node.data
        return None
