...
from .utils import np, xr, get_mask, MaskData
...
from shapefile import Reader as pyshp_Reader
...
class map:
    ...
    def add_mask(self, *args, reverse=False):
    ...
    def __make_args_masked(self, arg_list):
        if self.metamask is None:
            ...
            if np.array_equal(metax, x) & np.array_equal(metay, y):
                ...
                mask = np.array(self.metamask.extent(x, y))
    ...
    def __add_polygon(self, pts, **kwargs):
        self.ax.add_patch(Polygon(pts, **kwargs))
    ...
    @plt._copy_docstring_and_deprecators(plt.figure)
    ...
    def load(self, name=None, **kwargs):
        """
        ...
        and so on.
        """
        name_list.update(dict(china="china_country", provinces="cnhimap",
                              river="rivers", tp="TPBoundary_2500"))
        file = os.path.dirname(__file__) + f"/shapefiles/{name_list[name]}.shp"
