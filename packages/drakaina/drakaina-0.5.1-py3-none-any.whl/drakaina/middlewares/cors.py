...
from drakaina.utils import _iterable_str_arg
...
class CORSMiddleware(BaseMiddleware):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._allow_origin = ", ".join(_iterable_str_arg(allow_origin))

        if allow_methods:
            if "*" in allow_methods:
                _allow_methods = ALL_METHODS
            else:
                _allow_methods = ", ".join(_iterable_str_arg(allow_methods))
        else:
        ...
        if allow_headers:
            self._allow_headers = ", ".join(
                sorted(
                    set(_iterable_str_arg(DEFAULT_HEADERS))
                    | set(_iterable_str_arg(allow_headers)),
                ),
            )
        else:
        ...
        if expose_headers:
            _expose_headers = ", ".join(_iterable_str_arg(expose_headers))
        else:
