...
class GroupsView(ValidatingView):
    ...
    async def post(self):
        ...
        async with self.request.app['db_pool'].acquire() as conn:
            await conn.execute(
                'INSERT INTO groups (name, attrs) VALUES ($1, $2)',
    ...
    async def get(self):
        ...
        async with self.request.app['db_pool'].acquire() as conn:
            group = await conn.fetchrow(
                'SELECT * FROM groups WHERE id = $1', z_id)
            if not group:
                raise web.HTTPNotFound
            return web.json_response(group_schema.dump(group))

    async def patch(self):
        z_id = int(self.request.match_info['id'])
        async with self.request.app['db_pool'].acquire() as conn:
            l_row = await conn.fetchrow(
                'SELECT name, attrs FROM groups WHERE id = $1', z_id)
            if not l_row:
                raise web.HTTPNotFound
            ...
            group.update(new)
            await conn.execute(
                'UPDATE groups SET name = $1, attrs = $2 WHERE id = $3',
