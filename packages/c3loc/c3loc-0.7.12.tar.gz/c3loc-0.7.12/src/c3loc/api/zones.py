...
class ZonesView(ValidatingView):
    ...
    async def post(self):
        ...
        async with self.request.app['db_pool'].acquire() as conn:
            await conn.execute(
                'INSERT INTO zones (name, attrs) VALUES ($1, $2)',
    ...
    async def get(self):
        ...
        async with self.request.app['db_pool'].acquire() as conn:
            zone = await conn.fetchrow(
                'SELECT * FROM zones WHERE id = $1', z_id)
            if not zone:
                raise web.HTTPNotFound
            return web.json_response(zone_schema.dump(zone))

    async def patch(self):
        z_id = int(self.request.match_info['id'])
        async with self.request.app['db_pool'].acquire() as conn:
            l_row = await conn.fetchrow(
                'SELECT name, attrs FROM zones WHERE id = $1', z_id)
            if not l_row:
                raise web.HTTPNotFound
            ...
            zone.update(new)
            await conn.execute(
                'UPDATE zones SET name = $1, attrs = $2 WHERE id = $3',
