...
def to_dask_dataframes_xyw(lazy_tabular_data):
    ddf = to_dask_dataframe(lazy_tabular_data.data)
...
def to_dask_dataframe(dataflow):
    import dask.dataframe as dd
    from dask.delayed import delayed

    partition_count = dataflow.get_partition_count()
    if partition_count <= 0:
        return dd.from_pandas(pd.DataFrame(), chunksize=1)

    delayed_functions = \
        [delayed(dataflow._partition_to_pandas_dataframe)(i, False, True, 'null', 'null')
         for i in range(0, partition_count)]

    # Using verify_meta=False allows us to let dask not check for type-schema
    # This is OK since the featurized data is produced by AutoML
    return dd.from_delayed(delayed_functions, verify_meta=False)
