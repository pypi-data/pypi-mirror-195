...
class AthenaAdapter(SQLAlchemyAdapter):
    ...
    def _get_distinct_array_agg_func(self, field_ref: FieldReference) -> Any:
        return SA.func.split(
            SA.func.array_join(
                SA.func.array_agg(SA.distinct(field_ref)), ARRAY_JOIN_SEP
            ),
            ARRAY_JOIN_SEP,  # This removes the Nulls from the array
        )
    ...
    def _parse_map_type(
                ...
                SA.case(
                    [
                        (
                            ...
                            F.array_join(map_keys_func, ARRAY_JOIN_SEP),
                        )
                    ],
                    else_=None,
                ).label("sub_fields")
        ...
        keys = df.iat[0, 0].split(ARRAY_JOIN_SEP)
