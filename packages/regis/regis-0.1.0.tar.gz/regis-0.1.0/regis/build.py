...
def __launch_new_build(project : str, config : str, compiler : str, shouldClean : bool, alreadyBuild : [str]):
  ...
  if project_file_path == "":
    rpy.diagnostics.log_err(f"project '{project}' was not found, have you generated it?")
    return 1, alreadyBuild
    
  json_blob = rpy.rex_json.load_file(project_file_path)
  ...
  if compiler not in json_blob[project_lower]:
    rpy.diagnostics.log_err(f"no compiler '{compiler}' found for project '{project}'")
    return 1, alreadyBuild
  
  if config not in json_blob[project_lower][compiler_lower]:
    rpy.diagnostics.log_err(f"no config '{config}' found in project '{project}' for compiler '{compiler}'")
    return 1, alreadyBuild
  ...
  rpy.diagnostics.log_info(f"Building: {project}")

  ninja_path = tool_paths_dict["ninja_path"]
  if shouldClean:
    proc = rpy.subproc.run(f"{ninja_path} -f {ninja_file} -t clean")
    proc.wait()

  proc = rpy.subproc.run(f"{ninja_path} -f {ninja_file}")
  proc.wait()
