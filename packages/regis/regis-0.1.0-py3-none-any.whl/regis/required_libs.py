...
if os.path.exists(lib_paths_filepath):
  lib_paths_dict = rpy.rex_json.load_file(lib_paths_filepath)
required_libs = []
not_found_libs = []

def __load_required_libs_dict():
  libs_required = []
  json_blob = rpy.rex_json.load_file(os.path.join(root, "build", "config", "required_libs.json"))
...
def __print_lib_found(lib_path, path : str):
  rpy.diagnostics.log_no_color(f"{lib_path} found at {path}")
...
def __find_uncached_paths(lib):
  ...
  for lib_path in required_lib_paths:
    ...
    abs_path = rpy.util.find_directory_in_paths(lib_path, cached_lib_paths)
    ...
    if not os.path.exists(abs_path):
      rpy.diagnostics.log_warn(f"lib path cached but doesn't exist: {lib_path}")
      lib_paths_to_search.append(lib_path)
      continue
...
def __look_for_paths(lib, pathsToSearch : list[str], whereToSearch : list[str]):
  ...
  for path in pathsToSearch:
    abs_path = rpy.util.find_directory_in_paths(path, whereToSearch)
    if abs_path == None:
...
def __download_lib(name, version, numZipFiles):
  task_print = rpy.task_raii_printing.TaskRaiiPrint(f"Downloading lib {name} {version}")
...
def __unzip_lib(name):
  ...
  rpy.diagnostics.log_info(f"libs unzipped to {libs_install_dir}")
...
def __look_for_required_libs(required_libs):
  not_found_libs = []
  install_paths = rpy.util.env_paths()
  ...
  for required_lib in required_libs:
    if not __is_up_to_date(install_paths, required_lib):
      rpy.diagnostics.log_err(f"{required_lib['archive_name']} out of date")
    ...
    if len(paths_not_found) > 0:
      rpy.diagnostics.log_warn("Couldn't find some paths")
      
      for path in paths_not_found:
        rpy.diagnostics.log_warn(path)
...
def __are_installed():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("Checking if libs are installed")
...
def __download():
  ...
  for lib in libs_to_download:
    ...
    __unzip_lib(lib)
    rpy.util.create_version_file(os.path.join(libs_install_dir, lib["archive_name"]), lib["version"])
...
def __install():
  for lib in not_found_libs:
    ...
    if len(paths_not_found) > 0:
      rpy.diagnostics.log_err(f"failed to install {config_name}")
...
def run():
  ...
  rpy.rex_json.save_file(lib_paths_filepath, lib_paths_dict)
