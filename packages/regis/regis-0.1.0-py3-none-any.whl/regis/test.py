...
import rpy.rex_json
import rpy.code_coverage
import rpy.diagnostics
import rpy.generation
import rpy.build
...
tool_paths_dict = rpy.required_tools.tool_paths_dict
...
def __default_output_callback(output):
  ...
  for line in iter(output.readline, b''):
    ...
    if __is_in_line(new_line, error_keywords):
      rpy.diagnostics.log_err(new_line)
      continue
    elif __is_in_line(new_line, warn_keywords):
      rpy.diagnostics.log_warn(new_line)
      continue
    
    rpy.diagnostics.log_no_color(new_line)

def __run_include_what_you_use(fixIncludes = False):
  task_print = rpy.task_raii_printing.TaskRaiiPrint("running include-what-you-use")

  rpy.generation.new_generation(os.path.join(root_path, "build", "config", "settings.json"), "")

  intermediate_folder = os.path.join(root_path, settings["intermediate_folder"], settings["build_folder"])
  result = rpy.util.find_all_files_in_folder(intermediate_folder, "compile_commands.json")
  ...
  for compiler_db in result:
    ...
    rpy.diagnostics.log_info(f"include what you use info saved to {output_path}")
...
def __run_clang_tidy():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("running clang-tidy")
  ...
  for compiler_db in result:
    ...
    header_filters = rpy.util.retrieve_header_filters(compiler_db_folder, project_name)
    header_filters_regex = rpy.util.create_header_filter_regex(header_filters)
    ...
    proc = rpy.util.run_subprocess_with_callback(cmd, __default_output_callback)
    new_rc = rpy.util.wait_for_process(proc)
    if new_rc != 0:
      rpy.diagnostics.log_err(f"clang-tidy failed for {compiler_db}")
      rpy.diagnostics.log_err(f"config file: {config_file_path}")
    rc |= new_rc

  return rc

def __generate_test_files(sharpmakeArgs):
  root = rpy.util.find_root()
  settings_path = os.path.join(root, "build", "config", "settings.json")
  proc = rpy.generation.new_generation(settings_path, sharpmakeArgs)
  proc.wait()
  ...
  for project in projects:
    for config in configs:
      for compiler in compilers:
        result |= rpy.build.new_build(project, config, compiler, should_clean)

  return result

def __find_test_programs(folder, regex):
  intermediate_folder = os.path.join(folder)
  rpy.diagnostics.log_info(f"looking for executables in {os.path.join(root_path, intermediate_folder)}")
  result = rpy.util.find_all_files_in_folder(os.path.join(root_path, intermediate_folder), regex)
  coverage_programs = []
  for res in result:
    if rpy.util.is_executable(res):
...
def __generate_tests():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("generating unit test projects")
  return __generate_test_files("/generateUnitTests")

def __build_tests():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("building tests")
  return __build_test_files("test", ["debug", "debug_opt", "release"], ["msvc", "clang"])

def __run_unit_tests():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("running unit tests")
  ...
  for program in unit_test_programs:
    rpy.diagnostics.log_info(f"running: {Path(program).name}")
    proc = rpy.util.run_subprocess(program)
    new_rc = rpy.util.wait_for_process(proc)
    if new_rc != 0:
      rpy.diagnostics.log_err(f"unit test failed for {program}") # use full path to avoid ambiguity
    rc |= new_rc
...
def __generate_coverage():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("generating coverage code")
  return __generate_test_files("/generateUnitTests /enableCoverage")

def __build_coverage():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("building coverage code")
  return __build_test_files("_coverage", ["coverage"], ["clang"])

def __run_coverage():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("running coverage")
...
def __relocate_coverage_data(programsRun : list[str]):
  task_print = rpy.task_raii_printing.TaskRaiiPrint("relocating coverage files")
  data_files = []
...
def __index_rawdata_files(rawdataFiles : list[str]):
  task_print = rpy.task_raii_printing.TaskRaiiPrint("indexing rawdata files")
  output_files = []

  for file in rawdataFiles:
    output_files.append(rpy.code_coverage.create_index_rawdata(file))

  return output_files

def __create_coverage_reports(programsRun, indexdataFiles):
  task_print = rpy.task_raii_printing.TaskRaiiPrint("creating coverage reports")
  ...
  for index in range(len(programsRun)):
    ...
    if Path(program).stem != Path(indexdata_file).stem:
      rc = 1
      rpy.diagnostics.log_err(f"program stem doesn't match coverage file stem: {Path(program).stem} != {Path(indexdata_file).stem}")

    rpy.code_coverage.create_line_oriented_report(program, indexdata_file)
    rpy.code_coverage.create_file_level_summary(program, indexdata_file)
    rpy.code_coverage.create_lcov_report(program, indexdata_file)

  return rc

def __parse_coverage_reports(indexdataFiles):
  task_print = rpy.task_raii_printing.TaskRaiiPrint("parsing coverage reports")

  rc = 0
  for indexdata_file in indexdataFiles:
    report_filename = rpy.code_coverage.get_file_level_summary_filename(indexdata_file)
    rc |= rpy.code_coverage.parse_file_summary(report_filename)
...
def __generate_address_sanitizer():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("generating address sanitizer code")
  return __generate_test_files("/generateUnitTests /enableAddressSanitizer")

def __build_address_sanitizer():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("building address sanitizer code")
  return __build_test_files("_asan", ["address_sanitizer"], ["clang"])

def __run_address_sanitizer():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("running address sanitizer tests")
  ...
  for program in unit_test_programs:
    ...
    proc = rpy.util.run_subprocess_with_working_dir(program, log_folder)
    new_rc = rpy.util.wait_for_process(proc)
    log_file_path = os.path.join(log_folder, f"asan.log.{proc.pid}")
    if new_rc != 0 or os.path.exists(log_file_path):
      rpy.diagnostics.log_err(f"address sanitizer failed for {program}") # use full path to avoid ambiguity
      rpy.diagnostics.log_err(f"for more info, please check: {log_file_path}")
      new_rc = 1
    rc |= new_rc
...
def __generate_undefined_behavior_sanitizer():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("generating undefined behavior sanitizer code")
  return __generate_test_files("/generateUnitTests /enableUBSanitizer")

def __build_undefined_behavior_sanitizer():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("building undefined behavior sanitizer code")
  return __build_test_files("_ubsan", ["undefined_behavior_sanitizer"], ["clang"])

def __run_undefined_behavior_sanitizer():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("running undefined behavior sanitizer tests")
  ...
  for program in unit_test_programs:
    ...
    if new_rc != 0 or os.path.exists(log_file_path): # if there's a ubsan.log.pid created, the tool found issues
      rpy.diagnostics.log_err(f"undefined behavior sanitizer failed for {program}") # use full path to avoid ambiguity
      rpy.diagnostics.log_err(f"for more info, please check: {log_file_path}")
      new_rc = 1
    rc |= new_rc
...
def __generate_fuzzy_testing():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("generating fuzzy testing code")
  return __generate_test_files("/enableFuzzyTesting")

def __build_fuzzy_testing():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("building fuzzy testing code")
  return __build_test_files("_fuzzy", ["fuzzy"], ["clang"])

def __run_fuzzy_testing():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("running fuzzy tests")
  ...
  for program in fuzzy_programs:
    ...
    proc = rpy.util.run_subprocess_with_working_dir(f"{program} -runs={num_runs}", log_folder)
    new_rc = rpy.util.wait_for_process(proc)
    log_file_path = os.path.join(log_folder, f"fuzzy.log.{proc.pid}")
    if new_rc != 0 or os.path.exists(log_file_path): # if there's a ubsan.log.pid created, the tool found issues
      rpy.diagnostics.log_err(f"fuzzy testing failed for {program}") # use full path to avoid ambiguity
      if os.path.exists(log_file_path):
        rpy.diagnostics.log_err(f"issues found while fuzzing!")
        rpy.diagnostics.log_err(f"for more info, please check: {log_file_path}")
      new_rc = 1
    rc |= new_rc
...
def test_include_what_you_use():
  rpy.diagnostics.log_no_color("-----------------------------------------------------------------------------")
  rc = __run_include_what_you_use()

  if rc != 0:
    rpy.diagnostics.log_err(f"include-what-you-use pass failed")
...
def test_clang_tidy():
  ...
  if rc != 0:
    rpy.diagnostics.log_err(f"clang-tidy pass failed")
...
def test_unit_tests():
  ...
  if rc != 0:
    rpy.diagnostics.log_err(f"failed to generate tests")
  __pass_results["unit tests generation"] = rc

  rpy.diagnostics.log_no_color("-----------------------------------------------------------------------------")
  rc |= __build_tests() # works
  if rc != 0:
    rpy.diagnostics.log_err(f"failed to build tests")
  __pass_results["unit tests building"] = rc

  rpy.diagnostics.log_no_color("-----------------------------------------------------------------------------")
  rc |= __run_unit_tests() # works
  if rc != 0:
    rpy.diagnostics.log_err(f"unit tests failed")
...
def test_code_coverage():
  ...
  if rc != 0:
    rpy.diagnostics.log_err(f"failed to generate coverage")
  __pass_results["coverage generation"] = rc

  rpy.diagnostics.log_no_color("-----------------------------------------------------------------------------")
  rc = __build_coverage() # works
  if rc != 0:
    rpy.diagnostics.log_err(f"failed to build coverage")
  ...
  if rc != 0:
    rpy.diagnostics.log_err(f"failed to create coverage reports")
  __pass_results["coverage report creation"] = rc

  rpy.diagnostics.log_no_color("-----------------------------------------------------------------------------")
  rc |= __parse_coverage_reports(indexdata_files) # works
  if rc != 0:
    rpy.diagnostics.log_err(f"Not all the code was covered")
...
def test_asan():
  ...
  if rc != 0:
    rpy.diagnostics.log_err(f"failed to generate asan code")
  __pass_results["address sanitizer generation"] = rc

  rpy.diagnostics.log_no_color("-----------------------------------------------------------------------------")
  rc |= __build_address_sanitizer() # works
  if rc != 0:
    rpy.diagnostics.log_err(f"failed to build asan code")
  __pass_results["address sanitizer building"] = rc
  
  rpy.diagnostics.log_no_color("-----------------------------------------------------------------------------")
  rc |= __run_address_sanitizer() # works
  if rc != 0:
    rpy.diagnostics.log_err(f"invalid code found with asan")
...
def test_fuzzy_testing():
  ...
  if rc != 0:
    rpy.diagnostics.log_err(f"failed to generate fuzzy code")
  __pass_results["fuzzy testing generation"] = rc
  
  rpy.diagnostics.log_no_color("-----------------------------------------------------------------------------")
  rc |= __build_fuzzy_testing() # works
  if rc != 0:
    rpy.diagnostics.log_err(f"failed to build fuzzy code")
  __pass_results["fuzzy testing building"] = rc
  
  rpy.diagnostics.log_no_color("-----------------------------------------------------------------------------")
  rc |= __run_fuzzy_testing() # works
  if rc != 0:
    rpy.diagnostics.log_err(f"invalid code found with fuzzy")
  __pass_results["fuzzy testing result"] = rc

def __shutil_error(func, path, exec_info):
  rpy.diagnostics.log_err(f"shutil error: {func}, {path}, {exec_info}")
