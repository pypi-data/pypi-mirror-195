...
def run(projectName, compdb, srcRoot):
  ...
  headerFilters = rpy.util.retrieve_header_filters(compdb, project)
  headerFiltersRegex = rpy.util.create_header_filter_regex(headerFilters)

  clang_tidy_path = rpy.required_tools.tool_paths_dict["clang_tidy_path"]
  clang_format_path = rpy.required_tools.tool_paths_dict["clang_format_path"]
  clang_apply_replacements_path = rpy.required_tools.tool_paths_dict["clang_apply_replacements_path"]
  clang_config_file = os.path.join(compdb, clang_tidy_first_pass_filename)

  rpy.diagnostics.log_info("Running clang-tidy - auto fixes")
  ...
  rpy.diagnostics.log_info("Running clang-format")
...
if __name__ == "__main__":
 ...
 # useful for debugging
  rpy.diagnostics.log_info(f"Executing {__file__}")

 # execute the script
  run(args.project, args.compdb, args.srcroot)

 # print. We're done.
  rpy.diagnostics.log_info("Done.")
