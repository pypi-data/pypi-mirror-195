...
def __load_sharpmake_files(sharpmakePath : str):
  settings = rpy.rex_json.load_file(sharpmakePath)
...
def new_generation(settingsPath : str, sharpmakeArgs : list[str]):
  ...
  if len(sharpmake_path) == 0:
    rpy.diagnostics.log_err("Failed to find sharpmake path")
    return
  ...
  return rpy.subproc.run(f"{sharpmake_path} /sources({sharpmake_sources}) /diagnostics {sharpmakeArgs}")
