...
settings = rpy.rex_json.load_file(os.path.join(root, "build", "config", "settings.json"))
...
if os.path.exists(tool_paths_filepath):
  tool_paths_dict = rpy.rex_json.load_file(tool_paths_filepath)
required_tools = []
not_found_tools = []

def __load_required_tools_dict():
  tools_required = []
  json_blob = rpy.rex_json.load_file(os.path.join(root, "build", "config", "required_tools.json"))
...
def __print_tool_found(tool, path : str):
  rpy.diagnostics.log_no_color(f"{tool['config_name']} found at {path}")

def __get_tool_extension(tool):
  extension = "" 
  if "extension" in tool:
    extension = tool["extension"]
  elif rpy.util.is_windows():
    extension = ".exe"
...
def __look_for_tools(required_tools):
  ...
  for required_tool in required_tools:
    ...
    version = rpy.util.load_version_file(path)

    # check if our version is up to date
    if version != required_tool["version"]:
      rpy.diagnostics.log_err(f"{stem} is out of date")
    ...
    if config_name in tool_paths_dict:
      ...
      if (os.path.exists(tool_path)):
          rpy.diagnostics.log_no_color(f"{stem} found at {tool_path}")
          continue
      else:
        rpy.diagnostics.log_err(f"Error: tool path cached, but path doesn't exist: {tool_path}")
        not_found_tools.append(required_tool)
        continue
    ...
    tool_path = rpy.util.find_file_in_paths(f"{stem}{exe_extension}", paths_to_use)
...
def are_installed():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("Checking if tools are installed")
  ...
  if len(not_found_tools) == 0:
    rpy.diagnostics.log_info("All tools found")
    rpy.rex_json.save_file(tool_paths_filepath, tool_paths_dict)
    return True
  else:
    rpy.diagnostics.log_warn(f"Tools that weren't found: ")
    for tool in not_found_tools:
      rpy.diagnostics.log_warn(f"\t-{tool['stem']}")
...
def __download_tool(name, version, numZipFiles):
  task_print = rpy.task_raii_printing.TaskRaiiPrint(f"Downloading tool {name} {version}")
...
def __download_tools_archive():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("Downloading tools")
...
def __unzip_tools():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("Unzipping files")
  ...
  rpy.diagnostics.log_info(f"tools unzipped to {tools_install_dir}")
...
def __create_version_files(toolsToDownload : []):
  for tool in toolsToDownload:
    ...
    rpy.util.create_version_file(path, tool["version"])
...
def install():
  task_print = rpy.task_raii_printing.TaskRaiiPrint("installing tools")
  ...
  for tool in not_found_tools:
    ...
    path = rpy.util.find_file_in_folder(f"{tool['stem']}{exe_extension}", os.path.join(tools_install_dir, tool["path"]))

    # if not found, something is wrong and we have to investigate manually
    if path == "":
      tool_name = tool["stem"]
      rpy.diagnostics.log_err(f"failed to find {tool_name}")
    else:
