...
def __get_host(path):
    ...
    rpy.diagnostics.log_info("Unknown host!")
    return Host.UNKNOWN
...
def __build_host_path(baseUrl, name, tag):
    ...
    if host == Host.GITHUB:
        ...
        rpy.diagnostics.log_err(f"Unknown url host: {host} in url: {baseUrl}")
        return ""

def __load_externals_required():
    root = rpy.util.find_root()

    json_blob = rpy.rex_json.load_file(os.path.join(root, "build", "config", "required_externals.json"))
    if json_blob == None:
        rpy.diagnostics.log_err("Loaded json blob is None, stopping json parse")
        return []
...
def __download_external(url):
    ...
    if not os.path.exists(url_basename):
        ...
        if response.status_code == requests.codes.ok:
            ...
            rpy.diagnostics.log_err(f"Bad request [{str(response.status_code)}] for given url: {url}")
            return []
    ...
    rpy.diagnostics.log_info("Extracting: " + url)
    ...
    rpy.diagnostics.log_info("Looking for added extracted directories ...")
    added_directory_names = []
    for post_dir in post_list_dir:
        count = pre_list_dir.count(post_dir)
        if count == 0:
            added_directory_names.append(post_dir)
    rpy.diagnostics.log_info(f"Found ({str(len(added_directory_names))}): ".join(added_directory_names))
...
def __verify_external(externalPath, requiredTag):
    ...
    if os.path.exists(externalPath):
        rpy.diagnostics.log_no_color(f"External found: {external_name}")
        rpy.diagnostics.log_no_color(f"validating version ...")
        version = rpy.util.load_version_file(externalPath)
        if version != requiredTag:
            rpy.diagnostics.log_err(f"Invalid version data found, redownloading external: {external_name}")
            return False
        return True
...
def __install_external(external):
    ...
    if not valid_external:    
        ...
        if len(added_directories) == 1:
            ...
            rpy.diagnostics.log_err("No directories where extracted.")
            return

        rpy.util.create_version_file(externals_dir, external_tag)   

def run():
    rpy.diagnostics.log_info("Start installing externals ...")

    externals_required = __load_externals_required()
    if externals_required == None:
        rpy.diagnostics.log_err("Required externals is None, exiting ...")
        return
