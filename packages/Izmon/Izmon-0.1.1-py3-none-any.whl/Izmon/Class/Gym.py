class Gym:
	...
	def battle_by_default(self):
		...
		print("~" * self.st.SEPALATE_LEN)
		action = self.co_f.typing("どのジムリーダーと戦うのじゃ？ ", 1, leadar_len)
		...
		for i in range(gym_num):
			...
			res = self.bat.battle(leadar[i], 1)
			if res == 0:
	...
	def gym_main(self):
		...
		while True:
			...
			action = self.co_f.typing("0:パーティーを変える 1:ジムを出る", 1, 2)
			#("0:パーティーを変える 1:ジムリーダーと戦う 2:ジムを出る", 1, 3)
			if action == 0:
				self.create_party()
			# elif action == 1:
			# 	if self.bat_s.set_ene_level(1) <= self.st.LEVEL_MAX[sum(self.co_v.badges) + 1]:
			# 		self.co_f.show("パーティーのレベルが低すぎるぞ")
			# 		self.co_f.show("もう少し強くしてから来るのじゃ")
			# 		continue
