class Start:
	...
	def set_up(self): #開始する
		...
		#不具合による一時的措置
			...
			print("不具合のためデータの読み込み機能を一時的に削除しています。Ver1.0.0のリリース時に再度追加予定です。")
			self.co_f.show("=" * self.st.SEPALATE_LEN)
			self.select_izmon()
		# else:
		# 	while True:
		# 		self.player = self.co_f.typing("君の名前を教えるのじゃ（セーブデータコードを持っているならその時の名前を教えるのじゃ）：", 0)
		# 		play_type = self.co_f.typing(self.player + " よ、セーブデータコードは持っておるか？ 1:ある 0:ない", 1, 2)
		# 		if play_type == 1:
		# 			self.co_f.show("=" * self.st.SEPALATE_LEN)
		# 			code = self.co_f.typing("セーブデータコード：",0)
		# 			self.co_f.show("=" * self.st.SEPALATE_LEN)
		# 			res = Decode.decode(code, 0)
		# 			if res == 0:
