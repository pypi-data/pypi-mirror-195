...
class JobDB(DIRACJobDB):
    ...
    def getTimings(self, site, period=3600):
        ...
        req = f"SELECT JobID from Jobs WHERE Site={site} and EndExecTime > '{date}' "
        result = self._query(req)
        jobList = [str(x[0]) for x in result["Value"]]
        jobString = ",".join(jobList)

        req = f"SELECT SUM(Value) from JobParameters WHERE Name='TotalCPUTime(s)' and JobID in ({jobString})"
        result = self._query(req)
        if not result["OK"]:
            return result
        ...
        req = f"SELECT SUM(Value) from JobParameters WHERE Name='WallClockTime(s)' and JobID in ({jobString})"
        result = self._query(req)
        if not result["OK"]:
            return result
