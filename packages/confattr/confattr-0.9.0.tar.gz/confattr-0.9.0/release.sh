...
download_dependencies() {
	...
	if which python3.6 >/dev/null; then
		run python3 -m pip download --python-version 3.6 --only-binary=:all: -f "$package_dir" -r requirements.txt -r <(grep -vF pudb requirements-test.txt) importlib-metadata -d "$package_dir"
