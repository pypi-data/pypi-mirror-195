...
class OolerBLEDevice:
    ...
    async def async_poll(self) -> None:
        ...
        power = bool(int.from_bytes(power_byte, "big"))
        mode_int = int.from_bytes(mode_byte, "big")
        mode = MODE_INT_TO_MODE_STATE[mode_int]
        settemp_int = int.from_bytes(settemp_byte, "big")
        actualtemp_int = int.from_bytes(actualtemp_byte, "big")
        waterlevel_int = int.from_bytes(waterlevel_byte, "big")
        pumpwatts_int = int.from_bytes(pumpwatts_byte, "big")
        clean = bool(int.from_bytes(clean_byte, "big"))
    ...
    async def set_power(self, power: bool) -> None:
        ...
        if client is not None:
            power_byte = int(power).to_bytes(1, "big")
    ...
    async def set_mode(self, mode: str) -> None:
        ...
        if client is not None:
            ...
            mode_byte = mode_int.to_bytes(1, "big")
    ...
    async def set_temperature(self, settemp_int: int) -> None:
        client = self._client
        if client is not None:
            settemp_byte = settemp_int.to_bytes(1, "big")
    ...
    async def set_clean(self, clean: bool) -> None:
        client = self._client
        if client is not None:
            clean_byte = int(clean).to_bytes(1, "big")
    ...
    async def check_connection_status(self, device: BLEDevice) -> bool:
        ...
        orig_power = bool(int.from_bytes(orig_power_byte, "big"))
        write_power_byte = int(not orig_power).to_bytes(1, "big")
