...
class Dispatcher:
    ...
    def __init__(self, client: "darmilibs.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await darmilibs.types.Message._parse(self.client, update.message, users, chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await darmilibs.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                darmilibs.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                darmilibs.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                darmilibs.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                darmilibs.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                darmilibs.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                darmilibs.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except darmilibs.StopPropagation:
                                raise
                            except darmilibs.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.error(e, exc_info=True)
